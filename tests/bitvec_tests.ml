...
module IntSet : sig
  ...
  let map f t = fold (fun n -> add (f n)) t empty

  let map2 f s t = fold (fun n -> fold (fun m -> add (f n m)) t) s empty
end
...
let of_interval = IntSet.of_fold fold_finite_domain

let of_bitlist = IntSet.of_fold Bitlist.fold_domain
...
let interval sz =
  ...
  Gen.(
    ...
    return @@ Intervals.Int.interval_set
  ...
  let rec bitlist sz =
    ...
    then return (Z.zero, Z.zero)
      ...
      then
        ...
        let* is_set = bool in
        if is_set
        then return (Z.logor set_bits mask, clr_bits)
        else return (set_bits, Z.logor clr_bits mask)
      else bitlist (sz - 1)
  in
  let* set_bits, clr_bits = bitlist sz in
  let set_bits =
    Bitlist.extract
      (Bitlist.ones @@ Bitlist.exact set_bits Explanation.empty)
      0 sz
  in
  let clr_bits =
    Bitlist.extract
...
let subvec sz =
  Gen.(
    ...
    int_range i (sz - 1) >>= fun j -> return (i, j))

(* Check that [Intervals.extract] computes a correct overapproximation of the
   set of values obtained by the [extract] smt-lib function. *)
let test_extract sz =
...
let () = Test.check_exn (test_extract 3)
...
let test_shared_msb sz =
  Test.make ~count:1_000
    ~print:
      Print.(pair (Fmt.to_to_string Z.pp_print) (Fmt.to_to_string Z.pp_print))
    Gen.(pair (bitvec sz) (bitvec sz))
    (fun (lb, ub) ->
      let lb, ub = if Z.compare lb ub > 0 then ub, lb else lb, ub in
      let shared = Bitv_rel.Test.shared_msb sz lb ub in
      Z.equal (Z.shift_right lb (sz - shared)) (Z.shift_right ub (sz - shared))
      && (shared = sz
         || not
            @@ Z.equal
...
let () = Test.check_exn (test_shared_msb 3)
...
(* Check that [increase_lower_bound b n] returns the smallest value larger than
   [n] that matches [b]. Also check that if the [Not_found] exception is raised,
   no such value exists. *)
let test_increase_lower_bound sz =
  Test.make ~count:1_000
    ~print:
      Print.(pair (Fmt.to_to_string Bitlist.pp) (Fmt.to_to_string Z.pp_print))
...
let () = Test.check_exn (test_increase_lower_bound 3)
    ...
    (fun (bl, z) ->
      ...
      | new_ub ->
        ...
        && all_range (Z.to_int new_ub + 1) (Z.to_int z) @@ fun i ->
           not (IntSet.mem (Z.of_int i) set)
      | exception Not_found ->
        all_range 0 (Z.to_int z) @@ fun i -> not (IntSet.mem (Z.of_int i) set))

let () = Test.check_exn (test_decrease_upper_bound 3)

let test_bitlist_binop ~count sz zop bop =
  Test.make ~count
    ~print:
      Print.(pair (Fmt.to_to_string Bitlist.pp) (Fmt.to_to_string Bitlist.pp))
...
let zmul sz a b = Z.extract (Z.mul a b) 0 sz

let test_bitlist_mul sz =
  test_bitlist_binop ~count:1_000 sz (zmul sz) (fun (module BV) -> BV.mul)

let () = Test.check_exn (test_bitlist_mul 3)
...
let test_interval_shl sz =
  test_interval_binop ~count:1_000 sz (zshl sz) (Intervals.Int.bvshl ~size:sz)

let () = Test.check_exn (test_interval_shl 3)

let test_bitlist_shl sz =
  test_bitlist_binop ~count:1_000 sz (zshl sz) (fun (module BV) -> BV.shl)

let () = Test.check_exn (test_bitlist_shl 3)
...
let test_interval_lshr sz =
  test_interval_binop ~count:1_000 sz zlshr Intervals.Int.lshr

let () = Test.check_exn (test_interval_lshr 3)

let test_bitlist_lshr sz =
  test_bitlist_binop ~count:1_000 sz zlshr (fun (module BV) -> BV.lshr)

let () = Test.check_exn (test_bitlist_lshr 3)

let zudiv sz a b =
  if Z.equal b Z.zero then Z.extract Z.minus_one 0 sz else Z.div a b

let test_interval_bvudiv sz =
  test_interval_binop ~count:1_000 sz (zudiv sz) (Intervals.Int.bvudiv ~size:sz)

let () = Test.check_exn (test_interval_bvudiv 3)

let zurem a b = if Z.equal b Z.zero then a else Z.rem a b

let test_interval_bvurem sz =
  test_interval_binop ~count:1_000 sz zurem Intervals.Int.bvurem

let () = Test.check_exn (test_interval_bvurem 3)
