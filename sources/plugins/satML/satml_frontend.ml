...
module Main : Sat_solver_sig.S = struct
  ...
  let selector env f orig =
    (lazy_sat () || lazy_inst () || not (MF.mem f env.gamma))
    ...
    List.fold_left
      (fun acc a ->
        ...
        try
          ...
          if not (lazy_sat () || lazy_inst ()) && MF.mem f env.gamma then acc
          else gf :: acc
        with Not_found -> acc
      ) acc sa
  ...
  let atoms_from_sat_branches env ~greedy_round ~frugal =
    let sa = match greedy_round || greedy (), lazy_sat () || lazy_inst () with
