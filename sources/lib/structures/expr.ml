...
let rec is_in_model e =
  ...
  match e with
  | { f = Sy.Form _; _ } -> false
  | { f = Sy.Lit x ; xs; _ } -> List.exists is_in_model xs
...
let mk_forall_ter =
  ...
  fun new_q id ->
    let { name; loc; binders; user_trs;
          main = f; sko_v ; sko_vty ; toplevel ; _ } = new_q
    in
...
let resolution_triggers ~is_back { main = f; name;  binders; _ } =
