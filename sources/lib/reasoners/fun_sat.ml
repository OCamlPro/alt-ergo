...
module Make (Th : Theory.S) : Sat_solver_sig.S = struct
  ...
  let extract_pred = Inst_gen.extract_pred
           ...
           | F.Clause(f1,f2,is_impl) ->
             ...
             let p1, p2 =
               if ground then
                 ...
                 | Inst_gen.Fail, _ -> p2, p1
                 | _, Inst_gen.Fail -> p1, p2
  ...
  let greedy_instantiation env =
    if greedy () then
      begin
        let f =
          try
            if check_if_sat env == [] then
              (fun e -> raise (Sat e))
            else
              (fun e -> raise (I_dont_know e))
          with Inst_gen.Not_sat t -> fun e -> raise (I_dont_know e)
        in
