...
module Relation (X : Enum.ALIEN) (Uf : Uf.S with type r = X.r) = struct
  type r = X.r
  type uf = Uf.t

  module Sh = Enum.Shostak(X)
  open Sh
  ...
  let count_splits env la =
    let nb =
      List.fold_left
        (fun nb (_,_,_,i) ->
           match i with
           | Sig_rel.CS (Sig_rel.Th_sum, n) -> Numbers.Q.mult nb n
           | _ -> nb
        )env.size_splits la
    in
  ...
  let case_split env uf ~for_model =
    ...
    | Some (n,r,hs) ->
      ...
      if for_model ||
        ...
        [LR.mkv_eq r r', true, Sig_rel.CS(Sig_rel.Th_sum, n)]
      else []
    | None ->
      Debug.no_case_split ();
      []
  ...
  let query env uf a_ex =
    try ignore(assume env uf [a_ex]); Sig_rel.No
