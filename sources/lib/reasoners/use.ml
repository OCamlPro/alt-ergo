...
module type S = sig
  ...
  type r
  val empty : t
  val find : r -> t -> E.Set.t * SA.t
  val add : r -> E.Set.t * SA.t -> t -> t
  val mem : r -> t -> bool
  val print : t -> unit
  val up_add : t -> E.t -> r -> r list -> t
  val congr_add : t -> r list -> E.Set.t
  val up_close_up :t -> r -> r -> t
  val congr_close_up : t -> r -> r list -> E.Set.t * SA.t
end
