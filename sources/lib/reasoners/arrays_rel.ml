...
module Relation (X : Arrays.ALIEN) (Uf : Uf.S with type r = X.r) = struct
         ...
         if Tmap.splited get set env.seen then (env,acc)
           ...
           | true , [stab;si;sv] ->
             ...
             let dep = match are_eq gtab set with
                 Sig_rel.Yes (dep, _) -> dep | Sig_rel.No -> assert false
             in
             let env =
               {env with new_terms =
  ...
  let case_split env uf ~for_model =
    ...
    try
      ...
      Debug.case_split a;
      [LR.view a, true, Sig_rel.CS (Sig_rel.Th_arrays, two)]
    with Not_found ->
  ...
  let count_splits env la =
    let nb =
      List.fold_left
        (fun nb (_,_,_,i) ->
           match i with
           | Sig_rel.CS (Sig_rel.Th_arrays, n) -> Numbers.Q.mult nb n
           | _ -> nb
        )env.size_splits la
    in
  ...
  let assume env uf la =
    ...
    let l =
      Conseq.fold (fun (a,ex) l ->
          ((Sig_rel.LTerm a, ex, Sig_rel.Other)::l)) atoms []
    in
