...
let mangle_regexp = if Sys.win32 then Str.regexp {|\|} else Str.regexp {|/|}
...
module File : sig
  ...
  (** [has_extension_in fl exts] check if the [fl] has an extension in the list
      [exts]. *)
end = struct
  let touch fl =
  ...
  let scan_folder path =
    ...
    let rec aux files folders h =
      ...
      | exception End_of_file ->
        Unix.closedir h;
        List.sort String.compare files, List.sort String.compare folders
      | "." | ".." | "slow" | "cram.t" -> aux files folders h
      | s ->
        let f = path // s in
  ...
  let has_extension_in file = List.mem (Filename.extension file)
end
...
module Cmd : sig
  ...
  val make : name:string -> group:string -> bin:string -> args:string list -> t
...
module Test : sig
  ...
  let pp_stanza fmt tst =
    let pp_diff_command fmt tst =
      if tst.params.compare_should_succeed
      then
        Format.fprintf fmt "@[(diff %a %a)@]" pp_expected_output tst pp_output
          tst
      else
        Format.fprintf fmt
          "@[(ignore-stdout (with-accepted-exit-codes (not 0) (run diff %a \
           %a)))@]"
          pp_expected_output tst pp_output tst
    in
    let accepted_ae_exit_code =
      Format.(
        asprintf "@[<2>(or@ @[%a@]@])"
          (pp_print_list pp_print_int)
          tst.params.accepted_exit_codes)
    in
    Format.fprintf fmt
      "@[<v 1>(rule@,\
       (target %a)@,\
       ...
       @[<v 1>(run @[<hv>%a@]))))))@]@]@]@]@]@]@ @[<v 1>(rule@,\
       @[<v 1>(deps %a)@,\
       @[<v 1>(alias %s)@,\
       @[<v 1>(package alt-ergo)@,\
       @[<v 1>(action@ %a))@]@]@]@]@]"
      pp_output tst pp_input tst accepted_ae_exit_code Cmd.pp tst.cmd pp_output
      tst (Cmd.group tst.cmd) pp_diff_command tst
end
...
module Batch : sig
  ...
  val make :
    root:string -> path:string -> cmds:Cmd.t list -> pb_files:string list -> t
    ...
    let tests =
      List.fold_left
        (fun acc1 pb_file ->
          let params =
            List.fold_left
              (fun (acc : Test.params) -> function
                | "unix" when not Sys.unix -> { acc with ignore = true }
                | "fail" -> { acc with compare_should_succeed = false }
                | "err" -> { acc with accepted_exit_codes = [1] }
                | "timeout" -> { acc with accepted_exit_codes = [142] }
                | "default" -> { acc with filters = Some ["default"] }
                | "tableaux" -> { acc with filters = Some ["tableaux"] }
                | _ -> acc)
              Test.base_params
          ...
          List.fold_left
            (fun acc2 cmd ->
              if (not params.ignore) && filter params cmd
              then Test.make ~cmd ~pb_file ~params ~root ~path :: acc2
              else acc2)
            acc1 cmds)
        [] pb_files
    in
  ...
  let pp_stanza : t printer =
   fun fmt batch ->
    ...
    fprintf fmt "@[<v 2>%a@]@ "
      (pp_print_list ~pp_sep Test.pp_stanza)
      batch.tests;
...
let rec generate ~kind ~root path cmds =
  ...
  let () =
    match List.filter is_a_problem files with
    | [] -> ()
    | pb_files -> (
...
module Commands = struct
  ...
  let speclist = ["--kind", Arg.String set_kind, "rule or expected"]
...
let () =
  ...
  let shared = [timelimit; "--enable-assertions"] in
  let solvers =
    [ "runtest-quick", "default", ["--output=smtlib2"];
      "runtest-quick", "tableaux", ["--output=smtlib2"; "--sat-solver Tableaux"];
      "runtest-quick", "cdcl", ["--output=smtlib2"; "--sat-solver CDCL"];
      ( "runtest-ci",
        "ci_tableaux_cdcl_no_minimal_bj",
        ["--output=smtlib2"; "--sat-solver CDCL-Tableaux"; "--no-minimal-bj"] );
      ( "runtest-ci",
        "ci_cdcl_tableaux_no_tableaux_cdcl_in_theories",
        [ "--output=smtlib2";
          "--sat-solver CDCL-Tableaux";
          "--no-tableaux-cdcl-in-theories" ] );
      ( "runtest-ci",
        "ci_no_tableaux_cdcl_in_instantiation",
        [ "--output=smtlib2";
          "--sat-solver CDCL-Tableaux";
      ...
      ( "runtest-ci",
        ...
        [ "--output=smtlib2";
          ...
          "--no-tableaux-cdcl-in-instantiation" ] );
      ( "runtest-ci",
        "ci_cdcl_tableaux_no_minimal_bj_no_tableaux_cdcl_in_theories_and_instantiation",
        [ "--output=smtlib2";
          "--sat-solver CDCL-Tableaux";
          "--no-minimal-bj";
          "--no-tableaux-cdcl-in-theories";
          "--no-tableaux-cdcl-in-instantiation" ] );
      ( "runtest-ci",
        "ci_cdcl_no_minimal_bj",
        ["--output=smtlib2"; "--sat-solver CDCL"; "--no-minimal-bj"] ) ]
  in
  let cmds =
    List.map
      (fun (group, name, args) ->
        let args = shared @ args in
        Cmd.make ~name ~group ~bin ~args)
      solvers
  in
