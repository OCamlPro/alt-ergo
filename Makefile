...
BUILD_DIR := _build
INSTALL_BUILD_DIR := $(BUILD_DIR)/install/default
...
clean: generated-clean dune-clean ocamldot-clean js-clean
...
bin: gen
	$(DUNE) build $(DUNE_FLAGS) $(INSTALL_BUILD_DIR)/bin/alt-ergo
	ln -sf $(INSTALL_BUILD_DIR)/bin/alt-ergo alt-ergo

# Build the GUI (dev)
gui: gen
	$(DUNE) build $(DUNE_FLAGS) $(INSTALL_BUILD_DIR)/bin/altgr-ergo
	ln -sf $(INSTALL_BUILD_DIR)/bin/altgr-ergo altgr-ergo
# fm-simplex plugin
fm-simplex:
	$(DUNE) build $(DUNE_FLAGS) \
		$(INSTALL_BUILD_DIR)/share/alt-ergo/plugins/fm-simplex-plugin.cma \
		$(INSTALL_BUILD_DIR)/share/alt-ergo/plugins/fm-simplex-plugin.cmxs
	ln -sf $(INSTALL_BUILD_DIR)/share/alt-ergo/plugins/fm-simplex-plugin.cma fm-simplex-plugin.cma
	ln -sf $(INSTALL_BUILD_DIR)/share/alt-ergo/plugins/fm-simplex-plugin.cmxs fm-simplex-plugin.cmxs


# Ab-Why3 plugin
AB-Why3:
	$(DUNE) build $(DUNE_FLAGS) \
		$(INSTALL_BUILD_DIR)/share/alt-ergo/plugins/AB-Why3-plugin.cma \
		$(INSTALL_BUILD_DIR)/share/alt-ergo/plugins/AB-Why3-plugin.cmxs
	ln -sf $(INSTALL_BUILD_DIR)/share/alt-ergo/plugins/AB-Why3-plugin.cma AB-Why3-plugin.cma
	ln -sf $(INSTALL_BUILD_DIR)/share/alt-ergo/plugins/AB-Why3-plugin.cmxs AB-Why3-plugin.cmxs
...
alt-ergo-lib:
	$(DUNE) build $(DUNE_FLAGS) \
		-p alt-ergo-lib @install

# Build the alt-ergo-parsers (release)
alt-ergo-parsers:
	$(DUNE) build $(DUNE_FLAGS) \
		 -p alt-ergo-lib,alt-ergo-parsers @install

# Build the cli/text alt-ergo (release)
alt-ergo:
	$(DUNE) build $(DUNE_FLAGS) \
		 -p alt-ergo-lib,alt-ergo-parsers,alt-ergo @install

# Build the GUI (release)
altgr-ergo:
	$(DUNE) build $(DUNE_FLAGS) \
		 -p alt-ergo-lib,alt-ergo-parsers,altgr-ergo @install
...
# non-regression
non-regression: all
	cp $(INSTALL_BUILD_DIR)/bin/alt-ergo non-regression/alt-ergo.opt
...
js-node: gen
	...
	ln -sf $(DEFAULT_BUILD_DIR)/$(BJS_DIR)/main_text_js.bc.js alt-ergo.js
...
js-worker: gen
	...
	ln -sf $(DEFAULT_BUILD_DIR)/$(BJS_DIR)/worker_js.bc.js alt-ergo-worker.js \
...
js-example: gen js-worker
	...
	mkdir -p www
	cp $(BJS_DIR)/index.html www/
	cd www \
	&& ln -sf ../$(DEFAULT_BUILD_DIR)/$(BJS_DIR)/worker_js.bc.js alt-ergo-worker.js \
	&& ln -sf ../$(DEFAULT_BUILD_DIR)/$(BJS_DIR)/worker_example.bc.js alt-ergo-main.js \
...
archi: $(EXTRA_DIR)/ocamldot/ocamldot
	ocamldep \
		...
		-I $(DEFAULT_BUILD_DIR)/$(COMMON_DIR)/ \
		$(DEFAULT_BUILD_DIR)/$(PARSERS_DIR)/ -I $(DEFAULT_BUILD_DIR)/$(PLUGINS_DIR)/ \
		$(FTND_DIR)/*.ml $(RSNRS_DIR)/*.ml $(STRCT_DIR)/*.ml $(UTIL_DIR)/*.ml \
	        $(COMMON_DIR)/*.ml $(DEFAULT_BUILD_DIR)/$(COMMON_DIR)/*.ml \
		$(PARSERS_DIR)/*.ml $(DEFAULT_BUILD_DIR)/$(PARSERS_DIR)/*.ml \
		$(PLUGINS_DIR)/*/*.ml $(DEFAULT_BUILD_DIR)/$(PLUGINS_DIR)/*/*.ml \
