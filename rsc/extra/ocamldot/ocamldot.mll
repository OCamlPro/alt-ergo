...
let fold_dir f init path =
  ...
  let rec fold_dir_ (acum, dirs) =
    ...
    | [] ->
      acum
    | dir :: dirs ->
      fold_dir_ (Array.fold_left
                   (fun ad p -> collect dir p ad)
                   (acum, dirs)
                   (Sys.readdir dir)
                ) in
...
let dir_to_mod_names graph dir =
  ...
  fold_dir (fun dir_to_mod_names path ->
      ...
      then
        ...
        let files =
          mod_name :: (
            try StringMap.find dir dir_to_mod_names with Not_found -> []
          ) in
...
Arg.parse (Arg.align [
    ...
    ("-r",
     ...
     "<r> use a root in the graph; nodes reachable from <r> will be shown")
  ])
  ...
  begin
    ...
    | roots ->
      ...
      let clusterDirs = List.fold_left
          (fun z s -> StringSet.add s z) StringSet.empty !clusters in
