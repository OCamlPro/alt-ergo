...
let remove_trailing_whitespaces line =
  ...
  if !new_len <> len then Some (String.sub line 0 !new_len) else None
...
let check_file file_opt =
  let cin =
    ...
    | Some file -> (
      try open_in file
      with e ->
        Format.eprintf "Error while opening (in) file: %s@.%s@." file
          (Printexc.to_string e);
        exit 2)
  in
  let lines, spacesRemoved, longLines = check_buffer file_opt cin in
  close_in cin;
  if spacesRemoved || Option.is_none file_opt
  then begin
    let cout =
      match file_opt with
      | None -> stdout
      | Some file -> (
        try open_out file
        with e ->
          Format.eprintf "Error while opening (out) file: %s@.%s@." file
            (Printexc.to_string e);
          exit 2)
    in
...
let () =
  ...
  | 0 | 1 -> check_file None
  | 2 -> check_file @@ Some Sys.argv.(1)
  | _ ->
    Format.eprintf "%s: Too many arguments!@." Sys.argv.(0);
    exit 4
