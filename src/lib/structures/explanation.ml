...
let union s1 s2 = if s1 == s2 then s1 else S.union s1 s2

let singleton e = S.singleton e

let mem e s = S.mem e s

let remove e s = if S.mem e s then S.remove e s else raise Not_found
...
let exists_fresh t = S.exists (function Fresh _ -> true | _ -> false) t

let remove_fresh fe s = if S.mem fe s then Some (S.remove fe s) else None

let add_fresh fe s = S.add fe s

let print fmt ex =
  let open Format in
  if Options.get_debug_explanations ()
  then begin
    fprintf fmt "{";
    S.iter
      (function
...
let print_unsat_core ?(tab = false) fmt dep =
  iter_atoms
    (function
      | RootDep r ->
        if tab
        then Format.fprintf fmt "  %s@." r.name (* tab is too big *)
...
let make_deps sf = E.Set.fold (fun l acc -> S.add (Bj l) acc) sf S.empty

let has_no_bj s =
  try
    S.iter (function Bj _ -> raise Exit | _ -> ()) s;
    true
  with Exit -> false
