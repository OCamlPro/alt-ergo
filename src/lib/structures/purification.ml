...
let add_let sy e (lets: (Expr.t * int) Sy.Map.t) =
  incr lets_counter;
  Sy.Map.add sy (e, !lets_counter) lets

let rec purify_term (t: Expr.t) lets =
  if t.pure then t, lets
    ...
    | (Sy.Lit _ | Sy.Form _), _ ->
      ...
      Expr.mk_term fresh_sy [] t.ty , add_let fresh_sy t lets
...
and purify_eq l =
  purify_generic (fun l ->
      ...
      | [a; b ] -> Expr.mk_eq ~iff:true a b
      | l -> Expr.mk_nary_eq l
    ) l

and purify_distinct l =
  purify_generic (fun l -> Expr.mk_distinct ~iff:true l) l

and purify_builtin neg pred l =
  purify_generic (fun l -> Expr.mk_builtin ~is_pos:neg pred l) l

and purify_predicate p is_neg =
  purify_generic (fun l ->
      match l with
      | [e] -> if is_neg then Expr.neg e else e
      | _ -> failwith "unexpected expression in purify_predicate"
    ) [p]

and purify_literal (e: Expr.t) =
  if List.for_all Expr.is_pure e.xs then e (* this is OK for lits and terms *)
  else match Expr.lit_view e with
    | Eq {lhs; rhs} ->
...
and purify_form (e: Expr.t) =
  assert (e.ty == Ty.Tbool);
  if Expr.is_pure e then e (* non negated predicates *)
  else
    match e.f with
    ...
    | Sy.Op Get ->
      begin match e.xs with
        | [fa; i] ->
          let fa', lets =
            if Expr.is_pure fa then fa, Sy.Map.empty
            else
              purify_term fa Sy.Map.empty
          in
          let i', lets =
            if Expr.is_pure i then i, lets
            else
              match i.ty with
          ...
          in
          if i == i' && fa == fa' then e
          else mk_lifted (Expr.mk_term e.f [fa'; i'] e.ty) lets
    ...
    | Sy.Form x ->
      begin match x, e.xs, e.bind with
        | Sy.F_Unit imp, [a;b], _ ->
          ...
          if a == a' && b == b' then e else Expr.mk_and a' b' imp

        | Sy.F_Clause imp, [a;b], _ ->
          let a' = purify_form a in
          let b' = purify_form b in
          if a == a' && b == b' then e else Expr.mk_or a' b' imp

        | Sy.F_Iff, [a;b], _ ->
          let a' = purify_form a in
          let b' = purify_form b in
          if a == a' && b == b' then e else Expr.mk_iff a' b'

        | Sy.F_Xor, [a;b], _ ->
          let a' = purify_form a in
          let b' = purify_form b in
          if a == a' && b == b' then e else Expr.mk_xor a' b'

        | Sy.F_Lemma, [], B_lemma q ->
          let m = purify_form q.main in
          if m == q.main then e
          else Expr.mk_forall_ter {q with main = m}

        | Sy.F_Skolem, [], B_skolem q ->
          let m = purify_form q.main in
          if m == q.main then e
          else Expr.(neg (mk_forall_ter {q with main = (neg m)}))
...
and mk_lifted e lets =
  ...
  List.fold_left
    (fun acc (let_v, (let_e, _cpt)) ->
       ...
       mk_lifted (Expr.mk_let let_v let_e acc) lets
    )e ord_lets

and purify_non_toplevel_ite (e: Expr.t) lets =
  match e.f, e.xs with
  | _, [c; th; el] when Sy.is_ite e.f ->
    ...
    Expr.mk_term e.f [c; th; el] e.ty, lets
