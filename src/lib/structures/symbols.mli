...
type name_space =
  ...
  (** This symbol is a "fresh" internal name. Fresh internal names play a
      ...
      {!val:Id.Namespace.Internal.fresh}.
  ...
  (** This symbol has been introduced as part of model generation, and
      represents an abstract value in sense of the SMT-LIB standard.

      To ensure uniqueness, abstract names must always be generated using
      {!val:Id.Namespace.Abstract.fresh}. *)
  | GetValue
  (** This symbol is generated by the `get-value` statement to track
      new values computed in the model.

      To ensure uniqueness, get-value names must always be generated using
      {!val:Id.Namespace.GetValue.fresh}.

      In particular, this symbol can appear in a {!type:ModelMap.t} but
      they aren't printed by {!val:ModelMap.pp}. *)
...
module Name : sig
  ...
  (** [mk ~kind ~defined ~ns id] creates a new name with the given identifier
      [id].
...
(** [is_internal sy] checks if the symbol [sy] is an internal name that
    should never be printed on the regular output. *)
