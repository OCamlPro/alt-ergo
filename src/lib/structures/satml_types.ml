...
module type ATOM = sig
  ...
  val make_clause : string -> atom list -> E.t -> bool -> premise -> clause
...
(* (*module Make (Dummy : sig end) : sig*)
   ...
   val literal : atom -> E.t val weight : atom -> float val is_true : atom ->
   bool val level : atom -> int val index : atom -> int val neg : atom -> atom

   val cpt_mk_var : int ref val ma : var E.Map.t ref

   val make_var : E.t -> var * bool

   val get_atom : E.t -> atom (* get existing atom of a lit *) val vrai_atom :
   atom val faux_atom : atom
   ...
   val cmp_var : var -> var -> int val eq_var : var -> var -> bool val h_var :
   var -> int val tag_var : var -> int
...
module Atom : ATOM = struct
  ...
  module Debug = struct
    ...
    let value a =
      if a.is_true
      then Format.sprintf "[T%s]" (level a)
      else if a.neg.is_true
      then Format.sprintf "[F%s]" (level a)
      else ""

    let premise fmt v =
      List.iter (fun { name; _ } -> Format.fprintf fmt "%s," name) v

    let atom fmt a =
      Format.fprintf fmt "%s%d%s [index=%d | lit:%a] vpremise={{%a}}" (sign a)
        (a.var.vid + 1) (value a) a.var.index Shostak.Literal.pp a.lit premise
        a.var.vpremise

    let atoms_vec = Vec.pp atom

    let clause fmt { name; atoms = arr; cpremise = cp; _ } =
      Format.fprintf fmt "%s:@[@[{ %a@]}@ cpremise={{%a}}@]" name atoms_vec arr
        premise cp
  end
  ...
  let copy_hcons_env hcons = { tbl = HT.copy hcons.tbl; cpt = ref !(hcons.cpt) }
  ...
  let reason_atoms a =
    ...
    | Some c ->
      ...
      Vec.iter
        (fun (atom : atom) ->
          if eq_atom a atom then incr cpt else l := atom :: !l)
        c.atoms;
      if !cpt <> 1
      then begin
        Printer.print_err "cpt = %d@ a = %a@ c = %a" !cpt pr_atom a pr_clause c;
        assert false
      end;
      !l
...
module type FLAT_FORMULA = sig
  ...
  val simplify :
    ...
    (E.t -> t * 'a) ->
    Atom.var list ->
    t * (E.t * (t * Atom.atom)) list * Atom.var list
...
module Flat_Formula : FLAT_FORMULA = struct
  ...
  (** Map from flat formula tags to their proxy definitions. If flat formula [f]
      ...
      Note: If [ff] is a flat formula of shape [OR fl] (resp. [AND fl]), then
      the corresponding [is_and] entry is [false] (resp. [true]), and the list
      [l] contains the atoms or proxies for the values in [fl].

      Note: the [proxies] map contains either a flat formula or its negation;
      [get_proxy_of] tries both possibilities. *)
    ...
    let equal f1 f2 =
      let eq_aux c1 c2 =
        ...
        | AND u, AND v | OR u, OR v -> (
          try
            List.iter2 (fun x y -> if x.tag <> y.tag then raise Exit) u v;
            true
          with
          | Exit -> false
          | Invalid_argument s ->
  ...
  let make hcons pos neg =
    ...
    let rec p = { view = pos; tag = 2 * !(hcons.cpt); neg = n }
    and n = { view = neg; tag = (2 * !(hcons.cpt)) + 1; neg = p } in
    let res =
  ...
  let mk_lit hcons a acc =
    ...
    let at =
      if Options.get_disable_flat_formulas_simplification ()
      then at
      else if at.Atom.var.Atom.level = 0
      then
        if at.Atom.is_true
        then Atom.vrai_atom
        else begin
  ...
  let empty_hcons_env, vrai =
    let empty_hcons =
      { tbl = HT.create 4096; cpt = ref 0; atoms = Atom.empty_hcons_env () }
    in
            ...
            let _, delta_u =
              List.fold_left
                (fun ((c, l) as acc) e ->
                  ...
                  if equal c e then acc else e, e :: l)
                (e, [e]) l
            in
            delta_u
        in
  ...
  let intersect_list l1 l2 =
    let rec inter l1 l2 acc =
      ...
      | f1 :: r1, f2 :: r2 ->
        ...
        if c = 0
        then inter r1 r2 (f1 :: acc)
        else if c > 0
        then inter l1 r2 acc
        else inter r1 l2 acc
    in
    inter l1 l2 []
  ...
  let extract_common l =
    ...
    | _ :: _ :: _, _ ->
      ...
      then
        Printer.print_dbg ~module_name:"Satml_types"
          ~function_name:"extract_common" "Failure: many distinct atoms";
      None
    | ([_] as common), _ ->
      if Options.get_debug_sat ()
      then
        Printer.print_dbg ~module_name:"Satml_types"
          ~function_name:"extract_common"
    ...
    fun hcons l ->
      try
        ...
        | l -> (
          ...
          | None -> begin
            match l with
            | [({ view = UNIT _; _ } as fa); { view = AND ands; _ }] -> begin
              try mk_or hcons [fa; mk_and hcons (remove_elt (mk_not fa) ands)]
              with Not_included ->
  ...
  let simplify hcons f abstr new_vars =
    ...
    let rec simp topl ~parent_disj f =
      ...
      | E.Skolem _ -> mk_not (simp false ~parent_disj:false (E.neg f))
      | E.Unit (f1, f2) ->
      ...
      | E.Iff (f1, f2) ->
        simp topl ~parent_disj @@ E.elim_iff f1 f2 ~with_conj:(not parent_disj)
      | E.Xor (f1, f2) ->
  ...
  let get_proxy_of f proxies_mp =
    try
      let p, _, _ = Util.MI.find f.tag proxies_mp in
      Some p
    with Not_found -> (
      try
        let p, _, _ = Util.MI.find f.neg.tag proxies_mp in
        Some p.Atom.neg
      with Not_found -> None)

  let expand_proxy_defn acc (p, l, is_and) =
    if is_and
    then (* p <=> (l1 and ... and l_n) *)
      let np = p.Atom.neg in
      let cl, acc =
        List.fold_left
          (fun (cl, acc) a -> a.Atom.neg :: cl, [np; a] :: acc)
          ([p], acc) l
      in
      cl :: acc
    else (* p <=> (l1 or ... or l_n) *)
      ...
      | AND l | OR l -> (
        ...
        | None ->
          let l, new_proxies, proxies_mp, new_vars =
            List.fold_left
              (fun (l, new_proxies, proxies_mp, new_vars) f ->
                let f, new_proxies, proxies_mp, new_vars =
                  abstr f new_proxies proxies_mp new_vars
                in
                f :: l, new_proxies, proxies_mp, new_vars)
              ([], new_proxies, proxies_mp, new_vars)
              l
          in
          let l = List.rev l in
          let p, new_vars =
            atom_of_lit hcons (mk_new_proxy f.tag) false new_vars
          in
          let is_and =
            match f.view with
            | AND _ -> true
