...
let columns : (string * string * int * bool option * 'a) list =
  [
    ...
    (fun _ _ sz ->
       ...
       string_resize (Self.show_mod_ kd) sz);
    ...
    (fun _ gtime sz ->
       let curr = Timers.get_sum (get_timers ()) Self.M_Arith in
       Format.sprintf "%s~%s"
         (float_resize curr (sz - 5)) (string_resize (percent gtime curr) 4));

    "Arrays", "Time spent in Arrays module(s)", 16, Some false,
    (fun _ gtime sz ->
       let curr = Timers.get_sum (get_timers ()) Self.M_Arrays in
       Format.sprintf "%s~%s"
         (float_resize curr (sz - 5)) (string_resize (percent gtime curr) 4));

    "Sum", "Time spent in Sum module(s)", 16, Some false,
    (fun _ gtime sz ->
       let curr = Timers.get_sum (get_timers ()) Self.M_Sum in
       Format.sprintf "%s~%s"
         (float_resize curr (sz - 5)) (string_resize (percent gtime curr) 4));

    "Records", "Time spent in Records module(s)", 16, Some false,
    (fun _ gtime sz ->
       let curr = Timers.get_sum (get_timers ()) Self.M_Records in
       Format.sprintf "%s~%s"
         (float_resize curr (sz - 5)) (string_resize (percent gtime curr) 4));

    "AC", "Time spent in AC module(s)", 16, Some false,
    (fun _ gtime sz ->
       let curr = Timers.get_sum (get_timers ()) Self.M_AC in
       Format.sprintf "%s~%s"
         (float_resize curr (sz - 5)) (string_resize (percent gtime curr) 4));
    ...
    (fun _ _ sz ->
       ...
       let tsat = Timers.get_sum timers Self.M_Sat in
       let tmatch = Timers.get_sum timers Self.M_Match in
       let tcc = Timers.get_sum timers Self.M_CC in
       let tarith = Timers.get_sum timers Self.M_Arith in
       let tarrays = Timers.get_sum timers Self.M_Arrays in
       let tsum = Timers.get_sum timers Self.M_Sum in
       let trecs = Timers.get_sum timers Self.M_Records in
       let tac = Timers.get_sum timers Self.M_AC in
  ...
  List.iter
    (fun (k, f, id) ->
       Format.fprintf fmt "(%s, %s, %s) --> "
         (string_resize (Self.show_mod_ k) 5)
         (string_resize (Self.show_fn f) 10)
         (int_resize id 7)
    )(List.rev stack);
  let m, f, id = Timers.current_timer timers in
  Format.fprintf fmt "(%s, %s, %s)@."
    (string_resize (Self.show_mod_ m) 5)
    (string_resize (Self.show_fn f) 10)
    (int_resize id 7)
...
let line_of_module =
  let open Format in
  fun timers fmt f ->
    fprintf fmt "%s " (string_resize (Self.show_fn f) 13);
    let cpt = ref 0. in
    List.iter
      (fun m ->
...
let timers_table =
  ...
  fun forced fmt ->
    ...
    List.iter
      (fun f ->
         fprintf fmt"| %s" (string_resize (Self.show_mod_ f) 9))
      Self.all_mod_;
    fprintf fmt "|@.";
    for _ = 0 to 206 do fprintf fmt "-" done;
    fprintf fmt "|@.";
    List.iter (line_of_module timers fmt) Self.all_fn;
...
let init () =
  ...
  List.iter
    (fun (m : Self.mod_) ->
       let name = Format.sprintf "timer-%s" (Self.show_mod_ m) in
       register_float_stat
         name
