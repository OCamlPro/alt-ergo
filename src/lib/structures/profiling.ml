...
(** Calls the continuation only when the profiling is on. This allows to only
    force the state when the profiling is on. *)
...
let query () = if_profiling @@ fun state -> state.queries <- state.queries + 1
...
let instantiation ilvl =
  ...
  if not (state.instantiation_lvl = ilvl)
  then begin
    Printer.print_err "state.instantiation_lvl = %d et ilvl = %d"
      state.instantiation_lvl ilvl;
    assert false
  end

let bool_conflict () =
  if_profiling @@ fun state -> state.b_conflicts <- state.b_conflicts + 1

let theory_conflict () =
  if_profiling @@ fun state -> state.th_conflicts <- state.th_conflicts + 1

let bcp_conflict b1 b2 =
  if_profiling @@ fun state ->
  if b1 && b2
  then state.bcp_b_conflicts <- state.bcp_b_conflicts + 1
  else if (not b1) && not b2
  then state.bcp_th_conflicts <- state.bcp_th_conflicts + 1
  else state.bcp_mix_conflicts <- state.bcp_mix_conflicts + 1

let red b =
  if_profiling @@ fun state ->
  if b then state.b_red <- state.b_red + 1 else state.t_red <- state.t_red + 1

let elim b =
  if_profiling @@ fun state ->
  if b
  then state.b_elim <- state.b_elim + 1
  else state.t_elim <- state.t_elim + 1

let reset_ilevel n = if_profiling @@ fun state -> state.instantiation_lvl <- n

let reset_dlevel n = if_profiling @@ fun state -> state.decision_lvl <- n
  ...
  | ii ->
    ...
    then
      Errors.internal_error
        "Instances for axiom '%s' have incompatible locations.@ @ @[<v \
         2>Previous location was:@ %a@]@ @[<v 2>Now registering at location:@ \
         %a@]"
...
let new_instance_of axiom inst loc kept =
  ...
  let ii =
    if kept
    then { ii with kept = ii.kept + 1; all_insts = SE.add inst ii.all_insts }
    else { ii with ignored = ii.ignored + 1 }
  in
...
let decision_on_instance axiom_name =
  if_profiling @@ fun state ->
  try
    let ii = MS.find axiom_name state.instances_map in
...
let decision d origin =
  ...
  if state.decision_lvl <> d
  then begin
    Printer.print_err "state.decision_lvl = %d et d = %d" state.decision_lvl d;
    assert false
  end;
...
let percent total a = string_of_int (int_of_float (a *. 100. /. total)) ^ "%"

let current_timer () = Timers.current_timer (get_timers ())

(* Profiling columns: - Name - Description - Length of the cell - Is the column
   linked to a statistic (Some true), a timer (Some false) or not (None)? - Data
   formatter Todo(Steven): use Pierre's solution with Printbox *)
let columns : (string * string * int * bool option * 'a) list =
  [ ("GTimer", "Global timer", 11, None, fun _ gtime sz -> float_resize gtime sz);
    ( "Steps",
      "Number of Steps",
      14,
      None,
    ...
    ( "Module Id",
      "Each call to a module is tagged with a fresh Id",
      10,
      None,
      fun _ _ sz ->
    ...
    ( "i/r",
      "AVG number of generated instances per instantiation rounds",
      8,
      Some true,
      fun _ _ sz ->
        let state = Lazy.force state in
        int_resize (state.instances / max 1 state.instantiation_rounds) sz );
    ( "dlvl",
    ...
    ( "T-asm",
      "Number of calls to Theory.assume",
      6,
      Some true,
      fun _ _ sz ->
        let state = Lazy.force state in
    ...
    ( "T/d",
      "Number of Theory.assume after last decision",
      6,
      Some true,
      fun _ _ sz ->
        let state = Lazy.force state in
    ...
    ( "T-qr",
      "Number of calls to Theory.query",
      15,
      Some true,
      fun _ _ sz ->
        let state = Lazy.force state in
    ...
    ( "B-R",
      "Number of reduced clauses by Boolean propagation",
      6,
      Some true,
      fun _ _ sz ->
        let state = Lazy.force state in
    ...
    ( "B-E",
      "Number of eliminated clauses by Boolean propagation",
      6,
      Some true,
      fun _ _ sz ->
        let state = Lazy.force state in
    ...
    ( "T-R",
      "Number of reduced clauses by Theory propagation",
      6,
      Some true,
      fun _ _ sz ->
        let state = Lazy.force state in
    ...
    ( "T-E",
      "Number of eliminated clauses by Theory propagation",
      6,
      Some true,
      fun _ _ sz ->
        let state = Lazy.force state in
    ...
    ( "B-!",
      "Number of direct Boolean conflicts",
      5,
      Some true,
      fun _ _ sz ->
        let state = Lazy.force state in
    ...
    ( "T-!",
      "Number of direct Theory conflicts",
      5,
      Some true,
      fun _ _ sz ->
        let state = Lazy.force state in
    ...
    ( "B>!",
      "Number of Boolean conflicts deduced by BCP",
      5,
      Some true,
      fun _ _ sz ->
        let state = Lazy.force state in
    ...
    ( "T>!",
      "Number of Theory conflicts deduced by BCP",
      5,
      Some true,
      fun _ _ sz ->
        let state = Lazy.force state in
    ...
    ( "M>!",
      "Number of Mix conflicts deduced by BCP",
      5,
      Some true,
      fun _ _ sz ->
        let state = Lazy.force state in
    ...
    (*-----------------------------------------------------------------*)
    ( "SAT",
      "Time spent in SAT module(s)",
      16,
      Some false,
      fun _ gtime sz ->
        let curr = Timers.get_sum (get_timers ()) Timers.M_Sat in
        Format.sprintf "%s~%s"
          (float_resize curr (sz - 5))
          (string_resize (percent gtime curr) 4) );
    ( "Matching",
      "Time spent in Matching module(s)",
      16,
      Some false,
      fun _ gtime sz ->
    ...
    ( "AC",
      ...
      fun _ gtime sz ->
        ...
        Format.sprintf "%s~%s"
          ...
          (string_resize (percent gtime curr) 4) );
    ( "Total",
      "Time spent in 'supervised' module(s)",
      11,
      Some false,
      fun _ _ sz ->
...
let print_initial_info fmt =
  ...
  then begin
    ...
    let max =
      List.fold_left
        (fun z (id, _, _, _, _) -> max z (String.length id))
        0 columns
    in
    List.iter
      (fun (id, descr, _, _, _) ->
...
let print_header header fmt =
  ...
  in
  if header || !nb_prints >= max_nb_prints
  then begin
    nb_prints := 0;
...
let print_instances_generation forced _steps fmt =
  ...
  if (not forced) && state.instances_map_printed
  then Format.fprintf fmt "[Instances profiling] No change since last print@."
  else
    ...
    List.iter
      (let open Format in
       fun (name, i, card, r) ->
         ...
         fprintf fmt "consumed: %s| " (int_resize (SE.cardinal i.consumed) 5);
         fprintf fmt "produced: %s| " (int_resize (SE.cardinal i.produced) 5);
         fprintf fmt "new: %s|| " (int_resize (SE.cardinal i._new) 5);
...
let float_print =
  ...
  fun fmt v ->
    if Float.equal v 0.
    then fprintf fmt "--     "
    else if Float.compare v 10. < 0
    then fprintf fmt "%0.5f" v
    else if Float.compare v 100. < 0
    then fprintf fmt "%0.4f" v
...
let line_of_sum_module =
  ...
  fun fmt timers ->
    ...
    List.iter
      (fun m -> fprintf fmt "| %a  " float_print (Timers.get_sum timers m))
      Timers.all_modules;
...
(** The table in which we will put the statistics we want to display when the
    (get-statistics) instruction is performed. *)
...
let register_float_stat (key : string) (getter : unit -> float) =
  register_stat key (fun () -> Format.sprintf "%.9f" (getter ()))
...
let () = register_int_stat "steps" (fun () -> Steps.get_steps ())
...
let init () =
  ...
  List.iter
    (fun (m : Timers.ty_module) ->
      ...
      register_float_stat name (fun () -> Timers.get_sum state.timers m))
    Timers.all_modules
