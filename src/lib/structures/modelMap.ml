...
module M : Map.S with type key = Expr.t list = Map.Make (struct
  type t = Expr.t list
...
(* The type of this module represents a model value for a function [f] by a
   finite set of constraints of the form: f(e_1, ..., e_n) = e where [e_i] and
   [e] are model terms according to [Expr.is_model_term].
...
module Constraints = struct
  ...
  (* A fiber of the function [f] over a value [v] is the set of all the values
     ...
     The function [inverse] of this module constructs a map of all the non-empty
     fibers of the function represented by a set of constraints. *)
  module Fiber = struct
    ...
    (* For an argument (x_1, ..., x_n) of the function, prints the SMT-LIB
       formula: (and (= arg_0 x_1)(= arg_1 x2) ... (= arg_n x_n)). *)
    let pp_args ppf = function
      | [] -> ()
      | [arg] -> pp_arg ppf (0, arg)
      | args ->
    ...
    (* For a fiber [x; y; z; ...] of the function, prints the SMT-LIB formula:
       (or (and (= arg_0 x_0) (= arg_1 x_1) ...) (and (= arg_0 y_0) (= arg_1
       y_1) ...) ...) *)
    let pp ppf fiber =
      ...
      | _ -> Fmt.pf ppf "(or %a)" (Fmt.iter ~sep:Fmt.sp iter pp_args) fiber
  end
  ...
  let inverse c =
    M.fold
      (fun arg_vals ret_val acc ->
        match Expr.Map.find_opt ret_val acc with
        | Some fib -> Expr.Map.add ret_val (Fiber.add arg_vals fib) acc
        | None -> Expr.Map.add ret_val (Fiber.of_list [arg_vals]) acc)
      c Expr.Map.empty
  ...
  let pp_inverse ppf c =
    let rec aux ppf seq =
      ...
      | Cons ((ret_val, fiber), seq) ->
        Fmt.pf ppf "(@[<hv>ite %a@ %a@ %a)@]" Fiber.pp fiber Expr.pp_smtlib
          ret_val aux seq
    in
...
type graph =
  | Free of Expr.t
  (* Represents a graph without any constraint. The expression is an abstract
     value. *)
  | C of Constraints.t
...
let add ((id, arg_tys, _) as sy) arg_vals ret_val { values; suspicious } =
  if List.compare_lengths arg_tys arg_vals <> 0
  then
    Fmt.invalid_arg
      "The arity of the symbol %a doesn't agree the number of arguments" Id.pp
      id;
  let constraints =
...
let find k { values; _ } = P.find k values
...
let rec subst_in_term id e c =
  ...
  | Sy.Name { hs = id'; _ }, [] when Id.equal id id' ->
    ...
    if not @@ Ty.equal ty ty'
    then Errors.error (Model_error (Subst_type_clash (id, ty', ty)));
    e
  | _ -> begin
...
let subst id e { values; suspicious } =
  if not @@ Expr.is_model_term e
  then Errors.error (Model_error (Subst_not_model_term e));
  let values =
    P.map
      (fun graph ->
        match graph with
...
let pp_define_fun ~is_constant pp ppf ((id, arg_tys, ret_ty), a) =
  ...
  Fmt.pf ppf "(@[define-fun %a (%a) %a@ %a)@]" Id.pp id
    Fmt.(list ~sep:sp (pp_named_arg_ty ~unused:is_constant))
    named_arg_tys Ty.pp_smtlib ret_ty pp a

let pp_define_fun ppf (sy, graph) =
  match graph with
  | Free a -> pp_define_fun ~is_constant:true Expr.pp_smtlib ppf (sy, a)
  | C constraints ->
...
let pp ppf { values; suspicious } =
  ...
  if P.is_empty values
  then Fmt.pf ppf "@[<v 2>(@]@,)"
  else
    Fmt.pf ppf "@[<v 2>(@,%a@]@,)"
      (Fmt.iter_bindings ~sep:Fmt.cut P.iter pp_define_fun)
      values
