...
let mk_bound kind sort ~is_open ~is_lower = { kind; sort; is_open; is_lower }
...
let is_ac x = match x with Name { kind = Ac; _ } -> true | _ -> false

let is_internal sy =
  match sy with Name { ns = User; _ } -> false | Name _ -> true | _ -> false

let compare_kinds k1 k2 =
  Util.compare_algebraic k1 k2 (function _, (Ac | Other) -> assert false)

let compare_operators op1 op2 =
  Util.compare_algebraic op1 op2 (function
    | Constr h1, Constr h2 | Destruct h1, Destruct h2 ->
      DE.Term.Const.compare h1 h2
    | Extract (i1, j1), Extract (i2, j2) ->
      let r = Int.compare i1 i2 in
      if r = 0 then Int.compare j1 j2 else r
    | Sign_extend n1, Sign_extend n2 | Repeat n1, Repeat n2 -> Int.compare n1 n2
...
let compare_builtin b1 b2 =
  Util.compare_algebraic b1 b2 (function
...
let compare_lits lit1 lit2 =
  Util.compare_algebraic lit1 lit2 (function
...
let compare_forms f1 f2 =
  Util.compare_algebraic f1 f2 (function
    | F_Unit b1, F_Unit b2 | F_Clause b1, F_Clause b2 -> Bool.compare b1 b2
...
let compare_bounds a b =
  ...
  then c
    ...
    then c
      ...
      if c <> 0 then c else compare_bounds_kind a.kind b.kind

let compare s1 s2 =
  Util.compare_algebraic s1 s2 (function
    ...
    | ( _,
        ( True | False | Name _ | Int _ | Real _ | Bitv _ | Float _ | Op _
        | Lit _ | Form _ | Var _ | In _ | MapsTo _ | Let ) ) ->
      assert false)
...
let string_of_bound b =
  ...
  if b.is_lower
  then Format.sprintf "%s %s" (if b.is_open then "]" else "[") kd
  else Format.sprintf "%s %s" kd (if b.is_open then "[" else "]")
...
module AEPrinter = struct
  ...
  let pp_lit ppf lit =
    ...
    | L_built (IsConstr h) -> Fmt.pf ppf "? %a" DE.Term.Const.print h
    | L_neg_built (IsConstr h) -> Fmt.pf ppf "?not? %a" DE.Term.Const.print h
  ...
  let pp ?(show_vars = true) ppf sy =
    ...
    (* FixedSizedBitVectors theory *)
    | Bitv (n, s) -> Fmt.pf ppf "[|%s|]" (Z.format (Fmt.str "%%0%db" n) s)
...
let to_string_clean sy = Fmt.str "%a" (AEPrinter.pp ~show_vars:false) sy

let to_string sy = Fmt.str "%a" (AEPrinter.pp ~show_vars:true) sy
