...
let string_smt_reprs = List.map (fun c -> to_smt_string c, []) constrs

let string_ae_reprs = List.map (fun c -> to_ae_string c) constrs

(* The rounding mode is the enum with the SMT values. The Alt-Ergo values are
   injected in this type. *)
...
let term_cst_of_rounding_mode =
  ...
  List.iter2 (fun key bnd -> Hashtbl.add table key bnd) constrs d_constrs;
  fun key ->
    try Hashtbl.find table key
    with Not_found ->
      Fmt.failwith "Error while searching for mode %a." pp_rounding_mode key

let rounding_mode_of_smt =
  let table = Hashtbl.create 5 in
  List.iter2
    (fun (key, _) bnd -> Hashtbl.add table key bnd)
    string_smt_reprs constrs;
  fun key ->
    try Hashtbl.find table key
    with Not_found ->
      Fmt.failwith "Error while searching for SMT2 FPA value %s." key
        fpa_rounding_mode_type_name

let rounding_mode_of_ae =
  let table = Hashtbl.create 5 in
  List.iter2 (fun key bnd -> Hashtbl.add table key bnd) string_ae_reprs constrs;
  fun (key : string) ->
    try Hashtbl.find table key
    with Not_found ->
      Fmt.failwith "Error while searching for Legacy FPA value %s." key
        fpa_rounding_mode_type_name
...
let integer_log_2 =
  let rec aux m e =
    if Q.compare m two >= 0
    then aux (div_x_by_2_pow_n m 1) (e + 1)
    else if Q.compare m Q.one >= 0
    then e
    else
  ...
  in
  fun m ->
    if Q.compare_to_0 m <= 0
    then begin
      Printer.print_err "integer_log_2 not defined for input (%a)" Q.print m;
      assert false
    end;
    let res = aux m 0 in
    (* Printer.print_dbg "found that integer_log_2 of %a is %d" Q.print m res;*)
  ...
  let compare (prec1, exp1, mode1, x1) (prec2, exp2, mode2, x2) =
    ...
    then c
      ...
      then c
        ...
        let c = exp1 - exp2 in
        if c <> 0 then c else compare_rounding_mode mode1 mode2
end)
...
let round_to_integer mode q = Q.from_z (round_big_int mode q)

let empty_cache () = cache := MQ.empty
