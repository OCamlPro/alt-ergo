...
module type S = sig
  ...
  (** [merge f m1 m2] computes a map whose keys is a subset of keys of [m1] and
      ...
      we have [find_opt x (merge f m1 m2) = f (find_opt x m1) (find_opt x m2)]
  ...
  (** [union f m1 m2] computes a map whose keys is the union of keys of [m1] and
      of [m2]. When the same binding is defined in both arguments, the function
      [f] is used to combine them. This is a special case of [merge]:
  ...
  (** [iter f m] applies [f] to all bindings in map [m]. [f] receives the key as
      ...
      are passed to [f] in increasing order with respect to the ordering over
  ...
  (** [filter p m] returns the map with all the bindings in [m] that satisfy
      predicate [p]. If [p] satisfies every binding in [m], [m] is returned
  ...
  val height : 'a t -> int
  (** Return the height of the tree encodin the map. NOTE THAT: two maps that
      are equal may have a different height.
      @since now *)
  ...
  (** Same as {!Map.S.min_binding}, but returns the largest binding of the given
      map.
      @since 3.12.0 *)
  ...
  (** [find_opt x m] returns [Some v] if the current binding of [x] in [m] is
      [v], or [None] if no such binding exists.
      @since 4.05 *)
  ...
  (** [find_first f m], where [f] is a monotonically increasing function,
      ...
      the first binding [k, v] of [m] where [Ord.compare k x >= 0] (intuitively:
      [k >= x]), or raise [Not_found] if [x] is greater than any element of [m].
