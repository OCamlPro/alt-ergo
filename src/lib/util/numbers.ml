...
module Z = struct
  ...
  let my_gcd a b = if is_zero a then b else if is_zero b then a else Z.gcd a b
...
module Q = struct
  ...
  let root_num q n =
    ...
    then Some zero
      ...
      let v = to_float q in
      let w =
        if Float.compare v min_float < 0
        then min_float
        else if Float.compare v max_float > 0
        then max_float
        else v
      in
  ...
  let sqrt_excess q =
    ...
    | Some s ->
      if not (is_zero s)
      then Some (div (add s (div q s)) two)
