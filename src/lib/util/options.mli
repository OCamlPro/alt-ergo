...
(** Some values are defined once and for all in the options module since it will
    ...
    will stay like this until the next PR that's supposed to clean some parts of
    the program *)
...
type instantiation_heuristic =
  | INormal
      (** Least costly heuristic for instantiation, instantiate on a reduced set
          of term *)
  | IAuto
      (** Default Heuristic that try to do the normal heuristic and then try a
          greedier instantiation if no new instance have been made *)
  | IGreedy
      (** Force instantiation to be the greedier as possible, use all available
          ground terms *)

type smtlib2_version =
  [ `Latest  (** Latest version of the SMT-LIB standard. *)
  | `V2_6
  ...
  | `Poly
    (** Polymorphic extension of the SMT-LIB standard.
        ...
        See the {{:https://inria.hal.science/hal-01960203/document} tool paper}
    *) ]
...
(** Set [debug_interpretation] accessible with {!val:get_debug_interpretation}
*)
...
val set_enabled_theories : Theories.t list -> unit
...
(** Set [disable_flat_formulas_simplification] accessible with
    {!val:get_disable_flat_formulas_simplification} *)
...
(** Replay unsat-cores produced by {!val:get_unsat_core}. The option implies
    {!val:get_unsat_core} returns [true]. *)
...
(** [true] if replay with all axioms and predicates saved in [.used] files of
    the current directory is enabled. *)
val get_replay_all_used_context : unit -> bool
(** Default to [false] *)

(** [true] if replay with axioms and predicates saved in [.used] file is
    enabled. *)
val get_replay_used_context : unit -> bool
(** Default to [false] *)

(** [true] if used axioms and predicates will be saved in a [.used] file. This
    option implies {!val:get_unsat_core} returns [true]. *)
...
(** Value specifying the default input format. Useful when the extension does
    not allow to automatically select a parser (eg. JS mode, GUI mode, ...).
    possible values are
    - native
    - smtlib2
    - why3
...
val get_enabled_theories : unit -> Theories.t list
...
(** Value specifying the limit above which Fourier-Motzkin variables elimination
    ...
    limit are skipped. However, unit eliminations are always done. *)
...
(** Value specifying the time limit for model generation (not supported on
    Windows). *)
val get_timelimit_interpretation : unit -> float
(** Default to [1.] (not supported on Windows) *)

(** Value specifying the given timelimit for each goal in case of multiple goals
    per file. In this case, time spent in preprocessing is separated from
    resolution time.
...
(** [true] if the objectives_in_interpretation is set to inline pretty-printing
    of optimized expressions in the model instead of a dedicated section
    '(objectives ...)'. Be aware that the model may be shrunk or not accurate if
    some expressions to optimize are unbounded. *)
...
(** [true] if generated substitutions are normalised by matching w.r.t. the
    state of the theory.

    This means that only terms that are greater (w.r.t. depth) than the initial
    terms of the problem are normalized. *)
...
(** Value specifying which SAT solver is being used.
    ...
    Possible values are:
    {ul
     {- CDCL-tableaux : CDCL SAT-solver with formulas filtering based on
        tableaux method
        - satML-Tableaux
        - satML-tableaux
     }
...
(** [true] if the use of a tableaux-like method for instantiations is enabled
    with the CDCL solver if satML is used. *)
val get_cdcl_tableaux_inst : unit -> bool
(** Default to [true] *)

(** [true] if the use of a tableaux-like method for theories is enabled with the
    CDCL solver if satML is used. *)
...
(** [true] if substitution of variables bounds by Let is enabled. The default
    behavior is to only substitute variables that are bound to a constant, or
...
(** [true] if the AC (Associative and Commutative) theory is disabled for
    function symbols. *)
...
(** [true] if support for the SMT-LIB floating-point arithmetic (FPA) theory is
    enabled. *)
val get_smt_lib_fpa : unit -> bool
(** Default to [false] *)

(** [true] if non-linear arithmetic reasoning (i.e. non-linear multplication,
    division and modulo on integers and rationals) is disabled. Non-linear
...
(** [true] if backward reasoning step (starting from the goal) done in the
    default SAT solver before deciding is disabled. *)
...
(** [true] if the set of decision procedures (equality, arithmetic and AC) is
    restricted. *)
...
module Time : sig
  ...
  (** [with_timeout tm f] calls [f ()] with a timeout of [tm], and unsets the
      timeout once the call to [f ()] completes or raises an exception.

      @raise Util.Timeout if the timeout is reached before [f ()] completes. *)
end
...
(** [with_timelimit_if cond f] is:
    ...
    @raise Util.Timeout
      if the [cond] is [true] and the timeout is reached before the calls to
      [f ()] completes. *)
...
module Output : sig
  ...
  (** Set the regular output channel used by default to output results, models
      and unsat cores.

      Default to [Format.std_formatter]. *)

  val set_diagnostic : t -> unit
  (** Set the diagnostic output channel used by default to output errors, debug
      and warning informations.
  ...
  (** Value specifying the formatter used to output models printed by the
      `--dump-models` option.
...
(** [heavy_assert p] checks if the suspended boolean [p] evaluates to [true].
    ...
    @raise Assert_failure if [p] evaluates to [false]. *)
