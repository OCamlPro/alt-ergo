...
(* major_collections; (* num of completed major collection cycles *)
   minor_collections; (* num of minor collections *) stack_size; (* current size
   of the stack, in word *)

   heap_words; (* tot size of the major heap *) top_heap_words; (* Max size
   reached by major heap *)
    ...
    ignore
      (Gc.create_alarm (fun () ->
           ...
           Printer.print_dbg ~module_name:"Gc_debug"
             ...
             e.major_collections e.minor_collections e.stack_size e.heap_words
             e.top_heap_words
