...
let incr k =
  ...
  | Th_assumed n ->
    ...
    if n < 0 then Errors.run_error (Invalid_steps_count n);
    naive_steps := !naive_steps + n
  end;
  if !steps_bound <> -1 && (!steps > !steps_bound || !naive_steps > !steps_bound)
  then begin
    let n = max !naive_steps !steps in
    (* CR bclement: This is preserved legacy behavior, but figure out in which
       situation we can have [n <= 0]. *)
...
let save_steps, reinit_steps =
  ...
  let save_steps () = saved_mult_uf := !mult_uf in
  let reinit_steps () =
    Stack.clear all_steps;
    naive_steps := 0;
    steps := 0;
 ...
 * used *)
let get_steps () = max !naive_steps !steps
