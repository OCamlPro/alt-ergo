...
module Z : sig
  ...
  val sqrt_rem : t -> t * t
  (** returns sqrt truncated with the remainder. It assumes that the argument is
      positive, otherwise, [Invalid_argument] is raised. *)

  val testbit : t -> int -> bool
  (** [testbit z n] returns true iff the nth bit of z is set to 1. n is supposed
      to be positive *)
...
module Q : sig
  ...
  (* computing root and sqrt by default and "by excess". The given rational is
     supposed to be positive. The integer provided for root_xxx is also supposed
     to be positive. Computations use floats. None is returned in case of
     failure. sqrt_xxx versions are more accurate and faster than their
     equivalent root_xxx when the integer is 2*)
