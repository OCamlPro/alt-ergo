...
(* A nest is the set of all the constructors of a mutually recursive definition
   ...
   where: - the nodes are all the constructors of the nest; - for all type ty of
   the nest, there is a hyperedge from S(ty) to G(ty).

   In particular, our graph has exactly one outgoing hyperedge per node. *)

(* Node of the hypergraph. *)
type node =
  { id : DE.term_cst; (* Dolmen constructor identifier. *)
    weight : int;
    (* This weight is used to prioritize constructors with less destructors
       during the sorting (see [add_nest]). *)
    outgoing : edge;
    (* Hyperedge from a subset S in S(ty) to a subset G in G(ty) where ty is the
       type of [id]. At the beginning, we have S = S(ty) and G = G(ty).

       One use a double indirection because this hyperedge is shared between all
       the elements of S. *)
    mutable in_degree : int (* Number of hyperedges into this constructor. *)
  }

(* Type of a hyperedge. *)
and edge = node list ref

module Hp = Heap.MakeOrdered (struct
  type t = node
...
let def_of_dstr dstr =
  let* ty_cst =
    match dstr with
    | DE.{ builtin = B.Adt (B.Adt.Destructor _); id_ty; _ } -> begin
...
(* Build the hypergraph of dependencies between the constructors of the nest
   [defs].
...
let attach_orders defs =
  ...
  while not @@ Hp.is_empty hp do
    (* Loop invariant: the set of nodes in heap [hp] is exactly the set of the
       nodes of the graph without ingoing hyperedge. *)
    ...
    List.iter
      (fun node ->
        ...
        if node.in_degree = 0 then Hp.insert hp node)
      !outgoing;
    outgoing := []
  done
...
let perfect_hash id =
  ...
  | { builtin = B.Adt (B.Adt.Constructor _); _ } as id -> begin
    match DE.Term.Const.get_tag id order_tag with
    | Some h -> h
    | None ->
