...
module Output = struct
  ...
  let all_channels = [regular_output; diagnostic_output; dump_models_output]

  let close o =
    match o with
    | Stdout | Stderr | Fmt _ -> Format.pp_print_flush (to_formatter o) ()
    | Channel (_, cout, _) ->
...
(* We don't want to handle functions with more than 10 arguments so we need to
   split the debug options to gather them in the end. Problems with this way of
   doing is the options in each "group" are sorted alphabetically before we
   split the corresponding group. Adding a new one may break the sorting which
   is why each group contains 7/8/9 options as of now to allow the adding of new
   ones in the right group *)
...
let get_output_smtlib () =
  match !output_format with Smtlib2 _ -> true | _ -> false
...
let (timeout : (unit -> unit) ref) = ref (fun () -> raise Util.Timeout)
...
let tool_req n msg =
  if get_rule () = n
  then Format.fprintf (Output.get_fmt_diagnostic ()) "[rule] %s@." msg
...
module Time = struct
  ...
  let unset_timeout () =
    if Float.compare (get_timelimit ()) 0. <> 0
    then
      let _ : Unix.interval_timer_status =
        Unix.setitimer Unix.ITIMER_VIRTUAL
  ...
  let with_timeout tm f =
    Fun.protect ~finally:unset_timeout (fun () ->
        set_timeout tm;
        f ())
end

let with_timelimit_if cond f =
  if cond then Time.with_timeout (get_timelimit ()) f else f ()
