...
module Make (X : S) = struct
  ...
  module Debug = struct
    let pprint fmt p =
      ...
      M.iter
        (fun x n ->
          let s, n, op =
            if Q.equal n Q.one
            then (if !zero then "" else "+"), "", ""
            else if Q.equal n Q.m_one
            then "-", "", ""
            else if Q.sign n > 0
            then (if !zero then "" else "+"), Q.to_string n, "*"
      ...
      let s, n =
        if Q.sign p.c > 0
        then (if !zero then "" else "+"), Q.to_string p.c
        else if Q.sign p.c < 0
        then "-", Q.to_string (Q.minus p.c)
        else if !zero
        then "", "0"
        else "", ""
      in
    ...
    let print fmt p =
      ...
      then pprint fmt p
        ...
        M.iter
          (fun t n -> Format.fprintf fmt "%s*%a " (Q.to_string n) X.print t)
          p.m;
  ...
  let mult_const n p =
    if Q.sign n = 0
    then { m = M.empty; c = Q.zero; ty = p.ty }
  ...
  let mult_monome a x p =
    ...
    let m =
      M.fold (fun xi ai m -> M.add (X.mult x xi) (Q.mult a ai) m) p.m acx.m
    in
    { acx with m }
  ...
  let normal_form ({ m; _ } as p) =
    if M.is_empty m
    then { p with c = Q.zero }, p.c, Q.one
    else
