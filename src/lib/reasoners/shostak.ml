  ...
  module Debug = struct
    let print ppf r =
      if Options.get_term_like_pp () then begin
        ...
        | Bitv t ->
          Fmt.pf ppf "%s:@,[%a]" BITV.name BITV.print t
        | Enum t ->
          Fmt.pf ppf "%s:@,[%a]" ENUM.name ENUM.print t
        | Adt t ->
    ...
    let assert_have_mem_types tya tyb =
      if Options.get_enable_assertions () && not (Ty.compare tya tyb = 0) then
        Fmt.failwith "@[<v 0>@ tya = %a and @ tyb = %a@]"
  ...
  *)
  ...
  let make t =
    ...
    | false , false , false , true , false, false ->
      Timers.with_timer Self.M_Sum Self.F_make @@ fun () ->
      ENUM.make t
    ...
    | (a,b) :: eqs ->
      ...
      if CX.equal ra rb then solve_list pb
        ...
        let pb = match tya with
          ...
          | Ty.Tsum _          ->
            Timers.with_timer ENUM.timer Self.F_solve @@ fun () ->
            ENUM.solve ra rb pb
