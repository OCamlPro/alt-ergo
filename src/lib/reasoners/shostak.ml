...
module rec CX : sig
  ...
  module Debug = struct
    ...
    let print_sbt msg sbs =
      ...
      let print fmt (p, v) =
        incr c;
        Format.fprintf fmt "<%d) %a |-> %a@ " !c print p print v
      in
      if Options.get_debug_combine ()
      then
        print_dbg ~module_name:"Shostak" ~function_name:"print_sbt"
          "@[<v 2>%s subst:@ %a@]" msg (pp_list_no_space print) sbs

    let debug_abstraction_result oa ob a b acc =
      let c = ref 0 in
      let print fmt (p, v) =
        incr c;
        Format.fprintf fmt "(%d) %a |-> %a@ " !c CX.print p CX.print v
      in
      if Options.get_debug_combine ()
      then
        print_dbg ~module_name:"Shostak" ~function_name:"abstraction_result"
          "@[<v 0>== get_debug_abstraction_result ==@ Initial equaliy:   %a = \
           %a@ abstracted equality: %a = %a@ @[<v 2>selectors elimination \
           result:@ %a@]@]"
          CX.print oa CX.print ob CX.print a CX.print b (pp_list_no_space print)
          acc

    let solve_one a b =
      if Options.get_debug_combine ()
      then
        print_dbg ~module_name:"Shostak" ~function_name:"solve_one"
          "solve one %a = %a" CX.print a CX.print b

    let debug_abstract_selectors a =
      if Options.get_debug_combine ()
      then
        print_dbg ~module_name:"Shostak" ~function_name:"abstract_selectors"
          "abstract selectors of %a" CX.print a

    let assert_have_mem_types tya tyb =
      Options.heavy_assert (fun () ->
          if not (Ty.compare tya tyb = 0)
          then begin
            print_err "@[<v 0>@ Tya = %a  and @ Tyb = %a@]" Ty.print tya
              Ty.print tyb;
            false
          end
          else true)
  ...
  let reinit_cache () = HC.reinit_cache ()
  ...
  (* Constraint that must be maintained: all theories should have Xi < Term <
     Ac *)
  let theory_num x =
    match x with
    | Ac _ -> -1
  ...
  (*** implementations before hash-consing semantic values
    ...
    let hash r = match r.v with | Term t -> Expr.hash t | Ac x -> AC.hash x |
    Arith x -> ARITH.hash x | Bitv x -> BITV.hash x | Arrays x -> ARRAYS.hash x
    | Adt x -> ADT.hash x
  ...
  (* should be called hash_cmp and used where structural_compare is not needed

     let compare a b = let c = Stdlib.compare a.id b.id in let c' =
     Stdlib.compare b.id a.id in assert ((c = 0 && c' = 0) || (c*c' < 0)); c *)
    ...
    | _ -> (
      match
        ( ARITH.is_mine_symb ac.Sig.h,
          ...
          AC.is_mine_symb ac.Sig.h )
      with
      (*AC.is_mine may say F if Options.get_no_ac is set to F dynamically *)
      | true, false, false, false -> ARITH.color ac
      | false, true, false, false -> BITV.color ac
      | false, false, true, false -> ADT.color ac
      | _ -> ac_embed ac)
  ...
  let make_idemp a b sbs =
    ...
    let sbs = triangular_down (List.rev sbs) in
    (* triangular up *)
    ...
    (* This assert is not TRUE because of AC and distributivity of '*'
       Options.heavy_assert (fun () -> equal (apply_subst a sbs) (apply_subst b
       sbs)); *)
    sbs
  ...
  let solve_uninterpreted r1 r2 (pb : r Sig.solve_pb) =
    (* r1 != r2*)
    if Options.get_debug_combine ()
    then Printer.print_dbg "solve uninterpreted %a = %a" print r1 print r2;
    if CX.str_cmp r1 r2 > 0
    then { pb with sbt = (r1, r2) :: pb.sbt }
  ...
  let rec solve_list (pb : r Sig.solve_pb) =
    ...
    | (a, b) :: eqs ->
      ...
      then solve_list pb
        ...
        (solve_list pb
         [@ocaml.ppwarning
           "TODO: a simple way of handling equalities with void and unit is to \
            add this case is the solver!"])
  ...
  let assign_value r distincts eq =
    let opt =
      ...
      | _, Ty.Tint | _, Ty.Treal -> ARITH.assign_value r distincts eq
      ...
      | Term _t, Ty.Tbool ->
        ...
        then None
          ...
          | [] -> (
            ...
            | _ :: _ -> assert false
            | [] -> Some (Expr.faux, true))
          (* true <-> make a case split *)
        end
      | Term t, ty ->
        (* case disable_adts() handled here *)
        if
          Expr.is_model_term t
          || List.exists (fun (t, _) -> Expr.is_model_term t) eq
        then None
        else Some (Expr.mk_abstract ty, false) (* false <-> not a case-split *)
      | _ ->
        (* There is no model-generation support for the AC symbols yet. The
           function [AC.assign_value] always returns [None]. *)
        AC.assign_value r distincts eq
    in
    if Options.get_debug_interpretation ()
    then
      Printer.print_dbg ~module_name:"Shostak" ~function_name:"assign_value"
        "assign value to representative %a : %s" print r
        (match opt with
...
and TARITH : (Polynome.T with type r = CX.r) = Arith.Type (CX)

and ARITH : (Sig.SHOSTAK with type t = TARITH.t and type r = CX.r) =
  Arith.Shostak
    (CX)
    (struct
      include TARITH
...
and BITV : (Sig.SHOSTAK with type r = CX.r and type t = CX.r Bitv.abstract) =
Bitv.Shostak (struct
  include CX
...
and ADT : (Sig.SHOSTAK with type r = CX.r and type t = CX.r Adt.abstract) =
Adt.Shostak (struct
  include CX
