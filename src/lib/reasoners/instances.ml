...
module type S = sig
  ...
  val add_predicate :
    t -> guard:Expr.t -> name:string -> Expr.gformula -> Ex.t -> t
...
module Make (X : Theory.S) : S with type tbox = X.t = struct
  module EM = Matching.Make (struct
    ...
    (* This function from Ccx.Main is wrapped to ensure Fresh explanations will
       not leak through Ccx during matching, in which case assertions in various
  ...
  type t =
    { guards : (E.t * bool) list ME.t;
      (* from guards to list of guarded predicates. bool = true <-> pred is
         ground *)
  ...
  module Debug = struct
    ...
    let new_facts_of_axiom ax insts_ok =
      if get_debug_matching () >= 1 && insts_ok != ME.empty
      then
        let name =
          match Expr.form_view ax with
  ...
  let add_terms env s gf =
    ...
    in
    { env with matching = SE.fold (EM.add_term infos) s env.matching }
  ...
  let add_ground_pred env ~guard p np defn ex =
    ...
    { env with ground_preds = gp; guards = ME.add guard guarded env.guards }

  let add_predicate env ~guard ~name gf ex =
    let { Expr.ff = f; age; _ } = gf in
    let env =
      { env with matching = EM.max_term_depth env.matching (E.depth f) }
    in
    ...
    | E.Iff (f1, f2) ->
      ...
      let np = E.neg p in
      let defn =
        if E.equal f1 p then f2 else if E.equal f2 p then f1 else assert false
      in
    ...
    | E.Unit _ | E.Clause _ | E.Xor _ | E.Skolem _ | E.Let _ -> assert false
      ...
      let ground, non_ground =
        List.fold_left
          (fun (ground, non_ground) (f, is_ground) ->
            if is_ground
            then ME.remove f ground, non_ground
            else ground, ME.remove f non_ground)
          (env.ground_preds, env.predicates)
          guarded
      in
      { env with
        guards = ME.remove guard env.guards;
  ...
  let ground_pred_defn (p : E.t) env =
    try Some (ME.find p env.ground_preds) with Not_found -> None
      ...
      (fun acc
        ...
        List.fold_left
          (fun acc
            ...
            if inst_is_seen_during_this_round orig nf acc
            then acc
            else
              let accepted = selector nf orig in
              if not accepted
              then add_rejected_to_acc orig nf acc
              else
                let p =
                  { Expr.ff = nf;
                ...
                let dep =
                  ...
                  then dep
                    ...
                    (* Dep lorig used to track conflicted instances in profiling
                       mode *)
    ...
    fun lf ->
      List.fast_sort
        (fun (p1, _) (p2, _) ->
          ...
          if c <> 0 then c else E.compare p2.Expr.ff p1.Expr.ff)
        lf
  ...
  let mround env axs tbox selector ilvl kind mconf tconf =
    ...
    let triggers =
      ...
      | Util.Normal ->
        Matching.Triggers.add_triggers_of_formulas tconf Matching.Triggers.empty
          axs
      | Util.Backward ->
    ...
    in
    let ccx_tbox =
      if mconf.Util.use_cs || tconf.Util.greedy
      then X.get_case_split_env tbox
