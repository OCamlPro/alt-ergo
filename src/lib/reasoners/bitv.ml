...
type sort_var =
  ...
  | B
  | C
(* The variables used by the bitvector solver can be split into three categories
   that have associated invariants.

   These invariants are recalled in the functions that make use of them, but the
   outline is as follows. The solver works with *partitioned multi-equations*. A
   partitioned multi-equation is an equality: X == e1 = e2 = ... = en where X is
   a term (or an alien) and e1, e2, ..., en are terms in the bitvector algebra,
   ...
   - Each A variable appears at most once (this is used to represent portions of
   a term that are irrelevant to a particular constraint)
   ...
   - Each C variable appears at most once in each multi-equation, but appears in
   several (more precisely, two) distinct multi-equations. *)
...
and defn =
  | Droot of
      { id : int;
            (** Variable identifier. This is [-n] for the negation of the
                variable with identifier [n] (and in particular never [0]). *)
        neg : tvar;
            (** Negated variable. While this root is live, the [neg] field is
                guaranteed to be another [Droot] with negated identifier. Once
                the [Droot] is merged with an equivalence class, the [neg] field
                is merged with the negated class. *)
        sorte : sort_var
            (** The sort of this variable. See the type definition for
                {!sort_var} for details. *)
      }
  | Dcte of Z.t * int
      (** A [Dcte] is a variable that is forced to be equal to the specified
          integer representation in bits. The second argument is the bit width.
      *)
  | Dlink of tvar
      (** A [Dlink] is a defined variable. All the [Dlink] should be followed to
          ...
          should not occur before that and hence the [Dlink] case can be ignored
...
(** [s_var sorte] creates a new unconstrained variable with sorte [sorte]. The
    negated version of the variable is also created eagerly. *)
let s_var =
  let cnt = ref 0 in
  fun sorte ->
    let id =
      ...
      !cnt
    in
    let rec x = { defn = Droot { id; sorte; neg } }
...
let pp_alpha_term pp ppf { bv; sz } = Fmt.pf ppf "%a[%d]" pp bv sz
...
let hash_simple_term_aux hash = function
  ...
  | Ext (x, a, b, c) -> hash_signed hash x + (19 * (a + b + c))
...
let rec to_Z_opt_aux acc = function
  | [] -> Some acc
  | { bv = Cte n; sz } :: sts -> to_Z_opt_aux Z.((acc lsl sz) + n) sts
  | _ -> None

let to_Z_opt r = to_Z_opt_aux Z.zero r

let int2bv_const n z = [{ bv = Cte (Z.extract z 0 n); sz = n }]
...
let extract_st i j { bv; sz } =
  ...
  then { bv; sz }
    ...
    | Ext (r, sz, k, _) -> { bv = Ext (r, sz, i + k, j + k); sz = j - i + 1 }
...
let rec extract size i j = function
  ...
  | [st] ->
    assert (st.sz = size);
    [extract_st i j st]
  | { sz; _ } :: sts when j < size - sz -> extract (size - sz) i j sts
...
let repeat n b =
  assert (n > 0);
  let rec loop b n acc = if n = 0 then acc else loop b (n - 1) (b @ acc) in
  loop b (n - 1) b
...
let sign_extend n sts =
  ...
  | n, ({ sz; bv } as st) :: sts -> (
    match bv with
    | Cte b -> int2bv_const (n + sz) (Z.signed_extract b 0 sz) @ sts
    | _ ->
      List.rev_append (repeat n [extract_st (sz - 1) (sz - 1) st]) (st :: sts))

let zero_extend sz sts =
  if sz < 0 then Fmt.invalid_arg "zero_extend: got negative extension: %d" sz;
  match sts with
  | { bv = Cte _ as bv; sz = sz' } :: sts' -> { bv; sz = sz + sz' } :: sts'
...
module Shostak (X : ALIEN) = struct
  ...
  let is_mine_symb sy =
    ...
    | Sy.Bitv _
    | Op
        ( Concat | Extract _ | Sign_extend _ | Repeat _ | BV2Nat | BVnot | BVand
        | BVor | BVxor | BVadd | BVsub | BVmul | BVudiv | BVurem | BVshl
        | BVlshr ) ->
      true
    | _ -> false
  ...
  module Canon = struct
    ...
    let view t =
      ...
      | { f = Op BVnot; xs = [t]; ty = Tbitv size; _ } ->
        { descr = Vnot t; size }
      | { ty = Tbitv size; _ } -> { descr = Vother t; size }
      | _ -> assert false
    ...
    let other ~neg t _sz ctx =
      let r, ctx' =
        ...
        | { f =
              Op
                ( BVand | BVor | BVxor | BVadd | BVsub | BVmul | BVudiv | BVurem
                | BVshl | BVlshr );
            _
          } ->
          X.term_embed t, []
    ...
    let rec normalize = function
      ...
      | s :: (t :: ts as tts) -> begin
        ...
        | Ext (d1, ds, i, j), Ext (d2, _, k, l)
          ...
          if k = 0 then normalize_st d :: normalize ts else normalize (d :: ts)
    ...
    and vextract ~neg i j tv =
      let size = j - i + 1 in
      match tv.descr with
      | Vcte z -> vmake ~neg { descr = Vcte (Z.extract z i size); size }
      | Vother t ->
      ...
      | Vconcat (u, v) ->
        ...
        if j < vv.size
        then vextract ~neg i j vv
        else if i >= vv.size
        then vextract ~neg (i - vv.size) (j - vv.size) vu
        else
          let+ u = vextract ~neg 0 (j - vv.size) vu
          and+ v = vextract ~neg i (vv.size - 1) vv in
          u @ v
      | Vsign_extend (_, u) ->
        let vu = view u in
        if j < vu.size
      ...
      | Vrepeat (_, u) ->
        let vu = view u in
        (* i = ib * vu.size + il j = jb * vu.size + jl *)
        ...
        then (
          ...
          let suffix = if il = 0 then [] else extract vu.size il (vu.size - 1) u
          and prefix = if jl = 0 then [] else extract vu.size 0 jl u in
          prefix @ repeat (jb - ib - 1) u @ suffix
      | Vnot t -> vextract ~neg:(not neg) i j (view t)
    ...
    and vmake ~neg tv ctx =
      ...
      | Vextract (t', i, j) -> run ctx @@ vextract ~neg i j (view t')
      | Vsign_extend (n, t) ->
        run ctx
        @@
        let+ r = make ~neg t in
      ...
      | Vconcat (t1, t2) ->
        ...
        t1 @ t2
      | Vnot t -> run ctx @@ make ~neg:(not neg) t
  ...
  module Debug = struct
    ...
    let print_X ppf ast = Format.fprintf ppf "%a[%d]" X.print ast.bv ast.sz
    ...
    let print_sliced_sys l =
      let print fmt (a, b) = Format.fprintf fmt " %a == %a@ " print a print b in
      if Options.get_debug_bitv ()
      then
        Printer.print_dbg ~module_name:"Bitv" ~function_name:"slicing" "%a"
    ...
    let print_c_solve_res l =
      ...
      if Options.get_debug_bitv ()
      then
        Printer.print_dbg ~module_name:"Bitv" ~function_name:"c_solve"
    ...
    let print_partition_res l =
      ...
      if Options.get_debug_bitv ()
      then
        Printer.print_dbg ~module_name:"Bitv" ~function_name:"partition" "%a"
          (pp_list_no_space print) l
    ...
    let print_final_solution l =
      ...
      if Options.get_debug_bitv ()
      then
        Printer.print_dbg ~module_name:"Bitv" ~function_name:"solution" "%a"
          (pp_list_no_space print) l
  end
  (*BISECT-IGNORE-END*)
  ...
  module Solver = struct
    ...
    (* [st_slice st siz] splits the simple term [st] in two parts, the first of
       ...
       - [x] is [[b0; ..; b(siz-1)]] - [y] is [[b(siz); ..; bn]] *)
    let st_slice st siz =
      let siz_bis = st.sz - siz in
      match st.bv with
      | Cte b ->
    ...
    (* [slice t u] transforms the equality [t = u] between abstract terms (i.e.
       ...
       Requires: [t] and [u] must have the same total size. Ensures: there are
       no duplicates in the result (in particular, [x = y] and [y = x] cannot be
       both present) Ensures: there are no trivial equalities [x = x] in the
       result. *)
    ...
    let slice t u =
      ...
      let rec f_rec acc = function
        ...
        | s1 :: r1, s2 :: r2 ->
          if s1.sz = s2.sz
          then f_rec (f_add (s1, s2) acc) (r1, r2)
          else begin
            if s1.sz > s2.sz
            then
    ...
    let cte_vs_ext bol xt s_xt i j =
      ...
      let var = { bv = xt.value; sz = s_xt } in
      var, a2 @ cte @ a1

    (* Orient the equality [r1 = r2] where both [r1] and [r2] are uninterpreted
       ("Other") of size [sz], possibly negated.
    ...
    let other_vs_other r1 r2 sz =
      ...
      [{ bv = r1.value; sz }, c; { bv = r2.value; sz }, maybe_negate c]
    ...
    (* Orient the equality [r = xt[s_xt]^{i,j}] where [r] and [xt] are
       ...
       Requires: [size st = j - i + 1] Requires: [st] and [xt] are distinct (for
       the C variables invariant --- but this is actually impossible because the
       sizes wouldn't match) *)
    ...
    (* Orient the equality [id[s]^{i,j} = id'[s']^{i',j'}].
       ...
       that we maintain the invariant of C variables that they never occur twice
       in the same multi-equation. *)
    let ext1_vs_ext2 (id, s, i, j) (id', s', i', j') =
      (* id != id' *)
    ...
    (* Orient the equality [xt[siz]^{i1, i1 + tai} = xt[siz]^{i2, i2 + tai}]
       ...
       The [overl] variable contains the number of overlapping bits. If the [sp]
       variable is false, either size is negated and we negate every other
       occurence of the [b] variable below.

       - If there are no overlapping bits, the two parts are independent. We
       create a fresh B-variable [b] and A-variables [a1], [a2] and [a3] and
       orient:
       ...
       [nn_overl] is the number of *initial* non-overlapping bits (from [i1] to
       [i2]).

       The [b] vector is then split into two parts to properly align the
       repetition. The computation is:

       sz_b1 = b_box % nn_overl (size of b1) sz_b2 = nn_overl - sz_b1 (size of
       b2) a1[i] @ ((b1 @ b2) * (b_box/nn_overl)) @ b1 @ a2[siz - tai - i2]

       It will be more clear with an example:

       _ nn_overl = 3 / \ xxxxxxx??? ???yyyyyyy \________/ b_box = 10

       Which creates the following decomposition:

       $$$xxxxxxx???### $$$???yyyyyyy### \\\uvvuvvuvvu/// where a1 = \\\, b1 =
       u, b2 = vv, a2 = ///
    ...
    let ext_vs_ext sp xt siz (i1, i2) tai =
      ...
      then begin
        ...
        let nn_overl = tai - overl in
        (* =i2-i1 >0 sinon egalite sytaxique*)
    ...
    (* [sys_solve sys] orients a system of equations between simple terms.
       ...
       unconstrained in the original equation. Each A variable appears *exactly
       once* in the oriented system. - The B variables appear due to equalities
       involving multiple extractions of the same uninterpreted term. They
       appear in a single equation, but can appear *multiple times in a single
       equation*. - The C variables appear due to equalities involving multiple
       uninterpreted terms. They appear at most once in each equation, but can
       appear *in multiple equations*. They only appear in equations that have
       distinct left-hand-side.
    ...
    let sys_solve sys =
      let c_solve (st1, st2) =
        ...
        | Other o1, Other o2 ->
          ...
          else other_vs_other o1 o2 st1.sz
    ...
    (* [slice_var var pat_hd pat_tl] slices a variable to make it fit the
       pattern [pat_hd :: pat_tl]. Returns the list of created fresh variables,
       the left over pattern and the sort of the variable (if it is not a
       constant). *)
    let slice_var var pat_hd pat_tl =
      let mk, tr =
        match var.bv.defn with
        | Dcte (n, _) ->
          ( (fun ofs sz -> { bv = s_cte (Z.extract n (ofs - sz) sz) sz; sz }),
            None )
        | Droot { sorte; _ } ->
    ...
    let rec assoc_var_id vid sub =
      ...
      | ({ bv = { defn = Droot { id; _ } }; _ }, vs) :: sub ->
        ...
        then negate_bitv vs
        else assoc_var_id vid sub
      | _ :: _ -> (* Can only substitute [Droot] variables *) assert false

    (* [assoc_var v sub] is the same as [assoc_var_id] but takes a variable.

       Requires: [v] is an unconstrainted root variable or a constant. Raises:
       [Not_found] if [v] is not in the substitution, or if [v] is a constant.
       Requires: the variables in [sub] are unconstrained root variables. *)
    let assoc_var v sub =
    ...
    (* This is a helper function for [slice_vars].
       ...
       Computes the resulting composition (in normal order) in [acc] - Computes
       the resulting pattern in [pat]

       If [changed', comp', pat' = apply_sub_rev sub false [] [] rcomp] then the
       following holds:

       - [pat' = List.map (fun bv -> bv.st) comp'] - If [changed'] is [false],
       [comp' = List.rev rcomp]

       Note that the substitution can be recursive, so we must do a recursive
       call on the result of applying the substitution (we can't add [vl] to
       [acc], we must add [(`rev` vl)] to [r] instead). *)
    let rec apply_sub_rev sub changed acc pat = function
      | [] -> changed, acc, pat
      | v :: r -> (
        match assoc_var v sub with
        | vl ->
          (* Note that this is [(`rev` vl) @ r] and not [vl @ r] because the
             composition is given in reverse order. *)
    ...
    (* [slice_vars eq pat (c, req, subs)] slices the variables in [eq] according
       ...
       If the composition [eq] does not match the pattern in [pat]: - When
       variables in the composition are bigger than the expected pattern size,
       the variables are split into multiple parts, which fit the pattern.

       Splits involving A variables are not recorded, because A variables are
       guaranteed to have unique occurences.

       Splits involving B and C variables are accumulated in [subs], which is a
       pair [(b_sub, c_sub)] of substitutions for the B and C variables,
       respectively.

       Splits involving C variables can propagate to other compositions in the
       system, but not to the current composition, since C variables never occur
       twice in the same composition.
       ...
       - When variables in the composition are smaller than the expected pattern
       size (see above), the pattern must be broken up. This is recorded in [c],
       and it means that the pattern must be re-applied to any composition it
       was already applied to, which can cause more slicing.
    ...
    let rec slice_vars eq pat (c, req, subs) =
      ...
      | st :: eq, n :: pt when st.sz < n ->
        (* Since we start with a [slicing_pattern], this should only occur when
           a B-variable has been split into parts below. Therefore we assert
           that the variable is indeed a B variable and that list of
           substitutions for B variables is not empty. *)
      ...
      | st :: eq, n :: pt when st.sz = n -> slice_vars eq pt (c, st :: req, subs)
      | st :: eq, n :: pt ->
        let nvar_list, pt', flag = slice_var st n pt in
        begin match flag with
        | Some C ->
          (* A C variable got split: we must record the information in the
             C-substitution so that it gets to the instances of the variable in
             other multi-equations. *)
          let bsub, csub = subs in
          let subs = bsub, (st, nvar_list) :: csub in
          slice_vars eq pt' (c, List.rev_append nvar_list req, subs)
        | Some B ->
          (* A B variable got split: we must update the other occurences of the
             variable in the current composition. If there are other occurences
             before the current one, the information is recorded in the
             B-substitution, and we will also update it at the end. *)
          let eq =
            List.fold_right
              (fun st' acc ->
                if equal_alpha_term equal_tvar st' st
                then nvar_list @ acc
                else st' :: acc)
              eq []
          in
    ...
    (* [uniforme_slice vls] takes as argument the list of right-hand side
       ...
       same size in all concatenations). In particular, all concatenations have
       the same length.
       ...
       the original C variable has been replaced by the concatenation of the new
       C variables. The original C variable must not be used anymore, and must
       be replaced with the concatenation instead.

       This mapping can be recursive: a C-variable [c] can be substituted with
       the concatenation [c1 @ c2] of two fresh C-variables, and then [c2] can
       be substituted with the concatenation [c3 @ c4]. *)
    ...
    let apply_subs subs sys =
      ...
      in
      List.map (fun (t, vls) -> t, List.map f_aux vls) sys
    ...
    (* [equations_slice parts] takes a partitioned system [parts], i.e. a list
       ...
       - A variables occur at most once in the whole partitioned system - B
       variables occur in at most one composition of at most one multi-equation,
       but they appear multiple times within this single composition - C
       variables occur at most once in the multi-equation associated with a
    ...
    let equations_slice parts =
      let rec slice_rec bw = function
        ...
        | (t, vls) :: r ->
          ...
          if Compat.List.is_empty csub
          then slice_rec ((t, vls') :: bw) r
          else begin
            let _bw = apply_subs csub bw in
            let _fw = apply_subs csub r in
            let eq (_, l1) (_, l2) =
              (* [apply_subs] does not change the left-hand sides *)
              Compat.List.(equal (equal (equal_alpha_term equal_tvar))) l1 l2
            in
    ...
    (* [build_solution unif_slic] takes as argument a uniform system of
       ...
       by propagating equalities then replacing each term with the concatenation
       of representatives for the corresponding multi-equation.
    ...
    let build_solution unif_slic =
      ...
      List.iter
        (fun (_, vls) ->
          ...
          List.iter (List.iter2 union hd) (List.tl vls))
        unif_slic;
      ...
      in
      let to_external_ast v = { sz = v.sz; bv = get_rep v } in
      let rec cnf_max l =
        match l with
        | [] -> []
        | [elt] -> [elt]
      ...
      in
      List.map
        (fun (t, vls) -> t, cnf_max (List.map to_external_ast (List.hd vls)))
        unif_slic
      ...
      then raise Valid
        ...
        then (
          ...
          Options.heavy_assert (fun () ->
              not
              @@ equal_abstract X.equal (Canon.normalize u) (Canon.normalize v));
          raise Util.Unsolvable)
        else begin
          let st_sys = slice u v in
          if Options.get_debug_bitv () then Debug.print_sliced_sys st_sys;
          let sys_sols = sys_solve st_sys in
          if Options.get_debug_bitv () then Debug.print_c_solve_res sys_sols;
          let parts = partition (equal_alpha_term X.equal) sys_sols in
          if Options.get_debug_bitv () then Debug.print_partition_res parts;
          let unif_slic = equations_slice parts in
          if Options.get_debug_bitv () then Debug.print_partition_res unif_slic;
          let sol = build_solution unif_slic in
          if Options.get_debug_bitv () then Debug.print_final_solution sol;
          sol
        end
      end
  end
  ...
  let is_constant bitv =
    List.for_all
      (fun x ->
        match x.bv with
        | Cte _ -> true
        | Other r | Ext (r, _, _, _) -> X.is_constant r.value)
      bitv
  ...
  let is_mine bv = match is_mine_opt bv with Some r -> r | None -> X.embed bv

  let print = Debug.print_C_ast

  let make t =
    match E.term_view t with
    | { f = Op BV2Nat; _ } -> X.term_embed t, []
    | _ ->
  ...
  let solve_ter u t =
    try List.map (fun (p, v) -> p.bv, is_mine v) (Solver.solve u t)
    with Solver.Valid -> []
  ...
  let solve_bis u t =
    if Options.get_debug_bitv ()
    then
      Printer.print_dbg ~module_name:"Bitv" ~function_name:"solve_bis"
  ...
  let rec subst_rec (x : X.r) (subs : X.r) (biv : X.r abstract) : X.r abstract =
    match biv with
    | [] -> []
    | ({ bv = Cte _; _ } as y) :: biv -> y :: subst_rec x subs biv
    | { bv = Other y; _ } :: biv ->
      let y' =
        if X.equal x y.value then embed subs else embed (X.subst x subs y.value)
      in
  ...
  let subst x subs biv =
    if Options.get_debug_bitv ()
    then
      Printer.print_dbg ~module_name:"Bitv" ~function_name:"subst"
        "subst %a |-> %a in %a" X.print x X.print subs print biv;
    if Compat.List.is_empty biv
    then is_mine biv
    else
      let r = Canon.normalize (subst_rec x subs biv) in
      is_mine r

  (* module M = Map.Make (struct type t = X.r let compare = X.compare end)

     module Map = Map.Make (struct type t = (X.r simple_term) list let compare =
     compare_mine end)

     module Set = Set.Make ( struct type t = (X.r simple_term) list let compare
     = compare_mine end) *)
  ...
  let pop_bit = function
    ...
    | ({ bv = Cte n; sz } as st) :: rst ->
      ( Some (Z.testbit n (sz - 1)),
        if sz > 1 then { st with sz = sz - 1 } :: rst else rst )
  ...
  let rec search buf pos sz abstracts =
    ...
    | _, _ -> (
      (* Prefer searching where there are more candidates, i.e. less
         constraints. *)
      let x, cx, y, cy = if nt < nf then t, '1', f, '0' else f, '0', t, '1' in
    ...
    then None
      ...
      let distincts =
        List.fold_left
          (fun acc x ->
            let biv = embed x in
            if is_cte_abstract biv then biv :: acc else acc)
          [] distincts
      in
  ...
  let to_model_term r =
    ...
    | _ ->
      (* A constant semantic value cannot be a concatenation of constant simple
         term as the canonizer merges consecutive constant simple terms. *)
      None
end
