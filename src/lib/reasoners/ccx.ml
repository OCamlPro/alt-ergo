...
module Main : S = struct
      ...
      let aux fmt q =
        Q.iter
          (fun (lit, _, _) ->
            ...
            | LTerm a -> Format.fprintf fmt "  > LTerm %a@." E.print a)
          q
      in
      let aux2 fmt mp =
        Util.MI.iter
          (fun _ x -> Format.fprintf fmt "%a |-> ... (See Uf)@." X.print x)
          mp
      in
      ...
      then
        ...
        then
          print_dbg ~module_name:"Ccx" ~function_name:"make_ctx"
            "constraints of make(%a)@ %a" Expr.print t (pp_list_no_space print)
            ctx

    let rel_add_cst t ctx =
      if not (Compat.List.is_empty ctx)
      then
        Log.debug (fun k ->
            k "constraints of Rel.add(%a):@ %a" E.print t
              Fmt.(braces @@ list ~sep:comma @@ pair (A.print_view X.print) nop)
              ctx)

    let add_to_use t =
      if Options.get_debug_cc ()
      then
        print_dbg ~module_name:"Ccx" ~function_name:"add_to_use" "%a" E.print t

    (* unused -- let lrepr fmt = List.iter (fprintf fmt) "%a " X.print)

       let leaves t lvs = print_dbg (asprintf "[cc] leaves of %a" E.print t);
       lrepr (get_fmt_dbg ()) lvs *)

    let contra_congruence a ex =
      if Options.get_debug_cc ()
      then
        print_dbg ~module_name:"Ccx" ~function_name:"contra_congruence"
          "find that %a %a by contra-congruence" E.print a Ex.print ex
    ...
    let congruent a ex =
      ...
      then
        print_dbg ~module_name:"Ccx" ~function_name:"congruent"
          "new fact by congruence : %a ex[%a]" E.print a Ex.print ex
  ...
  let term_canonical_view env a ex_a = view (Uf.find env.uf) (E.lit_view a) ex_a
  ...
  let clean_use =
    List.fold_left (fun env a ->
        match E.lit_view a with
        | E.Distinct lt | E.Builtin (_, _, lt) ->
  ...
  let contra_congruence env facts r =
    ...
    if X.equal (fst (Uf.find_r env.uf r)) X.top
    then new_facts_by_contra_congruence env facts r E.faux
    else if X.equal (fst (Uf.find_r env.uf r)) X.bot
    then new_facts_by_contra_congruence env facts r E.vrai
  ...
  module HLR = Hashtbl.Make (struct
  ...
  let add env facts a ex =
    match E.lit_view a with
    | E.Pred (t1, _) -> add_term env facts t1 ex
    | E.Eq (t1, t2) ->
      let env = add_term env facts t1 ex in
      add_term env facts t2 ex
    | E.Eql lt -> List.fold_left (fun env t -> add_term env facts t ex) env lt
    | E.Distinct lt | E.Builtin (_, _, lt) ->
      let env = List.fold_left (fun env t -> add_term env facts t ex) env lt in
      let lvs = concat_leaves env.uf lt in
      (* A verifier *)
  ...
  let semantic_view env (a, ex, orig) facts =
    ...
    | LSem sa -> (
      ...
      | A.Builtin _ ->
        ...
        env, (sa, None, ex, orig)
      | _ ->
        (* XXX if we do canonical_view for A.Distinct, the theory of arrays will
           get lost *)
        env, (sa, None, ex, orig))
  ...
  let assume_eq env facts r1 r2 ex =
    ...
    if Options.get_no_contracongru () || X.type_info r1 != Ty.Tbool
    then env
    else begin
  ...
  let rec norm_queue env ineqs (facts : r Sig_rel.facts) =
    ...
    then env, List.rev ineqs
      ...
      let ineqs =
        match e with
        (* for case-split, to be sure that CS is given back to relations *)
  ...
  let are_equal env t1 t2 ~init_terms =
    if E.equal t1 t2
    then Th_util.Entailed { ex = Ex.empty; classes = [] }
    else if init_terms
    then
