...
module TSet = Set.Make (struct
  ...
  (* We use a dedicated total order on the constructors to ensure the
     termination of model generation. *)
  let compare id1 id2 = Nest.perfect_hash id1 - Nest.perfect_hash id2
end)
...
module Domain = struct
  ...
  let[@inline always] choose { constrs; _ } =
    (* We choose the minimal element to ensure the termination of model
       generation. *)
    TSet.min_elt constrs

  let[@inline always] as_singleton { constrs; ex } =
    if TSet.cardinal constrs = 1 then Some (TSet.choose constrs, ex) else None

  let domain ~constrs ex =
    if TSet.is_empty constrs then raise @@ Inconsistent ex else { constrs; ex }
  ...
  let unknown ty =
    ...
    | Ty.Tadt (name, params) ->
      ...
      let constrs =
        List.fold_left
          (fun acc Ty.{ constr; _ } -> TSet.add constr acc)
          TSet.empty cases
      in
      assert (not @@ TSet.is_empty constrs);
      { constrs; ex = Ex.empty }
    | _ ->
      (* Only ADT values can have a domain. This case shouldn't happen since we
         check the type of semantic values in both [add] and [assume]. *)
      assert false

  let equal d1 d2 = TSet.equal d1.constrs d2.constrs

  let pp ppf d =
    Fmt.(braces @@ iter ~sep:comma TSet.iter DE.Term.Const.print) ppf d.constrs;
    if Options.(get_verbose () || get_unsat_core ())
    then Fmt.pf ppf " %a" (Fmt.box Ex.print) d.ex
...
let is_adt_ty = function Ty.Tadt _ -> true | _ -> false
...
module Domains = struct
  type t =
    { domains : Domain.t MX.t;
      ...
      enums : SX.t;
          (** Set of tracked representatives whose the domain only contains enum
              constructors, that is constructors without payload.

              This field is used by the case split mechanism, see [pick_enum].
          *)
      changed : SX.t
          (** Representatives whose domain has changed since the last flush in
              [propagation]. *)
    }
  ...
  let is_enum_constr DE.{ builtin; _ } =
    match builtin with
    | B.Adt (B.Adt.Constructor { adt; case }) -> begin
      match DE.Ty.definition adt with
      | Some (Adt { cases; _ }) -> Array.length cases.(case).dstrs = 0
      | _ -> assert false
    end
    | _ -> false
  ...
  let get r t =
    ...
    | Constr { c_name; _ } ->
      (* For sake of efficiency, we don't look in the map if the semantic value
         is a constructor. *)
      Domain.singleton ~ex:Explanation.empty c_name
    | _ -> (
      try MX.find r t.domains with Not_found -> Domain.unknown (X.type_info r))
  ...
  let init r t =
    ...
    | Alien _ when not (MX.mem r t.domains) ->
      ...
      (* We have to add a default domain if the key `r` is not in map in order
         to be sure that the case split mechanism will attempt to choose a value
         for it. *)
  ...
  let tighten r d t =
    ...
    if Domain.equal od d then t else internal_update r d t
  ...
  (** [subst ~ex p v d] replaces all the instances of [p] with [v] in all
      ...
      @raise Domain.Inconsistent
        if this causes any domain in [d] to become empty. *)
  let subst ~ex r nr t =
  ...
  (* [propagate f a t] iterates on all the changed domains of [t] since the last
     call of [propagate]. The list of changed domains is flushed after this
     call. *)
...
let calc_destructor d e uf =
  ...
  match Th.embed r with
  | Constr { c_args; _ } -> begin
    match My_list.assoc DE.Term.Const.equal d c_args with
    | v -> Some (v, ex)
...
let delayed_destructor uf op = function
  | [e] -> begin
    match op with Sy.Destruct d -> calc_destructor d e uf | _ -> assert false
  end
  | _ -> assert false

let is_ready r = match Th.embed r with Constr _ -> true | _ -> false
...
type t =
  { delayed : Rel_utils.Delayed.t;
    ...
    (* Product of the size of all the facts learnt by CS assumed in the theory.

       Currently this field is unused. *)
    new_terms : SE.t
        (* Set of all the constructor applications built by the theory. See the
           function [deduce_is_constr]. *)
  }
...
module Debug = struct
  let assume l =
    Log.debug (fun k ->
        k "assume the literal:@ %a" (Xliteral.print_view X.print) l)
end
(*BISECT-IGNORE-END*)
(* ################################################################ *)
...
let assume_th_elt t th_elt _ =
  ...
  | Util.Adt -> failwith "This Theory does not support theories extension"
  | _ -> t
...
(* Update the domains of the semantic values [r1] and [r2] according to the
   ...
   We plan to support model generation for ADT. As a by-product, we will got a
   complete decision procedure for the ADT through the case-split mechanism as
   we did in the Enum theory.

   @raise Domain.Inconsistent if the disequality is inconsistent with the
   current environment [env]. *)
let assume_distinct =
  let aux ~ex r1 r2 domains =
    match Th.embed r1 with
    | Constr { c_args; _ } when List.length c_args = 0 -> begin
    ...
    | _ ->
      (* If `d1` is a singleton domain but its constructor has a payload, we
         cannot tighten the domain of `r2` because they could have distinct
         payloads. *)
      domains
  in
  fun ~ex r1 r2 domains -> aux ~ex r1 r2 domains |> aux ~ex r2 r1

(* Assumes the tester `((_ is c) r)` where [r] can be a constructor application
   or a uninterpreted semantic value.

   @raise Domain.Inconsistent if we already know that the value of [r] cannot be
   an application of the constructor [c]. *)
...
(* Assume `(not ((_ is c) r))` where [r] can be a constructor application or a
   uninterpreted semantic value.

   We remove the destructor equations associated with [r] and [c].

   @raise Domain.Inconsistent if we already know that the value of [r] has to be
   an application of the constructor [c]. *)
  ...
  List.fold_left
    (fun domains lit ->
      ...
      | (Distinct (false, [r1; r2]) as l), _, ex, _ when is_adt r2 ->
        ...
        (* The explanation [ex] explains why [r1] and [r2] are distinct, which
           is not sufficient to justify why [rr1] and [rr2] are distinct. *)
...
(* For a uninterpreted semantic value [r] and [c] a constructor of the form:
   (cons (d1 ty1) ... (dn tyn)) this function creates the equation: [t = cons x1
   ... xn] where x1, ..., xn are fresh names of type respectively ty1, ..., tyn
   and [t] is the term associated with the uninterpreted semantic value [r].

   Assume that [r] is a contructor application of an alien semantic value of
   type [Ty.Adt]. *)
let build_constr_eq r c =
  ...
  | Alien r -> begin
    ...
    | Ty.Tadt (name, params) as ty ->
      let cases = Ty.type_body name params in
      let ds = try Ty.assoc_destrs c cases with Not_found -> assert false in
  ...
  | Constr _ ->
    (* The semantic value [r] is already a constructor application, so we don't
       need to produce a new equation in this case. *)
    None
...
let propagate_domains new_terms domains =
  Domains.propagate
    (fun (eqs, new_terms) rr d ->
      ...
      | Some (c, ex) -> begin
        match build_constr_eq rr c with
        | Some (eq, cons) ->
...
let assume env uf la =
  ...
  let domains =
    try assume_literals la uf domains
  ...
  let env = { delayed; size_splits = count_splits env la; new_terms } in
...
(* Do a case split by choosing a semantic value [r] and constructor [c] for
   which there are delayed destructor applications and propagate the literal
   [(not (_ is c) r)]. *)
let split_delayed_destructor env =
  if not @@ Options.get_enable_adts_cs ()
  then None
  else
    try
...
(* Pick a constructor in a tracked domain with minimal cardinal. Returns [None]
...
let pick_domain ~for_model uf =
  ...
  match pick_enum ds with None when for_model -> pick_best ds | r -> r

let split_domain ~for_model env uf =
  let* cd, r, c = pick_domain ~for_model uf in
  if for_model || can_split env (Numbers.Q.from_int cd)
  then
...
let case_split env uf ~for_model =
  ...
  then []
    ...
    | Some cs ->
      Log.debug (fun k ->
          k "environment before case split:@ %a" Domains.pp
            (Uf.GlobalDomains.find (module Domains) (Uf.domains uf)));
      Log.debug (fun k ->
          k "assume by case splitting:@ %a" (Xliteral.print_view X.print) cs);
  ...
  then Th_util.Unknown
    ...
    try
      ...
      | Xliteral.Builtin (false, Sy.IsConstr c, [r]) ->
        ...
        Unknown
      | _ -> Unknown
    with Domain.Inconsistent ex -> Entailed { ex; classes = Uf.cl_extract uf }
