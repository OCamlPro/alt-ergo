...
module Ring (C : Core) (RT : RingType) = struct
  ...
  let trace1 fname u u' =
    Log.debug (fun m ->
        m "@[%s(%a) = %a@]" fname (Fmt.box pp) u (Fmt.box pp) u');
    u'

  let trace2 fname u1 u2 u =
    Log.debug (fun m ->
        m "@[%s(%a, %a) = %a@]" fname (Fmt.box pp) u1 (Fmt.box pp) u2
          (Fmt.box pp) u);
    u
  ...
  let scale alpha u =
    ...
    if c < 0
    then map_strict_dec (RT.mul alpha) u
    else if c > 0
    then map_strict_inc (RT.mul alpha) u
    else invalid_arg "scale: cannot scale by zero"
  ...
  let pow n u =
    ...
    else map_strict_inc (RT.pow n) u
end
...
module Field (C : Core) (FT : FieldType) = struct
  ...
  let inv ?(inv0 = Interval.full) u =
    ...
    @@ trisection_map_to_set FT.zero u (map_dec_to_set FT.inv)
         (fun () -> interval_set inv0)
         (map_dec_to_set FT.inv)

  let div ?inv0 x y = mul x (inv ?inv0 y)
end
...
module AlgebraicField (C : Core) (AT : AlgebraicType) = struct
  ...
  let root n u =
    ...
    else approx_map_inc_to_set (AT.root_default n) (AT.root_excess n) u
end
...
module ZEuclideanType = struct
  ...
  let succ = function Finite x -> Finite (Z.succ x) | inf -> inf

  let pred = function Finite x -> Finite (Z.pred x) | inf -> inf
  ...
  let mul x y =
    ...
    | ((Pos_infinite | Neg_infinite) as inf), Finite fin ->
      let s = Z.sign fin in
      if s > 0 then inf else if s < 0 then neg inf else Finite Z.zero

  let pow n x =
    if n <= 0 then Fmt.invalid_arg "pow: nonpositive exponent %d" n;
    match x with
    | Pos_infinite -> Pos_infinite
    | Neg_infinite -> if n mod 2 = 0 then Pos_infinite else Neg_infinite
    | Finite x -> Finite (Z.pow x n)

  let ediv x y =
    match x, y with
    | Pos_infinite, Pos_infinite | Neg_infinite, Neg_infinite -> Pos_infinite
    | Neg_infinite, Pos_infinite | Pos_infinite, Neg_infinite -> Neg_infinite
...
module QAlgebraicType = struct
  ...
  let floor = function
    ...
    | Finite (k, x) ->
      ...
      if Z.equal (Q.den x) Z.one
      then
        match k with
        | Lt -> Finite (Z.pred @@ Q.num x)
        | Eq | Gt -> Finite (Q.num x)
      else Finite (Z.fdiv (Q.num x) (Q.den x))
    | Pos_infinite -> Pos_infinite
  ...
  let ceiling = function
    ...
    | Finite (k, x) ->
      ...
      else Finite (Z.cdiv (Q.num x) (Q.den x))
    | Pos_infinite -> Pos_infinite
  ...
  let equal x y =
    ...
    | Pos_infinite, Pos_infinite -> true
    | Pos_infinite, _ | _, Pos_infinite -> false
    | Finite (kx, x), Finite (ky, y) -> Q.equal x y && equal_kind kx ky
  ...
  let compare x y =
    ...
    | Finite (kx, x), Finite (ky, y) ->
      ...
      if c <> 0 then c else compare_kind kx ky
    | Pos_infinite, Pos_infinite -> 0
    | Pos_infinite, _ -> 1
    | _, Pos_infinite -> -1
  ...
  let succ = function
    ...
    | Finite (Gt, _) as x -> Fmt.invalid_arg "succ: %a" pp x
    | inf -> inf

  let pred = function
    | Finite (Lt, _) as x -> Fmt.invalid_arg "pred: %a" pp x
  ...
  let sign_kind = function Lt -> -1 | Eq -> 0 | Gt -> 1

  let kind_of_sign s = if s < 0 then Lt else if s > 0 then Gt else Eq
  ...
  let add x y =
    ...
    | Finite (kx, qx), Finite (ky, qy) ->
      ...
      if skx * sky < 0 then Fmt.invalid_arg "add: %a + %a" pp x pp y;
  ...
  let sign = function
    ...
    | Finite (k, x) ->
      let s = Q.sign x in
      if s <> 0 then s else sign_kind k

  let mul x y =
    match x, y with
    | Pos_infinite, Pos_infinite | Neg_infinite, Neg_infinite -> Pos_infinite
    | Pos_infinite, Neg_infinite | Neg_infinite, Pos_infinite -> Neg_infinite
    | ((Pos_infinite | Neg_infinite) as i), (Finite _ as f)
    | (Finite _ as f), ((Pos_infinite | Neg_infinite) as i) ->
      let s = sign f in
      if s > 0 then i else if s < 0 then neg i else Finite (Eq, Q.zero)
    | Finite (kx, qx), Finite (ky, qy) ->
      let sx = sign x and sy = sign y in
      if sx = 0 || sy = 0
      then Finite (Eq, Q.zero)
      else
        let skx = sign_kind kx and sky = sign_kind ky in
        (* Check that [x] and [y] do not have opposite epsilon if they have the
           same sign, or identical epsilon if they have opposite signs: this
           matches the monotonicity of multiplication. *)
  ...
  let pow n x =
    ...
    | Finite (k, qx) ->
      ...
      if n mod 2 = 0 && sign x < 0
      then Finite (neg_kind k, qpow)
      else Finite (k, qpow)
  ...
  let inv = function
    ...
    | Finite (k, x) ->
      ...
      then
        ...
        | Gt -> Pos_infinite
      else Finite (neg_kind k, Q.inv x)
...
module Int = struct
    ...
    @@ map_to_set
         (fun i ->
           ...
           | ZEuclideanType.Neg_infinite, _ | _, Pos_infinite ->
             interval_set full
           | _, Neg_infinite | Pos_infinite, _ -> assert false
           | Finite lb, Finite ub ->
             let lb = Z.shift_right lb ofs in
             let ub = Z.shift_right ub ofs in
             if Z.(numbits (ub - lb)) <= len
             then
               (* The image spans an interval of length at most [len] *)
               let lb_mod = Z.extract lb 0 len in
               let ub_mod = Z.extract ub 0 len in
               if Z.(compare lb_mod ub_mod) <= 0
               then
                 interval_set
                 @@ Interval.of_bounds (Closed lb_mod) (Closed ub_mod)
               else
                 union_set
                   (interval_set
                   @@ Interval.of_bounds (Closed Z.zero) (Closed ub_mod))
                   (interval_set
                   @@ Interval.of_bounds (Closed lb_mod) (Closed max_val))
             else
               (* The image is too large; all values are possible. *)
               interval_set full)
         u

  let lognot u = trace1 "lognot" u @@ map_strict_dec ZEuclideanType.lognot u
  ...
  let bvurem u1 u2 =
    ...
    @@ map_to_set
         (fun i2 ->
           if ZEuclideanType.equal i2.ub ZEuclideanType.zero
           then
             (* [y] is always zero -> identity *)
             map_to_set interval_set u1
           else if ZEuclideanType.compare i2.ub ZEuclideanType.zero < 0
           then
             (* Safety check -- bvurem only makes sense if [u2] is
                nonnegative. *)
             invalid_arg "bvurem"
           else
             map_to_set
               (fun i1 ->
                 if ZEuclideanType.compare i1.ub i2.lb < 0
                 then
                   (* x < y : bvurem is the identity *)
                   interval_set i1
                 else if ZEuclideanType.equal i2.lb ZEuclideanType.zero
                 then
                   (* The range [0, i1.ub] is always valid; it is also the best
                      we can do if [y] can be [0]. *)
                   interval_set { i1 with lb = ZEuclideanType.zero }
                 else
                   (* y is non-zero; we have both [x % y < y] and [x % y <= x]
                      so take the min of these upper bounds. *)
                   let ub =
                     if ZEuclideanType.compare i1.ub i2.ub < 0
                     then i1.ub
  ...
  let bvshl ~size u1 u2 =
    ...
    extract ~ofs:0 ~len:size @@ of_set_nonempty
    @@ map_to_set
         (fun i2 ->
           assert (ZEuclideanType.sign i2.lb >= 0);
           if ZEuclideanType.(compare i2.lb (finite @@ Z.of_int max_int)) > 0
           then
             (* if i2.lb > max_int, the result is always zero must not call
                ZEuclideanType.shift_left or we will likely OOM *)
             interval_set zero_i
           else
  ...
  let lshr u1 u2 =
    of_set_nonempty @@ map2_mon_to_set ZEuclideanType.shift_right Inc u1 Dec u2
end
...
module Legacy = struct
  ...
  let check = function NonEmpty u -> u | Empty ex -> raise @@ NotConsistent ex
  ...
  let point v ty ex =
    match ty with
    | Ty.Treal -> of_real @@ Real.of_interval ~ex @@ Real.Interval.singleton v
  ...
  let map2 fname freal fint u1 u2 =
    ...
    | _ -> Fmt.invalid_arg "%s: cannot %s int and real" fname fname
  ...
  let affine_scale ~const ~coef u =
    ...
    match u with
    | Real u -> of_real @@ Real.map_strict_inc (fun b -> (b * scale) + offset) u
    | Int u ->
      of_int @@ check
      @@ Int.partial_map_inc
  ...
  let integer_hull = function
    ...
    | Int u ->
      ...
      let lb = match lb with Closed v -> Some (v, ex_lb) | _ -> None in
      let ub = match ub with Closed v -> Some (v, ex_ub) | _ -> None in
      lb, ub
  ...
  let coerce ty u =
    match ty, u with
    | Ty.Treal, Real _ -> u
    | Treal, Int u -> of_real @@ Real.map_strict_inc QAlgebraicType.of_bigint u
    | Tint, Int _ -> u
    | Tint, Real u ->
      of_int @@ check
  ...
  let pick ~is_max u =
    ...
    | Real u ->
      let pick_interval lb ub =
        ...
        | Open lb, (Open ub | Closed ub) -> Q.(lb + min ~$1 ((ub - lb) / ~$2))
  ...
  let fold f acc u =
    match u with
    | Real u -> Real.fold (fun acc i -> f acc (Real.Interval.view i)) acc u
    | Int u ->
      Int.fold
        (fun acc i ->
          f acc
  ...
  let new_up_bound idoms s ty q is_strict =
    let old_low, old_up, ty =
      try MV.find s idoms with Not_found -> None, None, ty
    in
    let new_up =
      match old_up with
      ...
      | Some (r, str) ->
        ...
        then Some (q, is_strict)
        else if is_strict == str || is_strict
        then old_up
        else Some (q, is_strict)
    in
    if new_up == old_up
    then idoms
    else if consistent_bnds old_low new_up
    then MV.add s (old_low, new_up, ty) idoms
    else raise Exit
  ...
  let new_low_bound idoms s ty q is_strict =
    ...
    if new_low == old_low
    then idoms
    else if consistent_bnds new_low old_up
    then MV.add s (new_low, old_up, ty) idoms
    else raise Exit

  let new_var idoms s ty =
    if MV.mem s idoms then idoms else MV.add s (None, None, ty) idoms
...
module DebugExplanations : Explanations with type t = string list = struct
  ...
  let union l1 l2 = List.rev_append l1 l2 |> List.sort_uniq String.compare

  let compare l1 l2 = Compat.List.compare String.compare l1 l2
end
