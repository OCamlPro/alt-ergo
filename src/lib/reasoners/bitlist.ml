...
(** A bitlist representing the known bits of an infinite-width bit-vector.
    ...
    Active bits in [bits_set] are necessarily equal to [1]. Active bits in
    [bits_unk] are not known and may be either [0] or [1]. Bits that are active
    in neither [bits_set] nor [bits_unk] are equal to [0].
...
let constant n ex = { bits_set = n; bits_unk = Z.zero; ex }
...
let exact value ex = { bits_set = value; bits_unk = Z.zero; ex }

let equal b1 b2 =
  Z.equal b1.bits_set b2.bits_set && Z.equal b1.bits_unk b2.bits_unk
...
let pp ppf { bits_set; bits_unk; ex } =
  begin if Z.sign bits_unk < 0
  then
    (* Sign is not known *)
    Fmt.pf ppf "(?)"
  else if Z.sign bits_set < 0
  then Fmt.pf ppf "(1)"
  else Fmt.pf ppf "(0)"
  end;
  let width = max (Z.numbits bits_set) (Z.numbits bits_unk) in
  for i = width - 1 downto 0 do
    if Z.testbit bits_set i
    then Fmt.pf ppf "1"
    else if Z.testbit bits_unk i
    then Fmt.pf ppf "?"
    else Fmt.pf ppf "0"
  done;
  if Options.(get_verbose () || get_unsat_core ())
  then Fmt.pf ppf " %a" (Fmt.box Ex.print) ex
...
let intersect b1 b2 =
  ...
  (* If there is a bit that is known in both bitlists with different values, the
     intersection is empty. *)
  let distinct = Z.logxor b1.bits_set b2.bits_set in
  let known = Z.lognot (Z.logor b1.bits_unk b2.bits_unk) in
  if not (Z.equal (Z.logand distinct known) Z.zero)
  then raise @@ Inconsistent (Ex.union b1.ex b2.ex);
...
let logor b1 b2 =
  ...
  let bits_unk =
    Z.logand (Z.logor b1.bits_unk b2.bits_unk) (Z.lognot bits_set)
  in
  (* Always consistent *)
  { bits_set; bits_unk; ex = Ex.union b1.ex b2.ex }
...
let logxor b1 b2 =
  ...
  (* Need to mask because [Z.logxor] will compute a bogus value for unknown
     bits. *)
  let bits_set =
    Z.logand (Z.logxor b1.bits_set b2.bits_set) (Z.lognot bits_unk)
  in
  (* Always consistent *)
  { bits_set; bits_unk; ex = Ex.union b1.ex b2.ex }
...
(* [left_cl_can_set highest_cleared cleared_can_set] returns the
   least-significant bit that is: - More significant than [highest_cleared],
   strictly; - Set in [cleared_can_set]
...
let increase_lower_bound b lb =
  ...
  if c > 0
  then
    (* [set_bits > cleared_bits] means that the most-significant changed bit was
       0, and is now 1.
...
let fold_domain f b acc =
  ...
  if Z.sign b.bits_unk < 0 then invalid_arg "Bitlist.fold_domain";
  ...
  let rec fold_domain_aux ofs b acc =
    ...
    then (
      ...
      f (value b) acc)
    else if not (Z.testbit b.bits_unk ofs)
    then fold_domain_aux (ofs + 1) b acc
    else
      let mask = Z.(one lsl ofs) in
      let bits_unk = Z.logand b.bits_unk (Z.lognot mask) in
      let acc = fold_domain_aux (ofs + 1) { b with bits_unk } acc in
      fold_domain_aux (ofs + 1)
        { b with bits_unk; bits_set = Z.logor b.bits_set mask }
        acc
  in
...
(* simple propagator: only compute known low bits
   ...
   example: ???100 * ???000 = ?00000 (trailing zeroes accumulate) ???111 *
   ????11 = ????01 (min of low bits known) *)
let mul a b =
  ...
  if zeroes_a = max_int || zeroes_b = max_int
  then zero ex
  else
    let a = a asr zeroes_a in
    let b = b asr zeroes_b in
    let zeroes = zeroes_a + zeroes_b in
    (* a = ah * 2^n + al (0 <= al < 2^n) b = bh * 2^m + bl (0 <= bl < 2^m)

       ((ah * 2^n) + al) * ((bh * 2^m) + bl) = al * bl (mod 2^(min n m)) *)
    ...
    in
    if low_known = max_int
    then exact Z.(mid_bits lsl zeroes) ex
    else
...
let bvshl ~size:sz a b =
  (* If the minimum value for [b] is larger than the bitwidth, the result is
     ...
     matches the bitlist pattern, and so is a lower bound. Ideally we would like
     to use the lower bound from the interval domain for [b] instead. *)
  match Z.to_int (increase_lower_bound b Z.zero) with
  | n when n < sz ->
    let low_zeros = Z.trailing_zeros @@ Z.logor a.bits_set a.bits_unk in
    if low_zeros + n >= sz
    then extract (exact Z.zero (Ex.union (explanation a) (explanation b))) 0 sz
    else if low_zeros + n > 0
    then
      (extract unknown 0 (sz - low_zeros - n) lsl (low_zeros + n))
      lor extract
            (exact Z.zero (Ex.union (explanation a) (explanation b)))
            0 (low_zeros + n)
    else extract unknown 0 sz
  | _ | (exception Z.Overflow) -> constant Z.zero (explanation b)
...
let bvlshr ~size:sz a b =
  ...
  | n when n < sz ->
    ...
    then (* MSB is zero *)
      ...
      if nb_zeros >= sz
      then constant Z.zero (Ex.union (explanation a) (explanation b))
      else
    ...
    then
      (constant Z.zero (explanation b) lsl (sz - n))
      lor extract unknown 0 (sz - n)
    else extract unknown 0 sz
  | _ | (exception Z.Overflow) -> constant Z.zero (explanation b)

let add a b =
  (* The implementation below is a bit magical, so let us draft a proof of
     correctness.

     Remark 1: A binary addition [x + y] has a carry at bit position [i] iff the
     addition of the lower bits of [x] and [y] overflows. In other words,
     ...
     addition can only introduce new carry positions but never remove them. This
     follows from Remark 1 since flipping bit [j] from [0] to [1] adds [2^j] to
     the value, and hence either increases or does not change the value of [x
     \bmod 2^i] in the inequality above, depending on whether [j < i] or [j >=
     i] holds.

     For a bitlist [b], let us write {m m_b} the value [a.bits_set] and {m M_b}
     the value [a.bits_set + a.bits_unk].

     Definition: We say that a bit position [i] is {e known} in a set {m S} (not
     necessarily a bitlist) if has the same value for all the integers in {m S}.

     Theorem: If [a] and [b] are bitlists, let us denote by [a + b] the set {m
     \{ x + y \mid x \in a, y \in b \}}. Then, a bit position [i] is known in [a
     + b] iff it is known in both [a] and [b], and the values {m m_a + m_b} and
     {M_a + M_b} agree on bit [i].
     ...
     still obtain a value in [a + b]. Moreover, since {m m_a + m_b} and {m M_a +
     M_b} is in [a + b], they must agree on bit position [i].

     Proof (reverse implication): Consider a bit position [i] that is known in
     [a] and in [b], and where {m m_a + m_b} and {m M_a + M_b} agree. Assume
     that we have {m x \in a} and {m y \in b} such that {m x + y} disagree with
     {m M_a + M_b} on bit position [i].

     Since bit [i] is known in both [a] and [b], the only difference at bit
     position [i] between {m m_a + m_b} and {m x + y} can come from one having a
     carry and not the other.

     Since [a] and [b] are bitlists, we can obtain {m x \bmod 2^i} (resp. {m y
     \bmod 2^i}) by flipping bits from [0] to [1] in {m m_a} (resp. {m m_a}),
     and we can obtain {m M_a} (resp. {m M_B})by flipping bits from [0] to [1]
     in {m x \bmod 2^i} (resp. {m y \bmod 2^i}).

     Hence, by remark 2, if {m m_a + m_b} has a carry at position [i], then so
     do {m x + y} and {m M_a + M_b}; conversely, if {m M_a + M_b} has no carry
     at position [i], then neither do {m x + y} and {m m_a + m_b}. *)
  let min_add = Z.(a.bits_set + b.bits_set) in
  let max_add = Z.(min_add + a.bits_unk + b.bits_unk) in
  let bits_unk = Z.(a.bits_unk lor b.bits_unk lor (min_add lxor max_add)) in
...
let sub a b =
  ...
  let bits_unk = Z.(a.bits_unk lor b.bits_unk lor (min_sub lxor max_sub)) in
