...
module DomainMap (X : ComparableType) (D : Domain) (W : OrderedType) : sig
  ...
  (** Removes the domain associated with a single variable. This will
      effectively remove the domains associated with all keys that have the same
      variable part. *)

  module Ephemeral :
    EphemeralDomainMap with type key = key and type Entry.domain = domain

  val edit : notify:(key -> unit) -> default:(key -> domain) -> t -> Ephemeral.t
  ...
  type t =
    { domains : D.t MX.t;  (** Map from variables to their domain. *)
  ...
  let add_watches x ws t =
    let watches =
      MX.update x
        (function None -> Some ws | Some ws' -> Some (W.Set.union ws ws'))
        t.watches
    and watching =
      W.Set.fold
        (fun w watching ->
          W.Map.update w
            (function
              | None -> Some (SX.singleton x) | Some xs -> Some (SX.add x xs))
            watching)
        ws t.watching
    in
  ...
  let watches x t = try MX.find x t.watches with Not_found -> W.Set.empty
  ...
  module Ephemeral = struct
    ...
    let entry t x =
      try X.Table.find t.entries x
      with Not_found ->
        let entry =
          { Entry.key = x;
            notify = t.notify;
            domain =
              (try X.Map.find x t.domains with Not_found -> t.default x);
            dirty = false;
...
module Make
  ...
  type t =
    { atoms : DMA.t;
      ...
      variables : A.Set.t; (* Set of all atomic variables being tracked. *)
      composites : DMC.t;
  ...
  let watch w r t =
    ...
    | Constant _ -> t
    | Atom (a, _) ->
      { t with atoms = DMA.add_watches a (W.Set.singleton w) t.atoms }
    | Composite (c, _) ->
      { t with composites = DMC.add_watches c (W.Set.singleton w) t.composites }
  ...
  let init r t =
    match NF.normal_form r with
    | Constant _ -> t
    | Atom (a, _) -> { t with variables = A.Set.add a t.variables }
    | Composite (c, _) -> { t with parents = track c t.parents }

  let default_atom a = D.unknown (NF.type_info a)

  let find_or_default_atom a t =
    try DMA.find a t.atoms with Not_found -> default_atom a

  let default_composite c = D.map_domain default_atom c

  let find_or_default_composite c t =
    try DMC.find c t.composites with Not_found -> default_composite c

  let find_or_default x t =
    match x with
    | NF.Constant c -> D.constant c
    | NF.Atom (a, o) -> D.add_offset (find_or_default_atom a t) o
    | NF.Composite (c, o) -> D.add_offset (find_or_default_composite c t) o
  ...
  let subst ~ex rr nrr t =
    ...
    let t =
      ...
      | Composite (c, _) ->
        ...
        { t with composites; parents }
    in
    if D.equal nnrrd nrrd
    then t
    else
      match nrr_nf with
      | Constant _ ->
        (* [nrrd] is [D.constant c] which must be a singleton; if we shrunk it,
  ...
  module Ephemeral = struct
    ...
    module Entry = struct
      ...
      let domain = function
        ...
        | Atom (a, o) -> D.add_offset (DMA.Ephemeral.Entry.domain a) o
        | Composite (c, o) -> D.add_offset (DMC.Ephemeral.Entry.domain c) o

      let set_domain e d =
        match e with
        | Constant _ -> assert false
        | Atom (a, o) -> DMA.Ephemeral.Entry.set_domain a (D.sub_offset d o)
        | Composite (c, o) ->
    ...
    let entry t r =
      match NF.normal_form r with
      | NF.Constant c -> Entry.Constant c
      | NF.Atom (a, o) -> Atom (DMA.Ephemeral.entry t.atoms a, o)
      | NF.Composite (c, o) ->
    ...
    let update ~ex entry domain =
      ...
      if not (D.equal domain current) then Entry.set_domain entry domain
    ...
    module Canon = struct
      ...
      module Entry = struct
        ...
        let domain { repr; entry; explanation = ex } =
          if Explanation.is_empty ex
          then Entry.domain entry
          else
      ...
      let entry t r =
        try HX.find t.cache r
        with Not_found ->
          let r, explanation = Uf.find_r t.uf r in
          let h = { Entry.repr = r; entry = entry t.domains r; explanation } in
          HX.replace t.cache r h;
          h
    ...
    let canon uf domains = { Canon.uf; cache = HX.create 17; domains }
  end
  ...
  let edit ~events t =
    ...
    in
    { Ephemeral.atoms =
        DMA.edit ~notify:notify_atom ~default:default_atom t.atoms;
      variables = t.variables;
      composites =
        DMC.edit ~notify:notify_composite ~default:default_composite
          t.composites;
      parents = t.parents
    }
