...
(* We put an ml file for the module type, to avoid issues when building the
   lib *)
...
module type S = sig
  ...
  (** [empty ~selector ()] creates an empty environment.
      ...
      The optional argument [selector] is used to filter ground facts discovered
      by the instantiation engine. *)

  val declare : t -> Id.typed -> unit
  (** [declare env id] declares a new identifier [id].

      If the environment [env] isn't unsatisfiable and the model generation is
      enabled, the solver produces a model term for [id] which can be retrieved
      with [get_model]. *)

  val push : t -> int -> unit
  (** [push env n] adds [n] new assertion levels in [env].

      Internally, for each new assertion level, a fresh guard [g] is created and
      all formulas [f] assumed at this assertion level is replaced by [g ==> f].
  ...
  (** [assume env f dep] assumes the ground formula [f] in [env]. The [dep]
  ...
  (** [assume env f exp] assumes a new formula [f] with the explanation [exp] in
      the theory environment of [env]. *)
  ...
  (** [get_model t] produces the current first-order model. Notice that this
      model is a best-effort.

      @return
        [None] if the model generation is not enabled or the environment is
        unsatisfiable. *)

  val get_unknown_reason : t -> unknown_reason option
  (** [get_unknown_reason t] returns the reason Alt-Ergo raised [I_dont_know] if
      it did. If it did not, returns [None]. *)

  val get_value : t -> Expr.t -> Expr.t option
  (** [get_value t e] returns the value of [e] as a constant expression in the
      current model generated. Returns [None] if can't decide. *)
