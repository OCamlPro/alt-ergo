...
module Shostak
  ...
  let mk_euc_division ~with_facts p p2 t1 t2 ctx =
    match P.to_list p2 with
    | [], coef_p2 ->
      let md = E.mk_term (Sy.Op Sy.Modulo) [t1;t2] Ty.Tint in
      let r, ctx' = X.make ~with_facts md in
      let rp =
  ...
  let mk_partial_interpretation_1 aux_func coef p_acc ty t x =
    let r_x, _ = X.make ~with_facts:false x in
    try
  ...
  let mk_partial_interpretation_2 aux_func coef p_acc ty t x y =
    let px = embed (fst (X.make ~with_facts:false x)) in
    let py = embed (fst (X.make ~with_facts:false y)) in
    try
  ...
  let rec mke ~with_facts coef p t ctx =
    let { E.f = sb ; xs; ty; _ } =
    ...
    | Sy.Op Sy.Mult, [t1;t2] ->
      let p1, ctx = mke ~with_facts coef (empty_polynome ty) t1 ctx in
      let p2, ctx = mke ~with_facts Q.one (empty_polynome ty) t2 ctx in
      if get_no_nla() && P.is_const p1 == None && P.is_const p2 == None
      then
        (* becomes uninterpreted *)
        let tau = E.mk_term (Sy.name ~kind:Sy.Ac "@*") [t1; t2] ty in
        let xtau, ctx' = X.make ~with_facts tau in
        P.add p (P.create [coef, xtau] Q.zero ty), List.rev_append ctx' ctx
      else
        P.add p (P.mult p1 p2), ctx

    | Sy.Op Sy.Div, [t1;t2] ->
      let p1, ctx = mke ~with_facts Q.one (empty_polynome ty) t1 ctx in
      let p2, ctx = mke ~with_facts Q.one (empty_polynome ty) t2 ctx in
      if get_no_nla() &&
         (P.is_const p2 == None ||
        ...
        let p3, ctx =
          try
            ...
            if approx then mk_euc_division ~with_facts p p2 t1 t2 ctx
            else p, ctx
    ...
    | Sy.Op Sy.Plus , l ->
      List.fold_left (fun (p, ctx) u -> mke ~with_facts coef p u ctx )(p, ctx) l

    | Sy.Op Sy.Minus , [t1;t2] ->
      let p2, ctx = mke ~with_facts (Q.minus coef) p t2 ctx in
      mke ~with_facts coef p2 t1 ctx
    ...
    | _ ->
      let a, ctx' = X.make ~with_facts t in
      let ctx = ctx' @ ctx in
  ...
  let make ~with_facts t =
    ...
    let p, ctx = mke ~with_facts Q.one (empty_polynome ty) t [] in
    is_mine p, ctx
