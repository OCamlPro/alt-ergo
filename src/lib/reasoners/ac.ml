...
module Make (X : Sig.X) = struct
  ...
  module Debug = struct
    ...
    let print fmt { h; l; _ } =
      if Sy.equal h (Sy.Op Sy.Mult)
      then Format.fprintf fmt "%a" (pr_xs "'*'") l
      else Format.fprintf fmt "%a(%a)" Sy.print h (pr_xs ",") l

    let assert_compare a b c1 c2 =
      assert (
        if not ((c1 = 0 && c2 = 0) || (c1 < 0 && c2 > 0) || (c1 > 0 && c2 < 0))
        then begin
          print_err
            "Ac.compare:@ %a vs %a@  = %d@ But@ Ac.compare:@ %a vs %a@  = %d"
    ...
    let subst p v tm =
      ...
      then
        print_dbg "[ac] subst %a by %a in %a" X.print p X.print v X.print
          (X.ac_embed tm)
  end
  (*BISECT-IGNORE-END*)
  ...
  (* This implements a variant of the term abstraction process described in
     ...
     symbols appearing in a non-AC context, but the implementation does not know
     about the context at the time the abstraction is performed (note that rules
     Abstract1 and Abstract2 are concerned with *equations* while the
     ...
     term ordering (older terms are ordered before newer terms, so in particular
     subterms are always smaller than terms that contains them) to cheaply
     prevent loops rather than introducing all the abstracted variables that the
     theoretical presentation in the paper would require.
     ...
     satisfy the T_AC definition from page 22 of the paper, although correctness
     of the corresponding abstraction process has not been proven. See also
     https://github.com/OCamlPro/alt-ergo/issues/989

     [1]: Canonized Rewriting and Ground AC Completion Modulo Shostak Theories:
     Design and Implementation. Sylvain Conchon, Evelyne Contejean, Mohamed
     Iguernelala. lmcs:1034 - Logical Methods in Computer Science, September 14,
     2012, Volume 8, Issue 3. doi:10.2168/LMCS-8(3:16)2012
     https://arxiv.org/pdf/1207.3262.pdf *)
  let abstract2 sy t r acc =
    if List.exists (is_other_ac_symbol sy) (X.leaves r)
    then
  ...
  let make t =
    ...
    | { xs; _ } ->
      Printer.print_err
        "AC theory expects only terms with 2 arguments; got %i (%a)."
        (List.length xs) Expr.print_list xs;
      assert false
  ...
  let compare { h = f; l = x; _ } { h = g; l = y; _ } =
    ...
    then c
      ...
      then c
        ...
        if c <> 0 then c else mset_cmp (x, y)
  ...
  (* let mset_compare ord {h=f ; l=x} {h=g ; l=y} = let c = Sy.compare f g in if
     c <> 0 then c else assert false *)
  ...
  let assign_value _r _distincts _eq = None
  (* Models Gen for AC symbols is not done yet. The code below would work, but
     the way models are currently generated makes the result of 'assign_value'
     not visible in the printed models, because we inspect the 'make : Expr.t ->
     X.r' to extract the model. Unfortunately, some AC expressions introduced by
     the AC(X) algorithm don't have their corresponding terms and don't appear
     in the 'make' map *)

  (* let is_fresh (r, _) = match X.term_extract r with | Some t, true ->
     Expr.is_fresh t | _ -> false

     let assign_value = let cache = ref Ty.Map.empty in let module SX =
     Set.Make(struct type t=r let compare = X.hash_cmp end) in let exception
     Found of Expr.t in fun r distincts eq -> if List.exists (fun (t,(_:r)) ->
     Expr.is_model_term t) eq then None else (*match X.ac_extract r with | None
     -> assert false | Some ac -> if List.for_all is_fresh ac.Sig.l then None
     else*) begin let ty = X.type_info r in let q = ref (Queue.create ()) in try
     let qc = Ty.Map.find ty !cache in let sdist = List.fold_left (fun s r ->
     SX.add r s) SX.empty distincts in Queue.iter (fun (kt, kx) -> if not
     (SX.mem kx sdist) then raise (Found kt) )qc; q := qc; raise Not_found with
     | Found kt -> Some (kt , true) | Not_found -> let fresh = Expr.fresh_name
     ty in Queue.push (fresh, X.term_embed fresh) !q; cache := Ty.Map.add ty !q
     !cache; Some (fresh , false) end *)
end
