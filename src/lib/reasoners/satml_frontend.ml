...
module Make (Th : Theory.S) : Sat_solver_sig.S = struct
  ...
  let empty_guards () =
    { current_guard = Expr.vrai; stack_guard = Stack.create () }
  ...
  module Debug = struct
    ...
    let assume gf =
      ...
      then begin
        ...
        | E.Lemma _ ->
          print_dbg ~module_name:"Satml_frontend" ~function_name:"assume"
            "I assume a [%d-atom] lemma: %a" (E.size f) E.print f
        | E.Literal a ->
          let n =
            match lem with
            | None -> ""
    ...
    let simplified_form f f' =
      ...
      then
        print_dbg ~module_name:"Satml_frontend" ~function_name:"simplified_form"
          "@[<v 2>Simplified form of: %a@,is: %a@]" E.print f FF.print f'

    (* unused -- let cnf_form f unit non_unit = if get_debug_sat () &&
       get_verbose () then begin print_dbg "[sat] CFF form of: %a" FF.print f;
       print_dbg " is:"; List.iter (List.iter (fun a -> print_dbg "UNIT: %a"
       Atom.pr_atom a)) unit; List.iter (fun c -> print_dbg "CLAUSE: ";
       List.iter (fun a -> print_dbg "%a or " Atom.pr_atom a) c; )non_unit
       end *)
    ...
    let model fmt env =
      ...
      then
        ...
        let print fmt a =
          Format.fprintf fmt " %f | %a@," (Atom.weight a) Atom.pr_atom a
        in
        Format.fprintf fmt "@[<v 2>(2) satML's model:@,%a@]"
    ...
    (* unused -- let generated_instances l = if get_verbose () && get_debug_sat
       () then begin print_dbg "[new_instances] %d generated" (List.length l);
       List.iter (fun { E.ff = f; origin_name; _ } -> print_dbg "
       instance(origin = %s): %a" origin_name E.print f; ) l end *)

    (* unused -- let trivial_fact p inst = if get_verbose () && get_debug_sat ()
       then begin if inst then print_dbg "already known instance: %a" E.print p
       else print_dbg "already known skolem: %a" E.print p end *)

    (* unused -- let generated_skolems l = if get_verbose () && get_debug_sat ()
       then begin print_dbg "[new_skolems] %d generated" (List.length l);
       List.iter (fun { E.ff = f; _ } -> print_dbg " skolem: %a" E.print f) l
       end *)
    ...
    let add_terms_of src terms =
      ...
      then begin
        ...
        SE.iter
          (fun e -> print_dbg ~flushed:false ~header:false ">> %a@," E.print e)
          terms;
        print_dbg ~header:false "@]"
      end

    (* unused -- let axiom_def f = if get_debug_sat () then print_dbg (asprintf
       "[sat] I assume an axiom: %a" E.print f) *)
    ...
    let internal_axiom_def f a at =
      ...
      then
        print_dbg ~module_name:"Satml_frontend"
          ...
          "I assume an internal axiom: %a <-> %a@,at of a is %a" E.print a
          E.print f Atom.pr_atom at

    (* unused -- let in_mk_theories_instances () = if Options.get_debug_fpa() >
       0 || get_debug_sat() then print_dbg "[sat] entering
       mk_theories_instances:" *)

    (* unused -- let out_mk_theories_instances normal_exit = if
       Options.get_debug_fpa() > 0 || get_debug_sat() then if normal_exit then
       print_dbg "[sat] normal exit of mk_theories_instances." else print_dbg
       "exit mk_theories_instances with Inconsistency." *)
  ...
  (* if get_debug_sat () then fprintf fmt "make_explanation of %d clauses@."
     (List.length lc); List.fold_left (fun ex ({ST.form = f} as c) -> if
     get_debug_sat () then fprintf fmt "unsat_core: %a@." Atom.pr_clause c;
     Ex.union (Ex.singleton (Ex.Dep f)) ex )Ex.empty lc*)
    ...
    List.fold_left
      (fun acc (gf, dep) ->
        ...
        | [] ->
          ...
          gf :: acc
        | [{ Atom.lit; _ }] -> (
          (* Instantiations from [internal_axiom_def] are justified by a single
             syntaxic literal (from [axs_of_abstr]) *)
          match Shostak.Literal.view lit with
          | LTerm lit ->
            { gf with E.ff = E.mk_or gf.E.ff (E.neg lit) false } :: acc
          | LSem _ -> assert false)
        | _ -> assert false)
      acc l
  ...
  let axiom_def env gf ex = env.inst <- Inst.add_lemma env.inst gf ex
  ...
  let register_abstraction env new_abstr_vars (f, (af, at)) =
    if Options.(get_debug_sat () && get_verbose ())
    then
      Printer.print_dbg ~module_name:"Satml_frontend"
        ~function_name:"register_abstraction" "abstraction of %a is %a" E.print
        f FF.print af;
    let lat =
    ...
    in
    if Atom.level at = 0
    then (* at is necessarily assigned if lvl = 0 *)
      if Atom.is_true at
      then
      ...
      let ded =
        match E.neg f |> E.form_view with
        | E.Skolem q -> E.skolemize q
        | E.Unit _ | E.Clause _ | E.Literal _ | E.Lemma _ | E.Let _ | E.Iff _
        | E.Xor _ ->
          assert false
      in
  ...
  let expand_skolems env acc sa inst_quantif =
    List.fold_left
      (fun acc a ->
        ...
        then
          Printer.print_dbg ~module_name:"Satml_frontend"
            ~function_name:"expand_skolems" "expand skolem of %a" E.print a;
        try
          if inst_quantif a
          then
            let ({ E.ff = f; _ } as gf) = ME.find a env.skolems in
            if (not (Options.get_cdcl_tableaux ())) && ME.mem f env.gamma
            then acc
            else gf :: acc
          else acc
        with Not_found -> acc)
  ...
  let inst_env_from_atoms env acc sa inst_quantif =
    List.fold_left
      (fun (inst, acc) a ->
        ...
        then
          Printer.print_dbg ~module_name:"Satml_frontend"
            ~function_name:"inst_env_from_atoms" "terms_of_atom %a" E.print a;
        ...
        try
          ...
          if inst_quantif a
          then internal_axiom_def ax a at inst, acc
          else inst, acc
        with Not_found -> inst, acc)
      (env.inst, acc) sa

  (* unused -- let take_max aux l = let ((lvl, _, ind) ,_) as acc =
     List.fold_left (fun ((mz,_) as acc) f -> match aux f with | None -> acc |
     Some (m, l) -> if cmp_tuples m mz > 0 then (m, l) else acc )((-1, -.1.,
     -1), []) l in if lvl = -1 && ind = -1 then None else Some acc *)

  (* unused -- let take_min aux l = let ((lvl, _, ind) ,_) as acc =
     List.fold_left (fun ((mz,_) as acc) f -> match aux f with | None -> acc |
     Some (m, l) -> if cmp_tuples m mz < 0 then (m, l) else acc )((max_int,
     -.1., max_int), []) l in if lvl = max_int && ind = max_int then None else
     Some acc *)
  ...
  let atoms_from_lazy_sat =
    let rec add_reasons_graph _todo _done =
      ...
      | a :: _todo ->
        if SA.mem a _done
        then add_reasons_graph _todo _done
        else
          let _todo =
            List.fold_left
  ...
  let atoms_from_lazy_greedy env =
    let aux accu ff =
      let sf =
        ...
        with Not_found ->
          if FF.equal ff FF.vrai
          then SE.empty
          else begin
            Printer.print_err "%a not found in env.conj" FF.print ff;
            assert false
          end
      in
  ...
  let atoms_from_bmodel env =
    ME.fold
      (fun f _ sa -> (E.atoms_rec_of_form ~only_ground:false) f sa)
      env.gamma SE.empty

  let instantiation_context env ~greedy_round ~frugal =
    let sa =
      match greedy_round, Options.get_cdcl_tableaux_inst () with
  ...
  let instantiate_ground_preds env acc sa =
    List.fold_left
      (fun acc a ->
        ...
        | Some (guard, res, _dep) ->
          (* To be correct in incremental mode, we'll generate the formula
             "guard -> (a -> res)" *)
  ...
  let new_instances use_cs tenv env sa inst_quantif acc =
    ...
    mround use_cs tenv { env with inst } acc
  ...
  let pre_assume env acc gf =
    ...
    then
      Printer.print_dbg ~module_name:"Satml_frontend"
        ~function_name:"pre_assume" "Entry of pre_assume: Given %a" E.print f;
    if SE.mem f acc.seen_f
    then acc
    else
      ...
      try
        ...
        match ff with
        | None -> acc [@ocaml.ppwarning "TODO: should be assert failure?"]
        | Some ff ->
          if SAT.exists_in_lazy_cnf env.satml ff
          then acc
          else
            { acc with activate = FF.Set.add ff acc.activate; updated = true }
      with Not_found -> (
        Debug.assume gf;
        match E.form_view f with
        | E.Lemma _ ->
        ...
        | E.Xor _ ->
          ...
          if FF.equal ff FF.vrai
          then acc
          else if cnf_is_in_cdcl
          then
            (* this means that there exists another E.t that is equivalent to f.
               These two formulas have the same ff *)
            if SAT.exists_in_lazy_cnf env.satml ff
            then acc
            else
              { acc with activate = FF.Set.add ff acc.activate; updated = true }
  ...
  let cdcl_assume env pending ~dec_lvl =
    ...
    (* fprintf fmt "pending : %d distinct forms@." (SE.cardinal seen_f); fprintf
       fmt "pending : %d to activate@." (SFF.cardinal activate); fprintf fmt
       "pending : %d new vars@." (List.length new_vars); fprintf fmt "pending :
       %d unit cnf@." (List.length unit); fprintf fmt "pending : %d non-unit
       cnf@." (List.length nunit); fprintf fmt "pending : updated = %b@."
       updated; *)
    if SE.is_empty seen_f
    then begin
  ...
  let rec assume_aux ~dec_lvl env l =
    ...
    let elit a =
      ...
      | LTerm a -> a
      | LSem _ ->
        (* This is only called on newly added skolems, which are always syntaxic
           literals *)
        assert false
    in
    let bot_abstr_vars =
  ...
  let instantiation env inst_strat dec_lvl =
    ...
    | Force_normal ->
      ...
      let tconf = frugal_tconf () in
      (* take frugal_tconf if normal is forced *)
      env.last_forced_normal <- nb_mrounds;
      let sa, inst_quantif =
        instantiation_context env ~greedy_round:false ~frugal:false
      in
      do_instantiation env sa inst_quantif mconf tconf "normal-inst (forced)"
        ~dec_lvl
    | Force_greedy ->
      let mconf = normal_mconf () in
      let tconf = normal_tconf () in
      (* take normal_tconf if greedy is forced *)
      env.last_forced_greedy <- nb_mrounds;
      let sa, inst_quantif =
        instantiation_context env ~greedy_round:true ~frugal:true
      in
      do_instantiation env sa inst_quantif mconf tconf "greedy-inst (forced)"
        ~dec_lvl
    | Auto ->
      List.fold_left
  ...
  (* Getting [unknown] after a query can mean two things: - The problem is
     [unsat] but we didn't manage to find a contradiction; - We produce a model
     of the formulas.
     ...
     For instance, consider the SMT-LIB problem: (declare-const x Int) (assert
     (or (<= x 2) (<= x 4))) (assert (or (<= y 3)) (maximize x) (maximize y)
     (check-sat) (get-model)
     ...
     best model for [y] it can got in this branch. Still we have to explore the
     second branch [(<= x 4)] to realize that [4] is actually the best model for
     [x].
     ...
     In our running example, after getting the model [2], the below function run
     again the SAT solver with the extra assert: (assert (or (> x 2) (and (= x
     2) (> y 3)))
  ...
  let op ty is_max is_le =
    ...
    if is_max
    then begin
      if is_le then mk_le ty_op else mk_lt ty_op
    end
    else begin
      if is_le then mk_ge ty_op else mk_gt ty_op
    end
  ...
  let analyze_unknown_for_objectives env unsat_rec_prem : unit =
    ...
    let acc =
      try
        Objective.Model.fold
          (fun { e; is_max; _ } value acc ->
            match (value : Objective.Value.t) with
            | Pinfinity | Minfinity -> raise (Give_up acc)
            | Value v -> (e, v, is_max, true) :: acc
            | Limit (_, v) -> raise (Give_up ((e, v, is_max, false) :: acc))
    ...
    | [] ->
      (* The answer for the first objective is infinity. We stop here as we
         cannot go beyond infinity and the next objectives with lower priority
         cannot be optimized in presence of infinity values. *)
      raise I_dont_know
    | (e, tv, is_max, is_le) :: l ->
      let neg =
        List.fold_left
          ...
          (E.Core.not ((op (E.type_info e) is_max is_le) e tv))
          l
      in
      if Options.get_debug_optimize ()
      then
        Printer.print_dbg
          "The objective function %a has an optimum. We should continue to \
           explore other branches to try to find a better optimum than %a."
      ...
      if Options.get_debug_optimize ()
      then
        Printer.print_dbg "We assert the formula %a to explore another branch."
          E.print neg;
  ...
  let rec unsat_rec env ~first_call:_ : unit =
    ...
    | Satml.Sat -> (
      try
        ...
        let strat =
          if env.nb_mrounds - env.last_forced_greedy > 1000
          then Force_greedy
          else if env.nb_mrounds - env.last_forced_normal > 50
          then Force_normal
          else Auto
        in
        (*let strat = Auto in*)
        ...
        let updated =
          if (not updated) && strat != Auto
          then instantiation env Auto dec_lvl
          else updated
        in
        let dec_lvl' = SAT.decision_level env.satml in
        let () =
          if strat == Auto && dec_lvl' = dec_lvl
          then
            (* increase chances of forcing Normal each time Auto instantiation
               doesn't allow to backjump *)
      ...
      | Util.Step_limit_reached n -> i_dont_know env (Step_limit n)
      | Satml.Unsat lc -> raise (IUnsat (env, make_explanation lc))
      | Ex.Inconsistent (expl, _cls) ->
        (*may be raised during matching or CS*)
        begin try
  ...
  let declare env id = env.declare_top <- id :: env.declare_top
  ...
  let push env to_push =
    Util.loop
      ~f:(fun _n () () ->
        ...
        with Util.Step_limit_reached _ ->
          (* This function should be called without step limit (see
             Steps.apply_without_step_limit) *)
          assert false)
      ~max:to_push ~elt:() ~init:()
  ...
  let assume env gf _dep =
    (* dep currently not used. No unsat-cores in satML yet *)
    assert (SAT.decision_level env.satml <= SAT.assertion_level env.satml);
    try ignore (assume_aux ~dec_lvl:0 env [add_guard env gf]) with
  ...
  let assume t ff dep =
    Timers.with_timer Timers.M_Sat Timers.F_assume @@ fun () -> assume t ff dep

  let unsat t ff =
    Timers.with_timer Timers.M_Sat Timers.F_unsat @@ fun () -> unsat t ff

  let assume_th_elt env th_elt dep = SAT.assume_th_elt env.satml th_elt dep
    ...
    | Ty.Tbool -> begin
      ...
      Compat.List.find_map
        (fun Atom.{ lit; neg = { lit = neglit; _ }; _ } ->
          ...
          if Shostak.Literal.equal tlit lit
          then Some E.vrai
          else if Shostak.Literal.equal tlit neglit
          then Some E.faux
          else None)
        bmodel
    end
...
(* (*+ no dynamic loading of SAT solvers anymore +*) let () =
   Sat_solver.set_current (module Main : Sat_solver_sig.S) *)
