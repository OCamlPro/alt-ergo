...
module Make (SAT : Sat_solver_sig.Internal) = struct
  open Sat_solver_sig
  include SAT

  let internal_assume env id e =
    SAT.assume env
      {Expr.ff= e;
       origin_name = id;
       gdist = -1;
      ...
      (fun ({ hs; _ } as name, _, ret_ty) graph ->
         ModelMap.Graph.iter
           (fun val_args val_ret ->
              ...
              internal_assume env (Hstring.view hs) eq
           )
           graph
      )
      mdl.Models.model;
  ...
  (* [get_value_in_boolean_model env e] retrieves the assignment of the boolean
     ...
     @return [None] if the formula [e] hasn't been assigned by the SAT
             solver. *)
  let get_value_in_boolean_model env e =
  ...
  let get_value_in_model env mdl e =
  ...
  let get_value env l =
    ...
    let res =
      List.map
        (fun e ->
           match get_value_in_model env mdl e with
           | Some v -> Some v, None
           | None ->
    ...
    let values =
      List.map
        (fun (v, name) ->
           ...
           | Some v, None -> v
           | None, Some name ->
             begin match get_value_in_model env mdl name with
               | Some v -> v
               | None ->
  ...
  let get_assignment env =
    List.map
      (fun e ->
         match get_value_in_boolean_model env e with
         | Some true -> Sat_solver_sig.True
         | Some false -> Sat_solver_sig.False
         | None -> Sat_solver_sig.Unknown
      )
end
