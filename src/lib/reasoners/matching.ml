...
module Triggers = struct
  type t = { pats : Matching_types.trigger_info list } [@@unboxed]

  let empty = { pats = [] }

  let add p triggers = { pats = p :: triggers.pats }
    ...
    let triggers_of q mconf =
      ...
      | [] -> (
        ...
        with Not_found ->
          ...
          trs)
    in
    let clear_triggers_of_trs_tbl () = HEI.clear trs_tbl in
  ...
  let backward_triggers, clear_backward_triggers_trs_tbl =
    ...
    let backward_triggers q =
      ...
      with Not_found ->
        ...
        trs
    in
    let clear_backward_triggers_trs_tbl () = HE.clear trs_tbl in
  ...
  let forward_triggers, clear_forward_triggers_trs_tbl =
    ...
    let forward_triggers q =
      ...
      with Not_found ->
        ...
        trs
    in
    let clear_forward_triggers_trs_tbl () = HE.clear trs_tbl in
    ...
    ME.fold
      (fun lem (guard, age, dep) env ->
        ...
        | E.Lemma ({ E.main = f; name; _ } as q) ->
          ...
          if Options.get_debug_triggers ()
          then
            Printer.print_dbg ~module_name:"Matching"
              ~function_name:"add_triggers"
              "@[<v 2>%s triggers of %s are:@ %a@]" kind name E.print_triggers
              tgs;
          List.fold_left
            (fun triggers tr ->
              let info =
  ...
  let add_triggers_of_formulas tconf env formulas =
    add_triggers_of_formulas0 env formulas
      (fun q -> triggers_of q tconf)
      "Normal"
...
module Make (X : Arg) : S with type theory = X.t = struct
  ...
  let empty = { fils = Symbols.Map.empty; info = ME.empty; max_t_depth = 0 }

  let make ~max_t_depth info fils = { fils; info; max_t_depth }
  ...
  module Debug = struct
    ...
    let add_term t =
      if Options.get_debug_matching () >= 3
      then
    ...
    let match_pats_modulo pat lsubsts =
      ...
      then
        let print fmt Matching_types.{ sbs; sty; _ } =
          Format.fprintf fmt ">>> sbs= %a | sty= %a@ " (SubstE.pp E.print) sbs
            Ty.print_subst sty
        in
        print_dbg ~module_name:"Matching" ~function_name:"match_pats_modulo"
          "@[<v 2>match_pat_modulo: %a  with accumulated substs@ %a@]" E.print
          pat (pp_list_no_space print) lsubsts

    let match_one_pat Matching_types.{ sbs; sty; _ } pat0 =
      if Options.get_debug_matching () >= 3
      then
        print_dbg ~module_name:"Matching" ~function_name:"match_one_pat"
          "match_pat: %a with subst: sbs= %a | sty= %a" E.print pat0
      ...
      then
        print_dbg ~module_name:"Matching" ~function_name:"match_one_pat_against"
          "@[<v 0>match_pat: %a against term %a@ with subst:  sbs= %a | sty= \
           %a@]"
          E.print pat0 E.print t (SubstE.pp E.print) sbs Ty.print_subst sty

    let match_term Matching_types.{ sbs; sty; _ } t pat =
      if Options.get_debug_matching () >= 3
      then
        print_dbg ~module_name:"Matching" ~function_name:"match_term"
          "I match %a against %a with subst: sbs=%a | sty= %a" E.print pat
          E.print t (SubstE.pp E.print) sbs Ty.print_subst sty

    let match_list Matching_types.{ sbs; sty; _ } pats xs =
      if Options.get_debug_matching () >= 3
      then
        print_dbg ~module_name:"Matching" ~function_name:"match_list"
          "I match %a against %a with subst: sbs=%a | sty= %a" E.print_list pats
          E.print_list xs (SubstE.pp E.print) sbs Ty.print_subst sty

    let match_class_of t cl =
      if Options.get_debug_matching () >= 3
      then
        Log.debug (fun k ->
            k "class of '%a' is:@  %a" E.print t
              Fmt.(box @@ braces @@ iter ~sep:comma E.Set.iter E.print)
              cl)

    let candidate_substitutions pat_info res =
      let open Matching_types in
      if Options.get_debug_matching () >= 1
      then
        print_dbg ~module_name:"Matching"
          ...
          (List.length res) E.print pat_info.trigger_orig E.print_list
          pat_info.trigger.E.content;
      if Options.get_debug_matching () >= 2
      then
        List.iter
          (fun gsbt ->
    ...
    let rec add_rec env t =
      ...
      then env
        ...
        let env =
          ...
          in
          (* - l'age d'un terme est le min entre l'age passe en argument et
             l'age dans la map - un terme est en lien avec le but de la PO
             seulement s'il ne peut etre produit autrement (d'ou le &&) - le
             lemme de provenance est le dernier lemme *)
          let g, b = infos min ( && ) t info.term_age info.term_from_goal env in
          let from_lems =
            List.fold_left
              (fun acc t ->
                try (ME.find t env.info).lem_orig @ acc with Not_found -> acc)
  ...
  let all_terms f ty env tbox
      ({ sbs = s_t;
        ...
        Matching_types.gsubst) lsbt_acc =
    let open Matching_types in
    Symbols.Map.fold
      (fun _ s l ->
        ME.fold
          (fun t _ l ->
            try
              ...
              let ng, but =
                try
                  let { age = ng; but = bt; _ } = ME.find t env.info in
                  max ng g, bt || b
                with Not_found -> g, b
              in
  ...
  module T2 = struct
    ...
    let compare (a, b) (x, y) =
      let c = E.compare a x in
      if c <> 0 then c else E.compare b y
  end
  ...
  (* These references are reset before and after each call to query. If some
     intermediate functions are exported in the future, the code should be
     adapted. *)
  ...
  let add_msymb tbox f t ({ sbs = s_t; _ } as sg : Matching_types.gsubst)
    ...
    then
      ...
      let t =
        if E.depth t > max_t_depth || Options.get_normalize_instances ()
        then X.term_repr tbox t ~init_term:true
        else t
      in
  ...
  let plus_of_minus t d ty = [E.mk_term (Symbols.Op Symbols.Minus) [t; d] ty; d]

  let minus_of_plus t d ty = [E.mk_term (Symbols.Op Symbols.Plus) [t; d] ty; d]

  let linear_arithmetic_matching f_pat pats _ty_pat t =
    let ty = E.type_info t in
    if (not (Options.get_arith_matching ())) || (ty != Ty.Tint && ty != Ty.Treal)
    then []
    else
      match f_pat, pats with
      | Symbols.Op Symbols.Plus, [p1; p2] ->
        if E.is_ground p2
        then [plus_of_minus t p2 ty]
        else if E.is_ground p1
        then [plus_of_minus t p1 ty]
        else []
      | Symbols.Op Symbols.Minus, [p1; p2] ->
        if E.is_ground p2
        then [minus_of_plus t p2 ty]
        else if E.is_ground p1
        then [minus_of_plus t p1 ty]
        else []
      | _ -> []

  let rec match_term ~use_ematching env tbox
      ({ sty = s_ty; gen = g; goal = b; _ } as sg : Matching_types.gsubst) pat t
      =
    Options.exec_thread_yield ();
    Debug.match_term sg t pat;
    let { E.f = f_pat; xs = pats; ty = ty_pat; _ } = E.term_view pat in
    match f_pat with
    | Symbols.Var v when Var.equal v Var.underscore -> begin
    ...
    | _ -> (
      try
        ...
        then [gsb]
          ...
          let cl =
            if not use_ematching then E.Set.singleton t else X.class_of tbox t
          in
  ...
  let match_one_pat ~use_ematching env tbox pat0 lsbt_acc sg =
    ...
    let { E.f; xs = pats; ty; _ } = E.term_view pat in
    match f with
  ...
  let matching ~use_ematching env tbox pat_info =
    ...
    Debug.matching pats;
    if List.length pats_list > Options.get_max_multi_triggers_size ()
    then pat_info, []
    else
      let egs =
        { sbs = SubstE.empty;
      ...
      | _ -> (
        ...
        try
          ...
          pat_info, res
        with Exit ->
          if Options.(get_debug_matching () >= 1 && get_verbose ())
          then
            Printer.print_dbg ~module_name:"Matching" ~function_name:"matching"
              "skip matching for %a : cpt = %d" E.print pat_info.trigger_orig
              !cpt;
          pat_info, [])
  ...
  (* unused -- let fully_uninterpreted_head s = match E.term_view s with |
     E.Not_a_term _ -> assert false | E.Term { E.f = Symbols.Op _; _ } -> false
     | _ -> true

     (* this function removes "big triggers" that are subsumed by smaller ones
     *) let filter_subsumed_triggers triggers = List.fold_left (fun acc tr ->
     match tr.E.content with | [t] -> let subterms = E.sub_terms E.Set.empty t
     in if List.exists (fun tr -> match tr.E.content with | [s] -> not (E.equal
     s t) && E.Set.mem s subterms && fully_uninterpreted_head s | _ -> false
     )triggers then acc else tr :: acc | _ -> tr :: acc )[] triggers |>
     List.rev *)

  let terms_info env = env.info, env.fils

  let reinit_caches () = reset_cache_refs ()
end
