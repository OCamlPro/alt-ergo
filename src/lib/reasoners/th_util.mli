...
type answer =
  ...
  | Unknown
      (** The theory could not prove that the literal is entailed by the current
          ...
          - The negation of the literal is entailed by the current environment,
            or
          - The truth value of the literal is independent of the current
            environment (it is true in some models and false in other models),
            or
...
type lit_origin =
  | Subst
      (** Only equalities can be {!Subst} literals, and they are oriented: the
          left-hand side is always an uninterpreted term or AC symbol
          application. Effectively, {!Subst} literals are the substitutions
          generated by calls to [X.solve] and propagated through the CC(X) and
          AC(X) algorithms.

          In practice, a {!Subst} equality [r = rr] is generated when the
          corresponding substitution is performed by CC(X), i.e. when [rr]
  ...
  | CS of theory * Numbers.Q.t
      (** Literals of {!CS} origin come from the case splits performed by a
          specific theory. Usually, they are equalities of the shape [x = v]
          where [x] is an uninterpreted term and [v] a value; however, this is
          not necessarily the case (e.g. {!CS} literals from the theory of
          arrays are often disequalities).

          Depending on the theory, the shape of {!CS} literals can be
          restricted. In particular, {!CS} literals of the {!Th_UF} theory:
          those come from model generation in the union-find, and are
          assignments, i.e. equalities [x = v] where [x] is uninterpreted and
          [v] is a value.
  ...
  | NCS of theory * Numbers.Q.t
      (** Literals of {!NCS} origin are created from a literal of {!CS} origin
          when the choice made in a case split turns out to be unsatisfiable.
          The literal is a negation of a {!CS} literal that was built by the
          corresponding theory, with the restrictions that this implies. *)
  | Other
      (** Literals of {!Other} are those that are not covered by any of the
          cases described above. In particular, user assertions, SAT decisions,
          SAT propagations and theory propagations all have the {!Other} origin.
      *)
...
(** A case split is a triple [(a, is_cs, origin)].
    ...
    *unit fact*, i.e. a fact that is true in all possible environments, not
    ...
    returned with [is_cs = true]; but if the [is_cs] flag is [false], the solver
    *will* assume that the literal can't take part in any conflict. Returning
    [is_cs = false] if the literal is not an unit fact **is unsound**.
  ...
  { value : Objective.Value.t;
    case_split : case_split
        (** The underlying case-split. Notice that the value propagate by this
            ...
            is unbounded or some objective functions involved strict
            inequalities. *)
  }
...
type 'literal acts =
  { acts_add_decision_lit : 'literal -> unit;
        (** Ask the SAT solver to decide on the given formula before it can
            answer [SAT]. The order of decisions among multiple calls of
    ...
    acts_add_split : 'literal -> unit;
        (** Let the SAT solver know about a case split. The SAT solver should
            decide on the provided formula before answering [Sat], although it
            is not required to do so. The order of decisions among multiple
            calls of [acts_add_split] is unspecified, and the solver is allowed
    ...
    acts_add_objective :
      Objective.Function.t -> Objective.Value.t -> 'literal -> unit
        (** Ask the SAT solver to optimistically select the appropriate value
            for the given objective function (encoded as a decision in the
            ['literal]). If the solver backtracks on that decision, the theory
            will have an opportunity to select another value in a context where
            the ['literal] is negated.

            In case multiple objectives are added before the solver gets to make
            a decision, only the *last* objective is taken into consideration;
            you cannot assume that the objective has been optimized until the
            objective is sent back to the theory through [add_objective].
...
(** The type of actions that a theory can take.
    ...
    [1] : https://github.com/Gbury/mSAT/blob/ \
