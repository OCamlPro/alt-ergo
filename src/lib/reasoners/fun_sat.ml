...
module Make (Th : Theory.S) = struct
  ...
  module Heuristics = struct
    ...
    let empty () = { mp = ME.empty; var_inc = 1.; var_decay = 1. /. 0.95 }
    ...
    let bump_activity ({ mp; var_inc; _ } as env) expl =
      ...
      in
      { env with mp; var_inc = var_inc *. env.var_decay }

    let choose delta env =
      let dec, no_dec =
        if Options.get_no_decisions_on_is_empty ()
        then delta, []
        else
          List.partition
            (fun (a, _, _, _) -> Options.get_can_decide_on a.E.origin_name)
            delta
      in
      let dec =
        List.rev_map
      ...
      let dec =
        List.fast_sort
          (fun (_, x1, b1) (_, x2, b2) ->
            ...
            if c <> 0 then c else Float.compare x2 x1)
          dec
      in
  ...
  type refs = { unit_facts : (E.gformula * Ex.t) ME.t }
  ...
  type t =
    { (* The field gamma contains the current Boolean model (true formulas) of
         the SAT. Each assumed formula is mapped to a tuple (gf, ex, dlvl,
         plvl), where: - gf is the rich form of the formula - ex is the
         explanation associated to the formula - dlvl is the decision level
         where the formula was assumed to true - plvl is the propagation level
         (w.r.t. dlvl) of the formula. It forms with dlvl a total ordering on
  ...
  let restore_guards_and_refs env =
    ...
    Steps.pop_steps ();
    { env with unit_facts_cache = ref refs.unit_facts }, guard
  ...
  module Debug = struct
    ...
    let print_nb_related env =
      ...
      then
        print_dbg ~module_name:"Fun_sat" ~function_name:"print_nb_related"
          "@[<v 0>----------------------------------------------------@ \
           nb_related_to_both = %d@ nb_related_to_goal = %d@ \
           nb_related_to_hypo = %d@ nb_unrelated       = %d@ \
           ----------------------------------------------------@]"
          env.nb_related_to_both env.nb_related_to_goal env.nb_related_to_hypo
          env.nb_unrelated
    ...
    let propagations (env, bcp, tcp, ap_delta, lits) =
      ...
      then
        print_dbg ~module_name:"Fun_sat" ~function_name:"propagations"
          "|lits| = %d, B = %b, T = %b, |Delta| = %d, |ap_Delta| = %d"
          (List.length lits) bcp tcp (List.length env.delta)
    ...
    let is_it_unsat gf =
      let s =
        ...
        | E.Let _ -> "let"
      in
      if Options.get_verbose () && Options.get_debug_sat ()
      then
        print_dbg ~module_name:"Fun_sat" ~function_name:"is_it_unsat"
          "the following %s is unsat ? :@ %a" s E.print gf.E.ff

    let pred_def f =
      if Options.get_debug_sat ()
      then
        print_dbg ~module_name:"Fun_sat" ~function_name:"pred_def"
          "I assume a predicate: %a" E.print f

    let unsat_rec dep =
      if Options.get_debug_sat ()
      then
        print_dbg ~module_name:"Fun_sat" ~function_name:"unsat_rec"
    ...
    let assume gf dep env =
      ...
      then (
        ...
        print_dbg ~flushed:false ~module_name:"Fun_sat" ~function_name:"assume"
        ...
        | E.Literal a ->
          let n =
            ...
            | Some ff -> begin
              ...
              | E.Unit _ | E.Clause _ | E.Literal _ | E.Skolem _ | E.Let _
              | E.Iff _ | E.Xor _ ->
                ""
            end
          in
          print_dbg ~header:false "LITERAL (%s : %a) %a@ " n E.print_list terms
            E.print a
        | E.Unit _ -> print_dbg ~header:false "conjunction"
        | E.Clause _ -> print_dbg ~header:false "clause %a" E.print f
        | E.Lemma _ ->
          print_dbg ~header:false "%d-atom lemma \"%a\"" (E.size f) E.print f
        | E.Skolem _ -> print_dbg ~header:false "skolem %a" E.print f
        | E.Let _ -> print_dbg ~header:false "let-in %a" E.print f
        | E.Iff _ -> print_dbg ~header:false "equivalence %a" E.print f
        | E.Xor _ -> print_dbg ~header:false "neg-equivalence/xor %a" E.print f
        end;
        if Options.get_verbose ()
        then
          print_dbg ~header:false "with explanations : %a" Explanation.print dep)

    let unsat () =
      if Options.get_debug_sat ()
      then print_dbg ~module_name:"Fun_sat" ~function_name:"unsat" "unsat"

    let decide f env =
      if Options.get_debug_sat ()
      then
        print_dbg ~module_name:"Fun_sat" ~function_name:"decide"
          "I decide: at level (%d, %d), on %a" env.dlevel env.plevel E.print f

    let instantiate env =
      if Options.get_debug_sat ()
      then
        print_dbg ~module_name:"Fun_sat" ~function_name:"instanciate"
          "I instantiate at level (%d, %d). Inst level = %d" env.dlevel
          env.plevel env.ilevel

    let backtracking f env =
      if Options.get_debug_sat ()
      then
        print_dbg ~module_name:"Fun_sat" ~function_name:"backtracking"
          "backtrack: at level (%d, %d), and assume not %a" env.dlevel
          env.plevel E.print f

    let backjumping f env =
      if Options.get_debug_sat ()
      then
        print_dbg ~module_name:"Fun_sat" ~function_name:"backjumping"
          "backjump: at level (%d, %d), I ignore the case %a" env.dlevel
          env.plevel E.print f

    let elim _ _ =
      if Options.(get_debug_sat () && get_verbose ())
      then print_dbg ~module_name:"Fun_sat" ~function_name:"elim" "elim"

    let red _ _ =
      if Options.(get_debug_sat () && get_verbose ())
      then print_dbg ~module_name:"Fun_sat" ~function_name:"red" "red"

    (* let delta d = print_dbg ~debug:(get_verbose () || get_debug_sat ())
       "[sat] - Delta ---------------------"; List.iter (fun (f1, f2, ex) ->
       print_dbg ~debug:(get_verbose () || get_debug_sat ()) "(%a or %a), %a"
       E.print f1.E.ff E.print f2.E.ff Ex.print ex) d; print_dbg
       ~debug:(get_verbose () || get_debug_sat ()) "[sat] ---------------------
       Delta -" *)
    ...
    let gamma g =
      ...
      then begin
        print_dbg ~module_name:"Fun_sat" ~function_name:"gamma"
          "@[<v 0>--- GAMMA ---------------------@ ";
        ME.iter
          (fun f (_, ex, dlvl, plvl) ->
            print_dbg ~header:false "(%d, %d) %a \t->\t%a@ " dlvl plvl E.print f
              Ex.print ex)
          g;
        print_dbg ~header:false " - / GAMMA ---------------------@]"
      end

    let bottom classes =
      if Options.get_bottom_classes ()
      then print_dbg "bottom:%a@?" E.print_tagged_classes classes

    let inconsistent expl env =
      if Options.get_debug_sat ()
      then
        print_dbg ~module_name:"Fun_sat" ~function_name:"inconsistent"
          "inconsistent at level (%d, %d), reason : %a" env.dlevel env.plevel
          Ex.print expl

    let in_mk_theories_instances () =
      if Options.(get_debug_fpa () > 0 || get_debug_sat ())
      then
        print_dbg ~module_name:"Fun_sat"
          ~function_name:"in_mk_theories_instances"
    ...
    let out_mk_theories_instances normal_exit =
      ...
      then
        print_dbg ~module_name:"Fun_sat"
          ~function_name:"out_mk_theories_instances"
          (if normal_exit
           then "normal exit of mk_theories_instances."
           else "exit mk_theories_instances with Inconsist.")
    ...
    let print_theory_instance hyp gf =
      if Options.get_debug_fpa () > 1 || Options.get_debug_sat ()
      then
        print_dbg ~module_name:"Fun_sat" ~function_name:"print_theory_instances"
          "@[<v 2>@ %s >@ hypotheses: %a@ conclusion: %a@]"
          (E.name_of_lemma_opt gf.E.lem)
          print_f_conj hyp E.print gf.E.ff
  end
  (*BISECT-IGNORE-END*)
  ...
  let do_case_split env origin =
    try
      if Options.get_debug_sat ()
      then
        Printer.print_dbg ~module_name:"Fun_sat" ~function_name:"do_case_split"
          "performing case-split";
      let tbox, new_terms = Th.do_case_split env.tbox origin in
      let inst =
        Inst.add_terms env.inst new_terms (mk_gf E.vrai "" false false)
      in
      { env with tbox; inst }
    with Ex.Inconsistent (expl, classes) ->
      Debug.inconsistent expl env;
  ...
  let update_unit_facts env ff dep =
    let f = ff.E.ff in
    if Options.get_sat_learning () && not (ME.mem f !(env.unit_facts_cache))
    then begin
  ...
  let learn_clause ({ gamma; _ } as env) ff0 dep =
    ...
    then
        ...
        Ex.fold_atoms
          (fun e (l, ex) ->
            ...
            | Ex.Bj f ->
              let d =
                try
                  let _, _, d, _ = ME.find f gamma in
                  d
      ...
      let f =
        List.fold_left (fun acc (f, _) -> E.mk_or f acc false) ff0.E.ff fl
      in
    ...
    with Not_found -> (
      ...
      with Not_found -> (
        ...
        | Entailed { ex; _ } as y ->
          ...
          tcp_cache := ME.add a y !tcp_cache;
          y))
  ...
  let th_elim tcp_cache tmp_cache ff env =
    ...
    | E.Literal a ->
      ...
      in
      ans
    | E.Unit _ | E.Clause _ | E.Lemma _ | E.Skolem _ | E.Let _ | E.Iff _
    | E.Xor _ ->
      Unknown
  ...
  let add_dep f dep =
    match E.form_view f with
    | E.Literal _ ->
      if (not (Options.get_unsat_core ())) || Ex.mem (Ex.Bj f) dep
      then dep
      else Ex.union (Ex.singleton (Ex.Dep f)) dep
    | E.Clause _ ->
      if Options.get_unsat_core ()
      then Ex.union (Ex.singleton (Ex.Dep f)) dep
      else dep
  ...
  let rec add_dep_of_formula f dep =
    ...
    | E.Lemma _ | E.Clause _ | E.Literal _ | E.Skolem _ | E.Let _ | E.Iff _
    | E.Xor _ ->
      dep

  (* currently: => this is not done modulo theories => unit_facts_cache not
     taken into account *)
  let update_distances =
    ...
    fun env gf red ->
      let nf = E.neg red in
      try
        let ff, _ = ME.find nf !(env.unit_facts_cache) in
        aux gf ff
      with Not_found -> (
        try
          let ff, _, _, _ = ME.find nf env.gamma in
          aux gf ff
        with Not_found -> gf)
      ...
      (fun (cl, u)
        ...
        then cl, u
          ...
          try
            ...
            let u =
              match
                ( th_elim tcp_cache tmp_cache gf1 env,
                  th_elim tcp_cache tmp_cache gf2 env )
              with
              | Unknown, Unknown -> raise Exit
              | (Entailed _, _ | _, Entailed _) when gf1.E.theory_elim -> u
              | Entailed _, Entailed _ -> u
              (* eliminate if both are true ? why ? *)
          ...
          with Exit ->
            begin
              ...
              | (Entailed { ex = d1; _ }, b), (Unknown, _) ->
                ...
                let gf2 =
                  { gf2 with E.nb_reductions = gf2.E.nb_reductions + 1 }
                in
              ...
              | (Unknown, _), (Entailed { ex = d2; _ }, b) ->
                if Options.get_profiling () then Profiling.red b;
                let gf1 =
                  { gf1 with E.nb_reductions = gf1.E.nb_reductions + 1 }
                in
  ...
  let theory_assume env facts =
    ...
    then env
      ...
      let facts, ufacts, inst, mf, gf =
        List.fold_left
          (fun (facts, ufacts, inst, mf, gf) (a, ff, ex, dlvl, plvl) ->
            ...
            then begin
              Printer.print_err "%a is not ground" E.print a;
              assert false
            end;
            ...
            let ufacts =
              if Ex.has_no_bj ex
              then (alit, Th_util.Other, ex, dlvl, plvl) :: ufacts
              else ufacts
            in
            if not ff.E.mf
            then begin
              Printer.print_err "%a" E.print ff.E.ff;
              assert false
            end;
            let inst =
              if ff.E.mf
              then Inst.add_terms inst (E.max_terms_of_lit a) ff
              else inst
            in
            facts, ufacts, inst, mf || ff.E.mf, gf || ff.E.gf)
          ([], [], env.inst, false, false)
          facts
      in
      let utbox, _, _ =
        (* assume unit facts in the theory *)
        if ufacts != [] && env.dlevel > 0
        then (
          ...
          with Ex.Inconsistent (reason, _) as e ->
            ...
            then
              Printer.print_dbg ~module_name:"Fun_sat"
                ~function_name:"theory_assume" "solved by unit_tbox";
            raise e)
      ...
      { env with tbox; unit_tbox = utbox; inst }
  ...
  let propagations ((env, bcp, tcp, ap_delta, lits) as result) =
    ...
    let acc =
      if bcp
      then do_bcp env tcp tcp_cache tmp_cache env.delta ([], [])
    ...
    { env with delta; tcp_cache = !tcp_cache }, unit
  ...
  let update_nb_related t ff =
    ...
    | true, true ->
      (* update these three counter to simplify comparaisons in the rest of the
         module: both+1 imples goal+1 *)
      { t with
  ...
  let rec asm_aux acc list =
    List.fold_left
      (fun ((env, _bcp, tcp, ap_delta, lits) as acc)
           (({ E.ff = f; _ } as ff), dep) ->
        Options.exec_thread_yield ();
        let dep = add_dep f dep in
        let dep_gamma = add_dep_of_formula f dep in
        (* propagate all unit facts to cache *)
        if Options.get_sat_learning () && Ex.has_no_bj dep_gamma
        then update_unit_facts env ff dep_gamma;
        ...
        (try
           ...
           (* missing VSID, but we have regressions when it is activated
              env.heuristics := Heuristics.bump_activity !(env.heuristics)
              exx;*)
           raise (IUnsat (exx, Th.cl_extract env.tbox))
         with Not_found -> ());
        if ME.mem f env.gamma
        then begin
          ...
          let env =
            if ff.E.mf && Options.get_greedy ()
            then
              { env with
                inst =
                  Inst.add_terms env.inst (E.max_ground_terms_rec_of_form f) ff
              }
            else env
          in
          ...
          | E.Unit (f1, f2) ->
            ...
            let lst =
              [{ ff with E.ff = f1 }, dep; { ff with E.ff = f2 }, dep]
            in
          ...
          | E.Literal a ->
            ...
            begin match Inst.ground_pred_defn a env.inst with
            | Some (guard, af, adep) ->
              (* in fun-SAT, guards are either forced to TRUE, or
                 Inst.ground_preds is cleaned when the guard is propagated to
                 FALSE *)
  ...
  (* this function has an internal state used to store the latest generated
     instances. These instances are used to try to backjump as far as possible
     using simple "assume"s, ie without decision. The reason for this
     modification is that a set of instances may cause several conflict, and we
     don't always detect the one which makes us backjump better. *)
  let update_instances_cache, clear_instances_cache =
    ...
    let update_instances_cache l_opt =
      ...
      | Some l ->
        ...
        None
    in
    let clear_instances_cache () = last_cache := [] in
  ...
  let inst_and_assume mconf tconf env inst_function inst_env =
    ...
    (* do this to avoid loosing instances when a conflict is detected directly
       with some of these instances only, ie before assumign the others *)
    if Options.get_sat_learning ()
    then
      List.iter
        (fun (gf, dep) -> if Ex.has_no_bj dep then update_unit_facts env gf dep)
        l;
  ...
  let may_update_last_saved_model env =
    if not @@ Options.get_produce_models ()
    then env
    else begin
      try
  ...
  let model_gen_on_timeout env =
    ...
    if
      (not i)
      (* not asked to gen a model *)
      || !(env.model_gen_phase)
      ||
      (* we timeouted in model-gen-phase *)
      Float.equal ti 0. (* no time allocated for extra model search *)
    then i_dont_know env (Timeout ProofSearch)
    else begin
      (* Beware: models generated on timeout of ProofSearch phase may be
         incohrent wrt. the ground part of the pb (ie. if delta is not empty
         ? *)
      env.model_gen_phase := true;
      Options.Time.unset_timeout ();
      Options.Time.set_timeout ti;
      update_model_and_return_unknown env ~unknown_reason:(Timeout ProofSearch)
        ...
        (fun (dep, acc) f ->
          ...
          with Not_found -> (
            ...
            with Not_found -> (
              ...
              | E.Literal a -> begin
                match
                  query_of tcp_cache tmp_cache { gf with E.ff = f } a env
                with
                | Entailed { ex; _ } ->
                  Ex.union dep ex, ({ gf with E.ff = f }, ex) :: acc
                | Unknown ->
                  Printer.print_err "Bad inst! Hyp %a is not true!" E.print f;
                  assert false
              end
              | E.Unit _ | E.Clause _ | E.Lemma _ | E.Skolem _ | E.Let _
              | E.Iff _ | E.Xor _ ->
                Printer.print_err
  ...
  let greedy_mconf () =
    { Util.no_ematching = false; use_cs = true; backward = Util.Normal }
  ...
  let greedy_instantiation env =
    ...
    | INormal ->
      ...
      update_model_and_return_unknown env
  ...
  let normal_instantiation env try_greedy =
    ...
    then greedy_instantiation env
    else env

  (* should be merged with do_bcp/red/elim ? calls to debug hooks are missing *)
  let propagate_unit_facts_in_cache env =
    if Options.get_no_sat_learning ()
    then None
    else
      let cache = !(env.unit_facts_cache) in
      let in_cache f =
        try Some (snd (ME.find f cache)) with Not_found -> None
      in
      let prop, delt =
        List.fold_left
  ...
  and back_tracking env =
    try
      if env.delta == [] || Options.get_no_decisions ()
      then back_tracking (normal_instantiation env true)
      else
    ...
    with Util.Timeout -> model_gen_on_timeout env
  ...
  and make_one_decision env =
    ...
    if Options.get_profiling ()
    then Profiling.decision new_level a.E.origin_name;
    ...
    try
      ...
      (* in the section below, we try to backjump further with latest generated
         instances if any *)
      begin match update_instances_cache None with
      | None -> assert false
      | Some [] -> ()
      | Some l ->
  ...
  let rec backward_instantiation_rec env rnd max_rnd =
    ...
    then env
      ...
      if Options.(get_verbose () || get_debug_sat ())
      then
        Printer.print_dbg ~flushed:false ~module_name:"Fun_sat"
          ~function_name:"backward_instantiation_rec" "round %d / %d@ " rnd
          max_rnd;
      ...
      if Options.(get_verbose () || get_debug_sat ())
      then
        Printer.print_dbg ~header:false ~module_name:"Fun_sat"
          ~function_name:"backward_instantiation_rec"
          "backward: %d goal-related hyps (+%d)" nb2 (nb2 - nb1);
      if (not new_i1) && not new_i2
      then env
      else if nb2 > nb1
      then backward_instantiation_rec env (rnd + 1) max_rnd
      else if nc2 > nc1
      then backward_instantiation_rec env (rnd + 1) (max_rnd / 2)
      else env
  ...
  let backward_instantiation env deepest_term =
    ...
    try
      ...
      then
        Printer.print_dbg ~module_name:"Fun_sat"
          ~function_name:"backward_instantiation" "@[<v 0>%a@]"
      ...
      then
        Printer.print_dbg ~module_name:"Fun_sat"
          ~function_name:"backward_instantiation" "done (after %2.4f seconds)"
          (Options.Time.value ());
      env
    with IUnsat _ as e ->
      Options.set_no_ematching no_ematching;
      ...
      then
        Printer.print_dbg ~module_name:"Fun_sat"
          ~function_name:"backward_instantiation" "solved with backward!";
      raise e
  ...
  let push env to_push =
    Util.loop
      ~f:(fun _n () acc ->
        ...
        { acc with
          guards = { acc.guards with current_guard = new_guard; guards }
        })
    ...
    Util.loop
      ~f:(fun _n () acc ->
        ...
        let () =
          ...
          with Stack.Empty -> Errors.error (Run_error Stack_underflow)
        in
        { acc with
          inst;
          guards = { acc.guards with current_guard = new_current_guard; guards }
        })
  ...
  let unsat env gf =
    ...
    try
      let guards_to_assume =
        ME.fold
          (fun _g gf_guard_with_ex acc -> gf_guard_with_ex :: acc)
          env.guards.guards []
      in
      ...
      let env =
        { env with
          inst =
            (* add all the terms of the goal to matching env *)
            Inst.add_terms env.inst (E.max_ground_terms_rec_of_form gf.E.ff) gf
        }
      in
  ...
  let assume env fg dep =
    try assume env [add_guard env fg, dep] with
    | IUnsat (d, classes) ->
      Debug.bottom classes;
      raise (Unsat d)
    | Util.Timeout ->
      (* don't attempt to compute a model if timeout before calling unsat
         function *)
  ...
  let pred_def env f name dep _loc =
    ...
    { env with inst = Inst.add_predicate env.inst ~guard ~name gf dep }

  let unsat env fg =
    Timers.with_timer Timers.M_Sat Timers.F_unsat @@ fun () -> unsat env fg

  let assume env fg =
    Timers.with_timer Timers.M_Sat Timers.F_assume @@ fun () -> assume env fg
  ...
  let empty ?(selector = fun _ -> true) () =
    (* initialize some structures in SAT.empty. Otherwise, E.faux is never added
       as it is replaced with (not E.vrai) *)
    reset_refs ();
