  ...
  *)
  ...
  let enqueue_assigned env a =
    if get_debug_sat () then
      Printer.print_dbg "[satml] enqueue_assigned: %a@." pr_atom a;
  ...
  let enqueue env a lvl reason =
    ...
    a.var.reason <- reason;
    if get_debug_sat () then
      Printer.print_dbg "[satml] enqueue: %a@." pr_atom a;
    Vec.push env.trail a;
  ...
  let propagate env =
    ...
    while env.qhead < Vec.size env.trail do
      ...
      if get_debug_sat () then
        Printer.print_dbg "[satml] propagate atom %a@." pr_atom a;
  ...
  let add_clause env f ~cnumber atoms =
    ...
    try
      ...
      | [a]   ->
        ...
        assert (lvl <> 0);
        begin
          if not (get_minimal_bj ()) then cancel_until env 0
