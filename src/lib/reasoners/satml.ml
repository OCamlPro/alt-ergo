...
(** This module implements a CDCL solver, as well as some extensions based on a
    ...
    [Satml_types] module). It is described in chapter 4 of Albin Coquereau's PhD
    thesis (in French): https://pastel.hal.science/tel-02504894
    ...
    In addition, there is an early stopping mechanism for satisfiable problems:
    if all disjunctions have had one "side" successfully assigned to [true], the
    solver will not try to assign truth values to the remaining atoms ([b] and
    [c] in the example). This can lead to partial boolean models.
...
module type SAT_ML = sig
  ...
  val instantiation_context : t -> FF.hcons_env -> Satml_types.Atom.Set.t
...
module Vheap = Heap.MakeRanked (struct
  ...
  (* Note: comparison is flipped because we want maximum weight first and [Heap]
     is a min-heap. *)
  let compare (a : t) (b : t) = Float.compare b.weight a.weight
end)

let is_semantic (a : Atom.atom) =
  match Shostak.Literal.view a.lit with LTerm _ -> false | LSem _ -> true
...
module Make (Th : Theory.S) : SAT_ML with type th = Th.t = struct
  ...
  type t =
    { hcons_env : Atom.hcons_env;
      ...
      mutable nchoices : int;
          (** Number of semantic choices (case splits) that have been made.
              Semantic literals are not counted as assignments but are still
              part of the trail; we keep track of this number to properly
              compute the number of assignments to boolean literals in
              [nb_assigns]. *)
      nchoices_stack : int Vec.t;  (** Stack for [nchoices] values. *)
      (* Tete de la File des faits unitaires a propager. C'est un index vers le
         trail *)
      mutable qhead : int;
      (* Nombre des assignements top-level depuis la derniere execution de
         'simplify()' *)
      mutable simpDB_assigns : int;
      (* Nombre restant de propagations a faire avant la prochaine execution de
         'simplify()' *)
      ...
      restart_inc : float;
      (* limite initiale du nombre de clause apprises, vaut 1/3 des clauses
         originales par defaut *)
      learntsize_factor : float;
      (* multiplier learntsize_factor par cette valeur a chaque restart, vaut
         1.1 par defaut *)
      ...
      tatoms_queue : Atom.atom Queue.t;
          (** Queue of atoms that have been added to the [trail] through either
              decision or boolean propagation, but have not been otherwise
              processed yet (in particular, they have not been propagated to the
              theory).

              In pure CDCL mode, all the atoms in [tatoms_queue] are propagated
              in the theory.

              In CDCL-Tableaux with the [get_cdcl_tableaux_th ()] option, the
              [tatoms_queue] is only used for relevancy propagation (see
              [relevancy_propagation]); instead, the [th_tableaux] field is used
              to dictate theory propagations. *)
      th_tableaux : Atom.atom Queue.t;
          (** Queue of atoms to propagate to the theory when using CDCL-Tableaux
              ...
              The atoms in [th_tableaux] correspond to the asserted components
              of the [lazy_cnf] formulas. In particular, they always correspond
              to [UNIT] flat formulas that have been asserted (or selected in a
              disjunction).

              This ensures that atoms are propagated to the theory according to
      ...
      mutable proxies : FF.proxies;
          (** Map from flat formulas to the proxies that represent them.
              ...
              Note: the [Satml] module does not touch the [proxies] field
              itself; it only uses it to find the atom associated with a flat
              formula. Appropriate proxies that cover all the flat formulas to
              be encountered need to be pre-emptively given using
              [set_new_proxies], which is called from [Satml_frontend]. *)
      mutable lazy_cnf : (FF.t list MFF.t * FF.t) Matoms.t;
          (** Map from atoms [a] to pairs [parents, ff].

              The [lazy_cnf] field is only used by the CDCL-Tableaux solver. It
              is used in relevancy propagation (see [relevancy_propagation])
              when the [get_cdcl_tableaux_th ()] option is enabled, and to stop
              decisions as soon as the *relevant* parts of the assertions have
              been decided if the [get_cdcl_tableaux_inst ()] option is enabled.

              It satisfies the following invariants:

              - [ff] is the flat formula that the atom [a] represents (either
                directly if [ff] is an UNIT formula, or indirectly as a proxy --
                in which case, [a] is the proxy associated with [ff] in
                [proxies]).
              ...
              - The formulas in [parents] represent the disjunctions that have
                been asserted but not yet proven (i.e. none of their components
                is [true]).

              - The atoms that appear as keys in [lazy_cnf] are not [true] (they
                are either undecided or forced to [false]). Once an atom in
                [lazy_cnf] is forced to [true], it is removed from the
                [lazy_cnf] and its parents are removed as parents of their other
                children (one of their components is [true], so they no longer
                constrain the problem). The corresponding flat formula is then
                added again, which may introduce new entries to the [lazy_cnf]
                if it was itself a disjunction or contained disjunctions (see
                [relevancy_propagation]).

              When [lazy_cnf] is empty, all the disjunctions have been decided,
              and we have a (partial!) boolean model. If there are nested
              disjunctions (for instance, if [a \/ (b /\ (c \/ d))] is asserted,
              and we decide or propagate that [a] is true, [c] and [d] may stay
              undecided). *)
      lazy_cnf_queue : (FF.t list MFF.t * FF.t) Matoms.t Vec.t;
          (** Checkpoint of the [lazy_cnf] field at each decision level. Only
              used with the CDCL-Tableaux solver. *)
      irrelevants : unit VH.t;
          (** Variables that have been removed from the set of possible
              decisions due to no longer being relevant to the original formula
              (i.e. they are undecided but also no longer in the [lazy_cnf]).

              We maintain the following invariants:

              - Any *undecided* variable is either in the [order] heap, or in
                the [irrelevants] set
              - Any variable in the [irrelevants] set is not relevant (i.e.
                neither [v.na] nor [v.pa] is in the [lazy_cnf] -- this invariant
                is locally broken inside [try_to_backjump_further], but we don't
                make decisions there, so it is fine)
      ...
      mutable relevants : SFF.t;
          (** Set of relevant flat formulas. These are the formulas that have
              been added to the lazy CNF (i.e. asserted), including sub-formulas
              that are true.

              For instance, when asserting [a \/ (b /\ c)], the formula
              [a \/ (b /\ c)] is added to the [relevants] set; when deciding to
              pick the second branch of the disjunction, the formulas [b /\ c],
              [b] and [c] are added to the [relevants] set (see
      ...
      relevants_queue : SFF.t Vec.t;
          (** Checkpoint of the [relevant] field at each decision level. Only
              used with the CDCL-Tableaux solver. *)
      mutable ff_lvl : int MFF.t;
          (** Map from flat formulas to the (earliest) decision level at which
              they were asserted. Only used with the CDCL-Tableaux solver. *)
      mutable lvl_ff : SFF.t Util.MI.t;
          (** Map from decision level to the set of flat formulas asserted at
              that level. Set inverse of [ff_lvl]. Only used with the
              CDCL-Tableaux solver. *)
      ...
      objectives : Objective.Function.t list Vec.t;
          (** Queue of the objectives per incremental levels.
              ...
              Notice that objectives of the incremental toplevel are directly
              added to the theory and so do not appear in this queue. *)
      mutable next_decisions : Atom.atom list;
          (** Literals that must be decided on before the solver can answer
              [Sat].
      ...
      mutable next_split : Atom.atom option;
          (** Literal that should be decided on before the solver answers [Sat].
              ...
              These are added by the theory through calls to [acts_add_split].
              The difference with [next_decisions] is that the [splits] are
              optional (i.e. they can be dropped, and the solver is allowed to
              answer [Sat] without deciding on the splits if it chooses so)
              whereas once added, the [decisions] are guaranteed to be decided
              on at some point (unless backtracking occurs). *)
      mutable next_optimized_split :
        (Objective.Function.t * Objective.Value.t * Atom.atom) option
          (** Objective functions that must be optimized before the solver can
              ...
              towards "more optimal" values if the objective is not reachable);
              once the decision is made by the solver, the optimized value is
              sent back to the theory through [Th.add_objective]. *)
    }
  ...
  let create hcons_env =
    { hcons_env;
      ...
      simpDB_assigns = -1;
      simpDB_props = 0;
      order = Vheap.create 0 Atom.dummy_var;
      (* sera mis a jour dans solve *)
  ...
  let insert_var_order env (v : Atom.var) = Vheap.insert env.order v
  ...
  let var_bump_activity env (v : Atom.var) =
    ...
    if Float.compare v.weight 1e100 > 0
    then begin
      Vec.iter (fun (v : Atom.var) -> v.weight <- v.weight *. 1e-100) env.vars;
      env.var_inc <- env.var_inc *. 1e-100
    end;
    if Vheap.in_heap v then Vheap.decrease env.order v

  let clause_bump_activity env (c : Atom.clause) =
    c.activity <- c.activity +. env.clause_inc;
    if Float.compare c.activity 1e20 > 0
    then begin
      Vec.iter
        (fun (clause : Atom.clause) ->
  ...
  let new_decision_level env =
    ...
    then Profiling.decision (decision_level env) "<none>";
    Vec.push env.tenv_queue env.tenv;
    (* save the current tenv *)
    if Options.get_cdcl_tableaux ()
    then begin
  ...
  let attach_clause env (c : Atom.clause) =
    ...
    if c.learnt
    then env.learnts_literals <- env.learnts_literals + Vec.size c.atoms
    else env.clauses_literals <- env.clauses_literals + Vec.size c.atoms

  let detach_clause env (c : Atom.clause) =
    c.removed <- true;
    (* Vec.remove (Vec.get c.atoms 0).neg.watched c; Vec.remove (Vec.get c.atoms
       1).neg.watched c; *)
    if c.learnt
    then env.learnts_literals <- env.learnts_literals - Vec.size c.atoms
    else env.clauses_literals <- env.clauses_literals - Vec.size c.atoms
  ...
  let satisfied (c : Atom.clause) =
    Vec.exists
      (fun (atom : Atom.atom) -> atom.is_true && atom.var.level == 0)
      c.atoms

  let is_assigned (a : Atom.atom) = a.is_true || a.neg.is_true
  ...
  let enqueue_assigned env (a : Atom.atom) =
    if Options.get_debug_sat ()
    then Printer.print_dbg "[satml] enqueue_assigned: %a@." Atom.pr_atom a;
    if a.neg.is_guard
    then begin
      (* if the negation of a is (still) a guard, it should be forced to true
         during the first decisions. If the SAT tries to deduce that a.neg is
         true (ie. a is false), then we have detected an inconsistency. *)
    ...
    if is_semantic a then env.nchoices <- env.nchoices + 1
    ...
    then begin
      ...
      for c = Vec.size env.trail - 1 downto env.qhead do
        ...
        then begin
          ...
          unassign_atom a;
          if a.is_guard then env.next_dec_guard <- env.next_dec_guard - 1;
          if not (is_semantic a) then insert_var_order env a.var
        end
      done;
      Queue.clear env.tatoms_queue;
      Queue.clear env.th_tableaux;
      env.tenv <- Vec.get env.tenv_queue lvl;
      (* recover the right tenv *)
      ...
      if Options.get_cdcl_tableaux ()
      then begin
        Vec.shrink env.lazy_cnf_queue lvl;
        Vec.shrink env.relevants_queue lvl
        [@ocaml.ppwarning "TODO: try to disable 'fill_with_dummy'"]
      end;
      (try
         let last_dec =
           if Vec.size env.trail_lim = 0 then 0 else Vec.last env.trail_lim
         in
      ...
      (* Some variables that were irrelevant because of canceled
         ...
         heap. *)
      if Options.get_cdcl_tableaux_inst ()
      then
        VH.filter_map_inplace
          (fun v () ->
            if is_relevant env v
            then (
  ...
  let debug_enqueue_level a lvl reason =
    ...
    | Some (c : Atom.clause) ->
      ...
      Vec.iter
        (fun (atom : Atom.atom) ->
          if not (Atom.eq_atom a atom) then maxi := max !maxi atom.var.level)
        c.atoms;
      !maxi = lvl
  ...
  let max_level_in_clause (c : Atom.clause) =
    ...
    Vec.iter
      (fun (a : Atom.atom) -> max_lvl := max !max_lvl a.var.level)
      c.atoms;
    !max_lvl

  let enqueue env (a : Atom.atom) lvl reason =
    assert (
      (not a.is_true) && (not a.neg.is_true) && a.var.level < 0
    ...
    a.var.reason <- reason;
    if Options.get_debug_sat ()
    then Printer.print_dbg "[satml] enqueue: %a@." Atom.pr_atom a;
    Vec.push env.trail a;
    if is_semantic a then env.nchoices <- env.nchoices + 1;
    a.var.index <- Vec.size env.trail;
    Options.heavy_assert (fun () -> debug_enqueue_level a lvl reason)

  (* let progress_estimate env = let prg = ref 0. in let nbv = Atom.to_float
     (nb_vars env) in let lvl = decision_level env in let _F = 1. /. nbv in for
     i = 0 to lvl do let _beg = if i = 0 then 0 else Vec.get env.trail_lim (i-1)
     in let _end = if i=lvl then Vec.size env.trail else Vec.get env.trail_lim i
     in prg := !prg +. _F**(Atom.to_float i) *. (Atom.to_float (_end - _beg))
     done; !prg /. nbv *)
  ...
  let best_propagation_level env c =
    let mlvl =
      if Options.get_minimal_bj ()
      then max_level_in_clause c
  ...
  let propagate_in_clause env (a : Atom.atom) (c : Atom.clause) i watched new_sz
    ...
    if first == a.neg
    then begin
      (* le literal faux doit etre dans .(1) *)
  ...
  let propagate_atom env (a : Atom.atom) res =
    ...
    begin try
      Vec.iteri
        (fun i (clause : Atom.clause) ->
          if not clause.removed
          then propagate_in_clause env a clause i watched new_sz_w)
        watched
    with Conflict c ->
      assert (!res == C_none);
      res := C_bool c
  ...
  let acts_add_decision_lit env lit =
    ...
    else assert (atom.is_true || atom.neg.is_true)
  ...
  let do_case_split env origin =
    try
      ...
      env.tenv <- tenv;
      C_none
    with Ex.Inconsistent (expl, _) -> C_theory expl
  ...
  let get_atom_or_proxy f proxies =
    ...
    | UNIT a -> a
    | _ -> (
      match get_proxy_of f proxies with Some a -> a | None -> assert false)
  ...
  (* [add_form_to_lazy_cnf env lazy_cnf ff] updates [env] and [lazy_cnf] by
     ...
     [relevant] field in [env]; - UNIT formulas (either [ff] or in a conjunctive
     component) are added to the [th_tableaux] queue in [env] (NOTE:
     [th_tableaux] is only used if the [get_cdcl_tableaux_th ()] option is
     enabled); - Disjunctive formulas that contain a component already known to
     be true are treated as that component (this is used by
     [relevancy_propagation] which adds the decided children of disjunctive
     formulas -- it is also necessary for soundness in case a child atom was
     propagated earlier than expected e.g. due to clause learning); -
     Disjunctive formulas that are still undecided are added to the [lazy_cnf]
     (see documentation of the [lazy_cnf] field in [env]). *)
    ...
    let rec add_aux env ma (f_a : t) =
      ...
      then ma
        ...
        | UNIT a ->
          ...
          ma
        | AND l -> List.fold_left (add_aux env) ma l
        | OR l -> (
          match
            List.find_opt
              (fun e ->
  ...
  (** [relevancy_propagation env lazy_cnf a] propagates the fact that atom [a]
      ...
      the form [a_0 \/ ... \/ a \/ ... \/ a_n] must hold given the current CNF
      unfolding), its parents are removed from the [lazy_cnf] and [a] is added
      again to the [lazy_cnf]. This will either add [a] to the [th_tableaux]
      field (if [a] corresponds to a [UNIT] flat formula that was asserted), or
      otherwise perform one step of CNF unfolding and add the formula that [a]
      was a proxy for to the [lazy_cnf] (see [add_form_to_lazy_cnf]). *)
              ...
              List.fold_left
                (fun ma bf ->
                  ...
                  then ma
                    ...
                    let mf_b, fb =
                      try Matoms.find b ma with Not_found -> assert false
                    in
                    assert (FF.equal bf fb);
                    let mf_b = MFF.remove fp mf_b in
                    if MFF.is_empty mf_b
                    then Matoms.remove b ma
                    else Matoms.add b (mf_b, fb) ma)
                ma lp)
            parents ma
        in
        assert (
          let a = get_atom_or_proxy f_a env.proxies in
          a.is_true);
  ...
  let unit_theory_propagate env _full_q lazy_q =
    ...
    in
    if Options.get_debug_sat ()
    then Printer.print_dbg "[satml] Unit theory_propagate of %d atoms@." !nb_f;
    if facts == []
    then C_none
    else
      try
        ...
        let t, _, cpt =
          Th.assume ~ordered:false (List.rev facts) env.unit_tenv
        in
  ...
  let theory_propagate env =
    ...
    if Options.get_cdcl_tableaux () then compute_facts_for_theory_propagate env;
    let tatoms_queue =
      if Options.get_cdcl_tableaux_th ()
      then begin
        env.th_tableaux
      end
    ...
    | C_none -> (
      ...
      while not (Queue.is_empty tatoms_queue) do
        ...
        let ta =
          if a.is_true
          then a
          else if a.neg.is_true
          then a.neg (* TODO: useful ?? *)
          else assert false
        in
        let ex =
          if Options.get_unsat_core () || ta.var.level > 0
          then Ex.singleton (Ex.Literal ta)
          else Ex.empty
        in
      ...
      if Options.get_debug_sat ()
      then Printer.print_dbg "[satml] Theory_propagate of %d atoms@." !nb_f;
      Queue.clear env.tatoms_queue;
      Queue.clear env.th_tableaux;
      if !facts == []
      then C_none
        ...
        try
          ...
          let t, _, cpt =
            Th.assume
              ~ordered:(not (Options.get_cdcl_tableaux_th ()))
              (List.rev !facts) env.tenv
          in
          Steps.incr (Steps.Th_assumed cpt);
          env.tenv <- t;
          do_case_split env Util.AfterTheoryAssume
          (*if full_model then expensive_theory_propagate () else None*)
        with Ex.Inconsistent (dep, _terms) ->
          (* XXX what to do with terms ? *)
          (* Printer.print_dbg
                     "th inconsistent : %a @." Ex.print dep; *)
  ...
  let propagate env =
    ...
    while env.qhead < Vec.size env.trail do
      ...
      if Options.get_debug_sat ()
      then Printer.print_dbg "[satml] propagate atom %a@." Atom.pr_atom a;
  ...
  (* unused -- let f_sort_db c1 c2 = let sz1 = Vec.size c1.atoms in let sz2 =
     Vec.size c2.atoms in let c = Stdlib.compare c1.activity c2.activity in if
     sz1 = sz2 && c = 0 then 0 else if sz1 > 2 && (sz2 = 2 || c < 0) then -1
     else 1

     let locked _ = false (* try for i = 0 to Vec.size env.vars - 1 do match
     (Vec.get env.vars i).reason with | Some c' when c ==c' -> raise Exit | _ ->
     () done; false with Exit -> true *) *)

  let reduce_db () = ()
  (* let extra_lim = env.clause_inc /. (to_float (Vec.size env.learnts)) in
     Vec.sort env.learnts f_sort_db; let lim2 = Vec.size env.learnts in let lim1
     = lim2 / 2 in let j = ref 0 in for i = 0 to lim1 - 1 do let c = Vec.get
     env.learnts i in if Vec.size c.atoms > 2 && not (locked c) then
     remove_clause c else begin Vec.set env.learnts !j c; incr j end done; for i
     = lim1 to lim2 - 1 do let c = Vec.get env.learnts i in if Vec.size c.atoms
     > 2 && not (locked c) && c.activity < extra_lim then remove_clause c else
     begin Vec.set env.learnts !j c; incr j end done; Vec.shrink env.learnts !j
     true *)
  ...
  module HUC = Hashtbl.Make (struct
    type t = Atom.clause
  ...
  let print_aux fmt hc = Format.fprintf fmt "%a@," Atom.pr_clause hc
  ...
  let report_b_unsat env linit =
    ...
    then begin
      ...
      match linit with
      | [] | _ :: _ :: _ ->
        (* currently, report_b_unsat called with a singleton if get_unsat_core =
           true *)
        assert false
      | [{ Atom.atoms; _ }] ->
        ...
        Printer.print_dbg ~header:false
          ...
          (Printer.pp_list_no_space print_aux)
          !l;
        ...
        let rec roots todo =
          ...
          | (c : Atom.clause) :: r -> (
            ...
            | [] ->
              if not (HUC.mem uc c) then HUC.add uc c ();
              roots r
            | prems ->
              roots prems;
              roots r)
  ...
  let report_t_unsat env dep =
    ...
    then begin
        ...
        Ex.fold_atoms
          (fun ex l ->
            ...
            | Ex.Literal { var = v; _ } ->
              ...
              begin match v.reason with None -> l | Some c -> c :: l
              end
            | _ -> assert false (* TODO *))
          dep []
      in
      ...
      roots l;
      let unsat_core = HUC.fold (fun c _ l -> c :: l) uc [] in
      Printer.print_dbg ~header:false "@[<v 2>T-UNSAT_CORE:@ %a@]"
        (Printer.pp_list_no_space print_aux)
        unsat_core;
      env.is_unsat <- true;
      let unsat_core = Some unsat_core in
  ...
  (*** experimental: taken from ctrl-ergo-2 ********************
    ...
    let rec theory_simplify () = let theory_simplification = 2 in let assume a =
    assert (E.is_ground ta.lit); ignore (Th.assume a.lit Ex.empty env.tenv) in
    if theory_simplification >= 2 then begin for i = 0 to Vec.size env.vars - 1
    do try let a = (Vec.get env.vars i).pa in if not (a.is_true ||
    a.neg.is_true) then try assume a; try assume a.neg with Ex.Inconsistent _ ->
    if debug () then Printer.print_dbg "%a propagated m/theory at level 0@."
    Atom.pr_atom a; enqueue a 0 None (* Mettre Some dep pour les unsat-core*)
    with Ex.Inconsistent _ -> if debug () then Printer.print_dbg "%a propagated
    m/theory at level 0@." Atom.pr_atom a.neg; enqueue a.neg 0 None (* Mettre
    Some dep pour les unsat-core*) with Not_found -> () done;

    let head = env.qhead in if propagate () <> None || theory_propagate () <>
    None then raise (Unsat []); let head' = env.qhead in if head' > head then
    theory_simplify () end *)
  ...
  let simplify env =
    ...
    if nb_assigns env <> env.simpDB_assigns && env.simpDB_props <= 0
    then begin
      if Options.get_debug_sat ()
      then Printer.print_dbg ~module_name:"Satml" ~function_name:"simplify" "";
      (*theory_simplify ();*)
  ...
  let record_learnt_clause env ~is_T_learn blevel learnt history =
    ...
    if (not is_T_learn) || Options.get_minimal_bj () || blevel = curr_level
    then begin
      ...
      | [(fuip : Atom.atom)] ->
        ...
        (* Note: there could potentially be semantic values that are no longer
           valid in the union-find at level 0 contained in [fuip] if [fuip] is a
           semantic literal, which could cause crashes -- although it is not
           clear to me that it can really happen since if we learn it at level 0
           it must be explainable by facts at level 0. *)
        enqueue env fuip 0 None
      | fuip :: _ ->
        let name = Atom.fresh_lname () in
        let lclause = Atom.make_clause name learnt vraie_form true history in
        (* Do not learn clauses involving semantic literals (but still record
           ...
           [conflict_analyze_aux], but we can still have semantic decisions that
           would end up in clauses.
  ...
  (* If [a] is a semantic literal that is not a decision, we ideally would like
     to replace it with its reason.

     For instance, if [x = 0] is implied by [a1 /\ .. /\ an] (i.e. the reason
     for [x = 0] is [x <> 0 \/ not a1 \/ .. \/ not an]) and [x = 0] is in
     conflict with [b] (i.e. we are learning [x <> 0 \/ not b]) we can apply
     resolution to learn [not a1 \/ .. \/ not an \/ not b] instead, eliminating
     the semantic literal [x = 0] in the process. *)
  let rec semantic_expansion env max_lvl blevel learnt seen (a : Atom.atom) =
    ...
    if (not a.var.seen) && a.var.level > 0
    then (
  ...
  let conflict_analyze_aux env c_clause max_lvl =
    ...
    while !cond do
      ...
      while
        ...
        (not v.seen) || (Options.get_minimal_bj () && v.level < max_lvl)
      do
        decr tr_ind
      done;
      decr pathC;
      ...
      decr tr_ind;
      (* Do not use semantic literals as UIPs unless forced to (i.e. there is no
         term UIP and the decision was a semantic literal) *)
      let is_good_uip (p : Atom.atom) =
        match p.var.reason with None -> true | Some _ -> not (is_semantic p)
      in
    ...
    let learnt =
      List.fast_sort
        (fun (a : Atom.atom) (b : Atom.atom) -> b.var.level - a.var.level)
        learnt
    in
    let bj_level =
      if Options.get_minimal_bj ()
      then match learnt with [] -> 0 | a :: _ -> max 0 (a.var.level - 1)
      else !blevel
    in
    bj_level, learnt, !history

  let fixable_with_simple_backjump (confl : Atom.clause) max_lvl lv =
    if not (Options.get_minimal_bj ())
    then None
    else
      try
        ...
        Ex.fold_atoms
          (fun ex (acc, max_lvl, c_hist) ->
            ...
            | Ex.Literal a ->
              ...
              let c_hist =
                match a.var.reason with None -> c_hist | Some r -> r :: c_hist
              in
              if a.var.level = 0
              then acc, max_lvl, c_hist
  ...
  (* try let env = ref (Th.empty()) in (); Ex.iter_atoms (fun atom -> let t,_,_
     = Th.assume ~cs:true atom.lit (Ex.singleton atom) !env in env := t) dep; (*
     ignore (Th.expensive_processing !env); *) assert false with Ex.Inconsistent
     _ -> () *)
  ...
  let reason_of_deduction true_atom =
    ...
    try find_uip_reason q with Last_UIP_reason r -> r
  ...
  let th_entailed tenv a =
    if Options.get_no_tcp () || not (Options.get_minimal_bj ())
    then None
    else
      match Shostak.Literal.view @@ Atom.literal a with
      | LSem _ -> None
  ...
  let make_decision env atom =
    ...
    | None ->
      ...
      if Options.get_debug_sat ()
      then Printer.print_dbg "[satml] decide: %a" Atom.pr_atom atom;
  ...
  let rec pick_branch_aux env (atom : Atom.atom) =
    ...
    else if Options.get_cdcl_tableaux_inst ()
    then
      ...
      if is_relevant env v
      then make_decision env atom
      else (
        VH.add env.irrelevants v ();
        pick_branch_lit env)
    else make_decision env atom
  ...
  and pick_branch_lit env =
    ...
    | Some (fn, value, atom) ->
      ...
      then (
        assert (v.pa.is_true || v.na.is_true);
        if atom.is_true then env.tenv <- Th.add_objective env.tenv fn value;
        pick_branch_lit env)
      else (
  ...
  let pick_branch_lit env =
    if env.next_dec_guard < Vec.size env.increm_guards
    then begin
      ...
      let tenv =
        List.fold_right
          (fun fn tenv -> Th.add_objective tenv fn Objective.Value.Unknown)
          fns env.tenv
      in
      env.tenv <- tenv
    end
    else pick_branch_lit env
  ...
  let search env n_of_conflicts n_of_learnts =
    ...
    while true do
      ...
      if is_sat env then raise Sat;
      if
        Options.get_enable_restarts ()
        && n_of_conflicts >= 0
        && !conflictC >= n_of_conflicts
      then begin
      ...
      if
        n_of_learnts >= 0
        && Vec.size env.learnts - nb_assigns env >= n_of_learnts
      then reduce_db ();
      pick_branch_lit env
    done

  (* unused -- let check_clause c = let b = ref false in let atoms = c.atoms in
     for i = 0 to Vec.size atoms - 1 do let a = Vec.get atoms i in b := !b ||
     a.is_true done; assert (!b)

     let check_vec vec = for i = 0 to Vec.size vec - 1 do check_clause (Vec.get
     vec i) done

     let check_model env = check_vec env.clauses; check_vec env.learnts *)

  let solve env : unit =
    if env.is_unsat then raise (Unsat env.unsat_core);
    let n_of_conflicts = ref (Atom.to_float env.restart_first) in
    let n_of_learnts =
      ref (Atom.to_float (nb_clauses env) *. env.learntsize_factor)
    in
    try
      while true do
        (try
           search env (Atom.to_int !n_of_conflicts) (Atom.to_int !n_of_learnts)
         with Restart -> ());
  ...
  let rec compute_concrete_model ~declared_ids env =
    ...
    | () -> (
      if is_sat env
      then Th.extract_concrete_model ~declared_ids env.tenv
      else
        try
          solve env;
          assert false
        with Sat -> compute_concrete_model ~declared_ids env)
  ...
  let compute_concrete_model ~declared_ids env =
    assert (is_sat env);
    (* Make sure all objectives are optimized before starting model generation.
       This can cause us to backtrack some of the splits done at the theory
       level, which is fine, because we don't care about these at the SAT
       level. *)
    let rec loop env =
      let acts = theory_slice env in
      ...
      then
        try
          ...
          assert false
        with Sat -> loop env
      else compute_concrete_model ~declared_ids env
    in
    loop env
        ...
        then
          ...
          then
            partition_aux trues unassigned falses
              (List.rev_append a.var.vpremise init)
              r
  ...
  let add_clause env f ~cnumber atoms =
    ...
    let init0 =
      ...
      then [Atom.make_clause init_name atoms f false []]
      else [] (* no deps if unsat cores generation is not enabled *)
    in
    try
      let atoms, init =
        if decision_level env = 0
        then
          let atoms, init =
            List.fold_left
          ...
          in
          ( List.fast_sort
              (fun a b -> Atom.compare_var a.Atom.var b.Atom.var)
              atoms,
            init )
      ...
      | a :: b :: _ ->
        ...
        if Options.(get_debug_sat () && get_verbose ())
        then
        ...
        else if (not a.is_true) && b.neg.is_true
        then
          begin
            (* clause is unit *)
          ...
          [@ocaml.ppwarning
            "TODO: add a heavy assert that checks that clauses are not \
             redundant, watchs are well set, unit and bottom are detected ..."]
      | [a] ->
        if Options.(get_debug_sat () && get_verbose ())
        then
          Printer.print_dbg ~module_name:"Satml" ~function_name:"add_clause"
        ...
        assert (lvl <> 0);
        begin if not (Options.get_minimal_bj ())
        then cancel_until env 0
        else if a.is_true || a.neg.is_true
        then cancel_until env (lvl - 1)
        end;
        a.var.vpremise <- init;
        enqueue env a 0 None;
        propagate_and_stabilize env propagate (ref 0)
      (* TODO *)
    with Trivial -> if Options.get_profiling () then Profiling.elim true

  let update_lazy_cnf env ~do_bcp sff ~dec_lvl =
    if Options.get_cdcl_tableaux () && dec_lvl <= decision_level env
    then begin
      let s =
        try Util.MI.find dec_lvl env.lvl_ff with Not_found -> SFF.empty
      in
      let lz, s =
        SFF.fold
          (fun ff (l, s) ->
        ...
        List.fold_left
          (fun ((unit_cnf, nunit_cnf) as accu) (v : Atom.var) ->
            ...
            | Some (c, sz) ->
              assert (sz >= 1);
              if sz = 1
              then c :: unit_cnf, nunit_cnf
              else
                ( unit_cnf,
                  (c :: nunit_cnf)
                  [@ocaml.ppwarning
                    "Issue: BAD decision_level, in particular, if minimal-bj \
                     is ON"] ))
          (unit_cnf, nunit_cnf) new_v
      in
      (* This assert is no longer true because some of the vars in the
         [hcons_env] are now semantic literals. assert (nbv = Vec.size
         env.vars); *)
      accu

  let set_new_proxies env proxies = env.proxies <- proxies
  ...
  let try_to_backjump_further =
    let rec better_bj env sf =
      ...
      let fictive_lazy =
        SFF.fold (fun ff acc -> add_form_to_lazy_cnf env acc ff) sf old_lazy
      in
    ...
    in
    fun env sff -> if Options.get_cdcl_tableaux () then better_bj env sff

  let assume env unit_cnf nunit_cnf f ~cnumber sff ~dec_lvl =
    if env.is_unsat then raise (Unsat env.unsat_core);
    begin match unit_cnf, nunit_cnf with
    | [], [] -> ()
    | _, _ ->
      let nbc =
        env.nb_init_clauses + List.length unit_cnf + List.length nunit_cnf
      in
      ...
      then
        Printer.print_dbg "%d clauses@ %d learnts" (Vec.size env.clauses)
  ...
  let instantiation_context env hcons =
    if Options.get_cdcl_tableaux_th ()
    then
      (* use atoms from theory environment if tableaux method is used for
         theories *)
      E.Set.fold
        (fun a accu -> SA.add (FF.get_atom hcons a) accu)
        (Th.get_assumed env.tenv) SA.empty
    else if Options.get_cdcl_tableaux_inst ()
    then
      (* use relevants atoms from environment if tableaux method is used for
         instantiation *)
      SFF.fold
        (fun f acc ->
          match FF.view f with FF.UNIT a -> SA.add a acc | _ -> acc)
  ...
  (* let copy (v : 'a) : 'a = Marshal.from_string (Marshal.to_string v []) 0
     ...
     let save env = let sv = { env = env; st_cpt_mk_var =
     !Solver_types.cpt_mk_var; st_ma = !Solver_types.ma } in copy sv *)
  ...
  let pop env =
    ...
    assert (not g.var.na.is_true);
    (* atom not false *)
    if g.var.pa.is_true
    then (* if already decided *)
      begin
      assert (g.var.level > 0);
      cancel_until env (g.var.level - 1);
      (* undo its decision *)
      (* all previous guards are decided *)
      env.next_dec_guard <- Vec.size env.increm_guards
    end
    else
      (* We are not decided, so the `next_dec_guard` should point to an earlier
         guard or we have a bug because guards must be decided in the
         `increm_guards` order. *)
  ...
  let optimize env fn =
    ...
    if Vec.size env.increm_guards = 0
    then env.tenv <- Th.add_objective env.tenv fn Objective.Value.Unknown
    else
      Vec.replace
        (fun fns -> fn :: fns)
        env.objectives
