    ...
    vbox
      ...
      ++ iter
           ~sep:
             (any "@,------------------------------------------------------@,")
           (fun f ->
             MapI.iter (fun _ (B ((module D), d)) -> f (hvbox (const D.pp d))))
           (fun ppf pp -> pp ppf ()))
    ++ cut
...
let terms_of_distinct env l =
  ...
  | Xliteral.Distinct (false, rl) ->
      ...
      List.fold_left
        (fun acc r ->
          try
            ...
            SE.iter
              (fun t ->
                if X.equal (ME.find t env.make) r then raise (Found_term t))
              cl;
            acc
          with
          | Found_term t -> t :: acc
    ...
    let rec distrib = function
      | x :: r -> distrib r @ List.map (fun y -> SE.add x (SE.singleton y)) r
      | [] -> []
    in
    distrib lt
  | _ -> assert false
...
module Debug = struct
  ...
  (* unused -- let rs_print fmt = SetX.iter (fprintf fmt "\t%a@." X.print) *)
  ...
  let pmake fmt m =
    ...
    ME.iter (fun t r -> Format.fprintf fmt "%a -> %a@," E.print t X.print r) m;
    Format.fprintf fmt "@]@,"
  ...
  let prules fmt s =
    ...
    RS.iter
      (fun _ srl ->
        SetRL.iter
          (fun (ac, d, dep) ->
            Format.fprintf fmt "%a ~~> %a %a@," X.print (X.ac_embed ac) X.print
              d Ex.print dep)
          srl)
      s;
    Format.fprintf fmt "@]@,"
  ...
  let pclasses fmt m =
    ...
    MapX.iter
      (fun k s ->
        Format.fprintf fmt "%a -> %a@," X.print k E.print_list (SE.elements s))
      m;
    Format.fprintf fmt "@]@,"

  (* unused -- let pgamma fmt m = fprintf fmt "------------- UF: Gamma map
  ...
  let pneqs fmt m =
    ...
    MapX.iter
      (fun k s -> Format.fprintf fmt "%a -> %a@ " X.print k lm_print s)
      m;
    Format.fprintf fmt "@]@ "

  let all env =
    if Options.get_debug_uf ()
    then
  ...
  let lookup_not_found t env =
    print_err "Uf: %a Not_found in env" E.print t;
    all env

  let canon_of r rr =
    if Options.(get_rewriting () && get_verbose ())
    then print_dbg "canon %a = %a" X.print r X.print rr
  ...
  let ac_solve p v dep =
    ...
    then
      print_dbg ~module_name:"Uf" ~function_name:"ac_solve"
        "ac-solve: %a |-> %a %a" X.print p X.print v Ex.print dep
      ...
      MapX.for_all
        (fun _ (rr, _) ->
          try
            ...
            false
          with Not_found ->
            (* all leaves that are in normal form should be in repr ? not AC
               leaves, which can be created dynamically, not for other, that can
               be introduced by make and solve*)
            true)
        repr
...
module Env = struct
  ...
  let disjoint_union l_1 l_2 =
    let rec di_un (l1, c, l2) (l_1, l_2) =
      ...
      | (a, m) :: r, (b, n) :: s ->
        ...
        then
          if m = n
          then di_un (l1, (a, m) :: c, l2) (r, s)
          else if m > n
          then di_un ((a, m - n) :: l1, (a, n) :: c, l2) (r, s)
          else di_un (l1, (b, n) :: c, (b, n - m) :: l2) (r, s)
        else if cmp > 0
        then di_un ((a, m) :: l1, c, l2) (r, (b, n) :: s)
    ...
    let rec di_un l1 l_1 l_2 =
      ...
      | (a, m) :: r, (b, n) :: s ->
        ...
        then
          ...
          then di_un ((a, m - n) :: l1) r s
          else raise List_minus_exn
        else if cmp > 0
        then di_un ((a, m) :: l1) r ((b, n) :: s)
        else raise List_minus_exn
    in
    di_un [] l_1 l_2
  ...
  let explain_repr_of_distinct x repr_x dep lit env =
    ...
    | Xliteral.Distinct (false, ([_; _] as args)) ->
      List.fold_left
        ...
        dep args
    | Xliteral.Pred (r, _) -> Ex.union dep (snd (find_or_normal_form env r))
  ...
  let update_neqs x repr_x dep env =
    let merge_disjoint_maps l1 ex1 mapl =
      ...
      with Not_found ->
        (* with the use of explain_repr_of_distinct above, I don't need to
           propagate dep to ex1 here *)
        MapL.add l1 ex1 mapl
    in
    let nq_r1 = lookup_for_neqs env x in
    let nq_r2 = lookup_for_neqs env repr_x in
    let small, big =
      if MapL.height nq_r1 < MapL.height nq_r2
      then nq_r1, nq_r2
      else nq_r2, nq_r1
    in
    let mapl = MapL.fold merge_disjoint_maps small big in
    (* remove x from the map to avoid eventual bugs if call this function again
       with x == repr_x *)
  ...
  let init_leaf env p =
    ...
    let env =
      { env with
        ...
        domains =
          (if in_repr then env.domains else GlobalDomains.init rp env.domains);
        classes =
          (if MapX.mem p env.classes
           then env.classes
           else update_classes p rp env.classes);
        gamma = (if in_repr then env.gamma else add_to_gamma p rp env.gamma);
        neqs =
          (if MapX.mem p env.neqs
           then env.neqs
  ...
  let init_term env t =
    ...
    let env =
      { env with
        ...
        neqs =
          (if MapX.mem rp env.neqs
           then env.neqs (* pourquoi ce test *)
  ...
  let head_cp eqs env pac ({ Sig.h; _ } as ac) v dep =
    try
      ...
      SetRL.iter
        (fun (g, d, dep_rl) ->
          if X.equal pac (X.ac_embed g) && X.equal v d
          then ()
          else
            ...
            | l1, _cm, l2 ->
              ...
              if not (X.equal rx ry)
              then Queue.push (rx, ry, Ex.union dep dep_rl) eqs)
        (RS.find h env.ac_rs)
  ...
  (* Patch modudo AC for CC: if p is a leaf different from r and r is AC and
     reduced by p, then r --> nrr should be added as a PIVOT, not just as
     TOUCHED by p |-> ... This is required for a correct update of USE *)
...
let union env r1 r2 dep =
  Timers.with_timer Timers.M_UF Timers.F_union @@ fun () -> union env r1 r2 dep
    ...
    List.fold_left
      (fun (env, mapr, newds) r ->
        ...
        with Not_found ->
          ...
          let mdis =
            try MapL.add d (Ex.merge uex (MapL.find d mdis)) mdis
      ...
      MapX.fold
        (fun r2 ex2 env ->
          ...
          try
            ...
            | [(a, b)] ->
              if (X.equal a r1 && X.equal b r2) || (X.equal a r2 && X.equal b r1)
              then env
...
let are_equal env t1 t2 ~added_terms =
  if E.equal t1 t2
  then Th_util.Entailed { ex = Ex.empty; classes = cl_extract env }
  else
    let lookup =
...
let rclass_of env r = try MapX.find r env.classes with Not_found -> SE.empty

let term_repr uf t =
  let st = class_of uf t in
  try SE.min_elt st with Not_found -> t
...
let add env t =
  Timers.with_timer Timers.M_UF Timers.F_add_terms @@ fun () -> add env t
...
let distinct_from_constants rep env =
  ...
  MapL.fold
    (fun lit _ acc ->
      ...
      let lit_vals =
        match LX.view lit with Xliteral.Distinct (_, l) -> l | _ -> []
      in
      let acc2 =
        List.fold_left
          (fun acc r ->
...
let assign_next env =
  ...
  let res, env =
    ...
    with Exit -> (
      ...
      | None -> assert false
      | Some (s, rep, is_cs) ->
        if Options.get_debug_interpretation ()
        then
          Printer.print_dbg ~module_name:"Uf" ~function_name:"assign_next"
            "TRY assign-next %a = %a" X.print rep E.print s;
        (* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! modify this to be
           able to returns CS on terms. This way, we will not modify env in this
           function !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! *)
...
let reinit_cache () = LX.reinit_cache ()
...
(* A map of expressions to terms, ordered by depth first, and then by
   [Expr.compare] for expressions with same depth. This structure will be used
   to build a model, by starting with the inner/smaller terms first. The values
   associated to the key will be their make *)
module MED = Map.Make (struct
  type t = Expr.t
...
let is_suspicious_name hs =
  match Hstring.view hs with "@/" | "@%" | "@*" -> true | _ -> false
...
module Cache = struct
  let store_array_get arrays_cache (t : E.t) (i : E.t) (v : E.t) =
    ...
    | values -> E.Table.replace values i v
end
...
let is_destructor = function Sy.Op (Destruct _) -> true | _ -> false

(* The environment of the union-find contains almost a first-order model. There
   are two situations that require some computations to retrieve an appropriate
   model value: - As our array theory has no semantic values, there are no value
   ...
   function retrieves all these accesses in order to build an expression which
   defines the approriate array. - If the problem involves declared terms whose
   the type is abstract, Alt-Ergo cannot produces a constant value for them.
   This function creates a new abstract value in this case. *)
let compute_concrete_model_of_val cache =
  ...
  fun env t ((mdl, mrepr) as acc) ->
    ...
    (* TODO: We have to filter out destructors here as we don't consider pending
       destructors as solvable theory symbols of the ADT theory. We should check
       if these symbols can be defined as solvable to remove this particular
       case here. *)
    if
      X.is_solvable_theory_symbol f
      || is_destructor f || Sy.is_internal f || E.equal t E.vrai
      || E.equal t E.faux
    then
      (* These terms are built-in interpreted ones and we don't have to produce
      ...
      | _ ->
        Printer.print_err
          "Expect a uninterpreted term declared by the user, got %a" E.print t;
        assert false
    end
    ...
    let model =
      E.Table.fold
        (fun t vals mdl ->
          ...
          let arr_val =
            E.Table.fold
              (fun i v arr_val -> Expr.ArraysEx.store arr_val i v)
              vals t
          in
          let id, is_user =
            let Expr.{ f; _ } = Expr.term_view t in
          ...
          let mdl =
            if is_user
            then ModelMap.add (id, [], ty) [] arr_val mdl
            else
              (* Internal identifiers can occur here if we need to generate a
                 model term for an embedded array but this array isn't itself
          ...
          (* We need to update the model [mdl] in order to substitute all the
             ...
             `compute_concrete_model_of_val` because we need to first iterate on
             all the union-find environment to collect array values. *)
          ModelMap.subst id arr_val mdl)
        cache.array_selects model
    in
    { Models.propositional = prop_model; model; term_values = mrepr }

let extract_concrete_model ~prop_model ~defaults =
  let cache : cache = { array_selects = E.Table.create 17 } in
  fun env -> extract_concrete_model cache ~prop_model ~defaults env
