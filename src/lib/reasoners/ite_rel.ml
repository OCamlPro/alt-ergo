...
(* The present theory simplifies the ite terms t of the form ite(pred, t1, t2)
   where pred is an assumed predicate by introducing the equation t = t1 or t =
   t2 according to the truth value of pred. *)

type t =
  { pending_deds : Ex.t ME2.t;
    (* Map of pending deductions to their explanation. A deduction is an
       equation of the form t = b where t is an ite term and b is one of its
       branches. A deduction is added to pending_deds if the condition of the
       ite term is entailed by the current state of the solver. If so, the
       appropriate branch is selected according to the truth value of the
       condition. *)
    guarded_pos_deds : SE2.t ME.t;
    (* Map of the condition of ite terms to its if branch. This map contains
       only condition that was not entailed yet by the current state of the
       solver during the registration of the ite term. *)
    guarded_neg_deds : SE2.t ME.t;
    (* Map of the condition of ite terms to its else branch. This map contains
       only condition whose the negation was not entailed yet by the current
       state of the solver during the registration of the ite term. *)
    ...
    assumed_neg_preds : Ex.t ME.t
        (* Map of all the predicates whose the negation is entailed by the
           current state of the solver to their explanation. *)
  }
...
(* Check if the condition of the ite t is a predicate entailed by the current
   ...
   produce the deduction t = b. Otherwise save the if and else branches of t in
   order to retrieve them quickly in the assume function. *)
let add_aux env t =
  if Options.get_disable_ites ()
  then env
  else
    match is_ite t with
    | None -> env
    | Some (p, t1, t2) -> (
      if get_debug_ite ()
      then
        Printer.print_dbg ~module_name:"Ite_rel" ~function_name:"add_aux"
          "(if %a then %a else %a)" E.print p E.print t1 E.print t2;
      try
...
let add env uf _ t = add_aux env t, Uf.domains uf, []
    ...
    (fun acc (_ra, root, expl, _orig) ->
      ...
      | Some a -> (
        ...
        | E.Pred (t, is_neg)
          ...
          if get_debug_ite ()
          then
            Printer.print_dbg ~module_name:"Ite_rel" ~function_name:"assume"
              "%a" E.print a;
...
let extract_pending_deductions env =
  let l =
    ME2.fold
      (fun (s, t) ex acc ->
        let a =
          (E.mk_eq ~iff:false s t [@ocaml.ppwarning "TODO: build IFF instead ?"])
        in
        if get_debug_ite ()
        then
          Printer.print_dbg ~module_name:"Ite_rel" ~function_name:"assume"
            "deduce that %a with expl %a" E.print a Ex.print ex;
...
(* Save in the environment env all the assumed predicates of la. Produce new
   deductions implied by these new assumed predicates. Eventually, return all
   the pending deductions. *)
      ...
      TB.fold
        (fun (t, is_neg) ex env ->
          ...
          then
            ...
            let deds =
              try ME.find t env.guarded_neg_deds with Not_found -> SE2.empty
            in
            let pending_deds =
            ...
            let deds =
              try ME.find t env.guarded_pos_deds with Not_found -> SE2.empty
            in
            let pending_deds =
              SE2.fold (fun e acc -> ME2.add e ex acc) deds env.pending_deds
            in
