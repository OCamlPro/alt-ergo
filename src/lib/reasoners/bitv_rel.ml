...
let is_bv_ty = function Ty.Tbitv _ -> true | _ -> false

let is_bv_r r = is_bv_ty (X.type_info r)

let bitwidth r = match X.type_info r with Tbitv n -> n | _ -> assert false

let const sz n = Shostak.Bitv.is_mine @@ Bitv.int2bv_const sz n
...
module BitvNormalForm = struct
  ...
  let normal_form r =
    let rec loop cte rev_acc = function
      | [] -> (
        ...
        | [{ Bitv.bv = Bitv.Other { value; negated = false }; _ }] ->
          Atom (value, cte)
        | _ -> Composite (Shostak.Bitv.is_mine (List.rev rev_acc), cte))
    ...
    in
    loop Z.zero [] (Shostak.Bitv.embed r)
end
...
module BV2Nat = struct
    ...
    { bv2nat : (P.t * Ex.t) Extraction.Map.t MX.t;
      ...
      nat2bv : (X.r * Extraction.t * Ex.t) P.Map.t;
          (** Map from integer polynomials to their bit-vector representation.
              ...
              If [p] is associated with [(bv, ext, ex)] in this map, it means
              that [p = bv2nat(bv<ext>)] is justified by [ex].

              Note that this is the inverse of the [bv2nat] mapping, not an
              implementation of the [int2bv] function (although both coincide
              for entries in the map). *)
      use : P.Set.t MX.t;
          (** Map from integer variables to the polynomials that use them.
              ...
              extractions in [bv2nat]: we encode [bv2nat(bv<i, j>)] in terms of
              [bv2nat(bv<0, i>)] and [bv2nat(bv<0, j>)] which we create
              dynamically.

              This is done to reduce the number of variables introduced to
              represent extractions from quadratic to linear in the bitwidth.

              The polynomials associated with these expressions are thus not
              tracked by the UnionFind module, and are not given to [subst]. *)
      eqs : (X.r Xliteral.view * Ex.t) list
    }

  let pp_bv2nat_ext ppf ((x, ext), (p, _ex)) =
    Fmt.pf ppf "@[<2>bv2nat(%a<%d, %d>) =@ %a@]" X.print x ext.ofs ext.len
      P.print p

  let iter_ext f t =
    MX.iter (fun x m -> Extraction.Map.iter (fun ext -> f (x, ext)) m) t.bv2nat

  let pp ppf t =
    Fmt.pf ppf "@;<2>@[<v>%a@]" (Fmt.iter_bindings iter_ext pp_bv2nat_ext) t

  let empty =
    { bv2nat = MX.empty; nat2bv = P.Map.empty; use = MX.empty; eqs = [] }
  ...
  let fold_use_p x f t acc =
    ...
    | nps ->
      P.Set.fold
        (fun p acc ->
          let v = try P.Map.find p t.nat2bv with Not_found -> assert false in
          f p v acc)
        nps acc
  ...
  let find_ext bv ext bv2nat = Extraction.Map.find ext @@ MX.find bv @@ bv2nat
  ...
  (* Add the defining equation for [bv2nat(bv<ext>)] to [t].
     ...
     We define [bv2nat(bv<ext>)] in terms of the shifts [bv asr ext.ofs] and [bv
     asr ext.len]. *)
  ...
  let find_p p t = P.Map.find p t.nat2bv
  ...
  let add ~ex bv ext p t =
    ...
    | Some cte ->
      ...
      let lit = Uf.LX.mkv_eq (T.BV.extract bv ext) (const ext.len cte) in
    ...
    let rec loop p ex t = function
      ...
      | { bv = Other s; sz } :: bv' ->
        ...
        let (p_value, ex_value), t =
          find_or_init_ext s.value (Extraction.full ~size:sz) t
        in
        let p_value =
          if s.negated then lognot_p ~size:sz p_value else p_value
        in
        loop T.Ints.(p + p_value) (Ex.union ex ex_value) t bv'
      | { bv = Ext (s, _s_sz, i, j); sz } :: bv' ->
        let p = T.Ints.(p *$$ Z.(one lsl sz)) in
        let (p_value, ex_value), t =
          find_or_init_ext s.value (Extraction.of_ae i j) t
        in
        let p_value =
          if s.negated then lognot_p ~size:sz p_value else p_value
        in
  ...
  let add_bv2nat ~ex nat_r bv_r t =
    ...
    | Atom (x, ofs) ->
      ...
      add ~ex x (Extraction.full ~size:(bitwidth x)) p { t with eqs }
    | Composite (x, ofs) -> (
      ...
      | [{ bv = Other s; sz }] ->
        assert s.negated;
        (* Otherwise we would be an atom *)
        let p = lognot_p ~size:sz p in
        add ~ex s.value (Extraction.full ~size:sz) p t
  ...
  (* Add the equality [bv_r = int2bv(int_r)].
     ...
     [int_r] is equal to [bv2nat(bv)] for some bit-vector [bv]) we introduce the
    ...
    | bv', ext', ex' ->
      ...
      let lit =
        ...
        then
          ...
          Uf.LX.mkv_eq bv_r (T.BV.zero_extend_to sz @@ T.BV.extract bv' ext')
      in
      { t with eqs = (lit, Ex.union ex ex') :: t.eqs }
    | exception Not_found ->
      (* int_r is not known to be a bv2nat bv_r = int2bv(int_r) iff exists k,
         int_r = bv2nat(bv_r) + 2^|bv_r| * k *)
      let k = T.Ints.of_repr @@ X.term_embed @@ E.fresh_name Tint in
      let p_k_2exp = T.Ints.(k *$$ Z.(one lsl sz)) in
      add ~ex bv_r (Extraction.full ~size:sz)
        T.Ints.(of_repr int_r + p_k_2exp)
        t
  ...
  let subst_int ~ex rr nrr t =
    ...
    if not (X.is_a_leaf rr)
    then t
    else
      let nrr_p = Shostak.Arith.embed nrr in
      fold_use_p rr
        (fun p (bv, ext, ex_bv) t ->
    ...
    | Atom (x, ofs) ->
      fold_ext x
        (fun x ext (p, ex_p) t ->
          ...
          let ofs =
            if ext.len < 0
            then Z.shift_right ofs ext.ofs
  ...
  let filter_ty = function Ty.Tint | Tbitv _ -> true | _ -> false
...
module Interval_domain = struct
  ...
  let unknown = function
    | Ty.Tbitv n -> Intervals.Int.of_bounds (Closed Z.zero) (Open Z.(~$1 lsl n))
    | ty ->
...
module ExplainedOrdered (V : Domains_intf.OrderedType) :
  Domains_intf.OrderedType with type t = V.t explained = struct
...
module ExplainedComparable (V : Domains_intf.ComparableType) :
  Domains_intf.ComparableType with type t = V.t explained = struct
...
module Constraint : sig
  ...
  type fun_t = Fbinop of binop * X.r * X.r
  ...
  type rel_t = Rbinrel of binrel * X.r * X.r
  ...
  val compare : t -> t -> int
  (** Comparison function for constraints. The comparison function is arbitrary
      and has no semantic meaning. You should not depend on any of its
      properties, other than it defines an (arbitrary) total order on constraint
      representations. *)
  ...
  let pp_view ppf = function
    | Cfun (r, fn) -> Fmt.(pf ppf "%a =@ %a" (box X.print) r (box pp_fun_t) fn)
    | Crel rel -> pp_rel_t ppf rel

  let equal_view c1 c2 =
    match c1, c2 with
    | Cfun (r1, f1), Cfun (r2, f2) -> X.equal r1 r2 && equal_fun_t f1 f2
    | Cfun _, _ | _, Cfun _ -> false
    | Crel r1, Crel r2 -> equal_rel_t r1 r2
  ...
  let fold_args_fun_t f fn acc =
    match fn with Fbinop (_, x, y) -> f y (f x acc)

  let fold_args_rel_t f r acc =
    match r with Rbinrel (_op, x, y) -> f y (f x acc)
...
module Interval_domains = Domains.Make (BitvNormalForm) (Interval_domain) (EC)
...
module Bitlist_domain = struct
  ...
  let add_offset d cte = Bitlist.logor d (Bitlist.exact cte Explanation.empty)
  ...
  let map_domain f r =
    List.fold_left
      (fun bl { Bitv.bv; sz } ->
        ...
        | Ext (r, r_sz, i, j) -> extract (map_signed r_sz f r) i (j - i + 1))
      (constant Z.zero) (Shostak.Bitv.embed r)
end

module Bitlist_domains = Domains.Make (BitvNormalForm) (Bitlist_domain) (EC)
...
module Propagator : sig
  ...
  (** [simplify c acts] simplifies the constraint [c] by calling appropriate
      ...
      Returns [true] if the constraint has been fully simplified and can be
  ...
  let propagate_binop ~ex sz dx op dy dz =
    ...
    | Band ->
      ...
      (* Reverse propagation for y: if [x = y & z] then: - Any [1] in [x] must
         be a [1] in [y] - Any [0] in [x] that is also a [1] in [z] must be a
         [0] in [y] *)
    ...
    | Bor ->
      ...
      (* Reverse propagation for y: if [x = y | z] then: - Any [0] in [x] must
         be a [0] in [y] - Any [1] in [x] that is also a [0] in [z] must be a
         [1] in [y] *)
    ...
    | Bshl ->
      (* Only forward propagation for now *)
    ...
    | Budiv ->
      (* No bitlist propagation for now *)
      ()
    | Burem ->
      (* No bitlist propagation for now *)
      ()
  ...
  let unwrap msg = function
    ...
    | Intervals_intf.Empty _ -> Util.internal_error "%s" msg
  ...
  let propagate_interval_binrel ~ex op dx dy =
    match op with
    | Rule -> propagate_less_than ~ex ~strict:false dx dy
    | Rugt -> propagate_less_than ~ex ~strict:true dy dx
    | Rdistinct -> propagate_distinct ~ex dx dy

  let propagate_rel_t ~ex dom r =
    let get r = Bitlist_domains.Ephemeral.Canon.entry dom r in
    match r with Rbinrel (op, x, y) -> propagate_binrel ~ex op (get x) (get y)

  let propagate_interval_rel_t ~ex dom r =
    let get r = Interval_domains.Ephemeral.Canon.entry dom r in
    match r with
    | Rbinrel (op, x, y) -> propagate_interval_binrel ~ex op (get x) (get y)
  ...
  let propagate_bitlist dom ~ex c = propagate_view ~ex dom (view c)

  let propagate_interval dom ~ex c = propagate_interval_view ~ex dom (view c)

  let cast ty n =
    match ty with Ty.Tbitv sz -> const sz n | _ -> invalid_arg "cast"
  ...
  let add_eq ~ex acts r x = acts.acts_add_eq ~ex r x

  (* Add the constraint: r = c *)
  let add_eq_const ~ex acts r c = add_eq ~ex acts r @@ const (bitwidth r) c
  ...
  let add_and_const ~ex acts r x c =
    ...
    then (
      ...
      true)
    else if Z.equal c (Z.extract Z.minus_one 0 (bitwidth r))
    then (
      add_eq ~ex acts r x;
      true)
    else false
  ...
  let add_add_const ~ex acts r x c =
    ...
    then (
      add_eq ~ex acts r x;
      true)
    else if X.is_constant r
    then (
      (* c1 = x + c2 -> x = c1 - c2 *)
      add_eq_const ~ex acts x Z.(value r - c);
      true)
    else if Z.testbit c (sz - 1)
    then (
      if
        ...
        Z.popcount c = 1
      then
        (* INT_MIN (2^(sz - 1)) is special because -INT_MIN = INT_MIN and so r =
           x + INT_MIN and x = r + INT_MIN are actually equivalent, so we just
           pick a normalized order between x and r. *)
        if X.hash_cmp r x > 0
        then (
  ...
  let add_shl_const ~ex acts r x c =
    ...
    | n when n < sz ->
      ...
      add_eq_const ~ex acts
        ...
        Z.zero
    | _ | (exception Z.Overflow) -> add_eq_const ~ex acts r Z.zero
  ...
  let eval_binop op ty x y =
    ...
    | Budiv ->
      if Z.equal y Z.zero then cast ty Z.minus_one else cast ty @@ Z.div x y
    | Burem -> if Z.equal y Z.zero then cast ty x else cast ty @@ Z.rem x y
    | Bshl -> (
      match ty, Z.to_int y with
      | Tbitv sz, y when y < sz -> cast ty @@ Z.shift_left x y
      | _ | (exception Z.Overflow) -> cast ty Z.zero)
    | Blshr -> (
      match ty, Z.to_int y with
      | Tbitv sz, y when y < sz -> cast ty @@ Z.shift_right x y
      | _ | (exception Z.Overflow) -> cast ty Z.zero)
  ...
  let rw_binop_const ~ex acts op r x y =
    ...
    match op with
    | Band when X.is_constant x -> add_and_const ~ex acts r y (value x)
    | Band when X.is_constant y -> add_and_const ~ex acts r x (value y)
    | Band -> false
    | Bor when X.is_constant x -> add_or_const ~ex acts r y (value x)
    | Bor when X.is_constant y -> add_or_const ~ex acts r x (value y)
    | Bor -> false
    | Bxor when X.is_constant x -> add_xor_const ~ex acts r y (value x)
    | Bxor when X.is_constant y -> add_xor_const ~ex acts r x (value y)
    | Bxor when X.is_constant r -> add_xor_const ~ex acts x y (value r)
    | Bxor -> false
    | Badd when X.is_constant x -> add_add_const ~ex acts r y (value x)
    | Badd when X.is_constant y -> add_add_const ~ex acts r x (value y)
    | Badd -> false
    | Bmul when X.is_constant x -> add_mul_const ~ex acts r y (value x)
    | Bmul when X.is_constant y -> add_mul_const ~ex acts r x (value y)
    | Bmul -> false
    | Budiv | Burem -> false
  ...
  let rw_binop_algebraic ~ex acts op r x y =
    ...
    | Bxor when X.equal r x ->
      add_eq_const ~ex acts y Z.zero;
      true
    | Bxor when X.equal r y ->
      add_eq_const ~ex acts x Z.zero;
      true
    | Badd when X.equal x y ->
      (* r = x + x -> r = 2x -> r = x << 1 *)
      add_shl_const ~ex acts r x Z.one;
      true
  ...
  let simplify_binop ~ex acts op r x y =
    if X.is_constant x && X.is_constant y
    then (
      add_eq ~ex acts r @@ eval_binop op (X.type_info r) (value x) (value y);
      true)
    else
      rw_binop_const ~ex acts op r x y || rw_binop_algebraic ~ex acts op r x y
  ...
  let simplify uf c acts = simplify_view uf acts (view c)
end
...
let extract_term r terms = if X.is_a_leaf r then SX.add r terms else terms
...
let extract_constraints terms domain int_domain uf r t =
  ...
  | { f = Op op; xs = [x; y]; _ } -> (
    ...
    | Some mk ->
      let rx, exx = Uf.find uf x and ry, exy = Uf.find uf y in
      let c = mk r rx ry in
...
let rec mk_eq ex lhs w z =
  ...
  | Bitv.{ bv = Cte z'; sz } :: rest ->
    ...
    if sz = w then [] else mk_eq ex rest (w - sz) (Z.extract z 0 (w - sz))
  | (Bitv.{ bv = _; sz } as lhs) :: rest ->
    let lhs = Shostak.Bitv.is_mine [lhs] in
    let rhs =
      Shostak.Bitv.is_mine [{ sz; bv = Cte (Z.extract z (w - sz) sz) }]
    in
    if Options.get_debug_bitv ()
    then
      Printer.print_dbg ~module_name:"Bitv_rel" ~function_name:"mk_eq"
...
let add_eqs =
  let rec aux x x_sz acc width bl =
    ...
    then aux x x_sz acc nbits (Bitlist.extract bl 0 nbits)
    else
      ...
      if nbits = 0
      then lits @ acc
      else aux x x_sz (lits @ acc) nbits (Bitlist.extract bl 0 nbits)
  in
  fun acc x x_sz bl -> aux x x_sz acc x_sz bl
...
module Any_propagator : sig
  ...
  let make hashable run = { id = Compat.Type.Id.make (); hashable; run }
...
(* If m and M are the minimal and maximal values of an union of intervals, the
   ...
   Sharpening Constraint Programming approaches for Bit-Vector Theory. Zakaria
   Chihani, Bruno Marre, François Bobot, Sébastien Bardin. CPAIOR 2017.
   International Conference on AI and OR Techniques in Constraint Programming
   for Combinatorial Optimization Problems, Jun 2017, Padova, Italy.
...
let constrain_bitlist_from_interval ~size:sz bv int =
  ...
  if Z.sign distinct >= 0
  then
    let ofs = Z.numbits distinct in
    update ~ex:Ex.empty bv
    @@ Bitlist.(
         exact Z.((inf asr ofs) lsl ofs) (Ex.union inf_ex sup_ex)
         lor extract unknown 0 ofs)
...
let constrain_interval_from_bitlist ~size:sz int bv =
  ...
  let remove ~ex i2 i1 =
    ...
    | NonEmpty i1 -> (
      ...
      | NonEmpty i -> i
      | Empty ex -> raise @@ Interval_domains.Inconsistent ex)
  in
  update ~ex int
  @@ Intervals.Int.fold
       (fun acc i ->
...
let bitlist_structural_propagator =
  ...
  then
    iter_parents r
      (fun p ->
        if X.is_a_leaf p
        then assert (X.equal r p)
        else update p (Bitlist_domain.map_domain get p))
      vars
  else
  ...
  then
    iter_parents r
      (fun p ->
        if X.is_a_leaf p
        then assert (X.equal r p)
        else update p (Interval_domain.map_domain get p))
      vars
  else
    ...
    let j =
      List.fold_left
        (fun j { Bitv.bv; sz } ->
          ...
          | Bitv.Cte z ->
            (* Nothing to update, but still check for consistency *)
            ignore @@ Interval_domain.intersect int (Interval_domain.constant z)
          ...
          | Ext (r, r_size, i, j) ->
            ...
            let hi = Intervals.Int.scale Z.(~$1 lsl Stdlib.(i + sz)) hi in
...
let simplify_all uf eqs touched (dom, idom) =
  ...
  let simplify c c_ex (dom, idom) =
    ...
    in
    let acts_add_eq ~ex u v = acts_add_lit_view ~ex (Uf.LX.mkv_eq u v) in
    let acts_add_constraint ~ex c =
      let c = explained ~ex:(Explanation.union ex c_ex) c in
      HC.replace to_add c.value c.explanation
    in
    let acts = { acts_add_eq; acts_add_constraint } in
    if Propagator.simplify uf c acts
    then
  ...
  ( !eqs,
    HC.fold
      (fun c c_ex (dom, idom) ->
        let c = explained ~ex:c_ex c in
        Constraint.fold_args
          (fun r (dom, idom) ->
            let r, _ = Uf.find_r uf r in
            Bitlist_domains.watch c r dom, Interval_domains.watch c r idom)
          c.value (dom, idom))
      to_add (dom, idom) )
...
(* Holds the queues for propagation.
   ...
   propagators that only use one domain) and for multi-domain propagators (i.e.
   propagators that propagate information across different domains, e.g. between
   bitlists and intervals).
...
let state uf idom bdom =
  ...
  let events constraint_propagator structural_propagator consistency_propagator
      =
    ...
    in
    { Domains_intf.evt_atomic_change; evt_composite_change; evt_watch_trigger }
  in
  let bitlist_events =
    events bitlist_constraint_propagator bitlist_structural_propagator
      interval_from_bitlist_propagator
  and interval_events =
    events interval_constraint_propagator interval_structural_propagator
...
module Schedule = struct
  ...
  type t =
    ...
    | Repeat of t  (** Repeat the given schedule to completion. *)
  ...
  let rec run state schedule =
    ...
    | Sequence schedules ->
      Array.fold_left
        (fun did_run schedule -> run state schedule || did_run)
        false schedules
    | Repeat schedule -> run_repeatedly state schedule false

  and run_repeatedly state schedule did_run =
    if run state schedule then run_repeatedly state schedule true else did_run
...
let rec propagate_all uf eqs bdom idom =
  ...
  then (
    ...
    (* We run all propagations over a single domain to completion, then run the
       consistency propagators to perform cross-domain propagations. *)
    ignore
      Schedule.(
        run state @@ repeat
        @@ sequence
...
let empty uf =
  ( { terms = SX.empty; size_splits = Q.one },
...
let assume env uf la =
  ...
  let int_domain = Uf.GlobalDomains.find (module Interval_domains) ds in
  let domain, int_domain, eqs, size_splits =
    try
      let domain, int_domain, eqs, size_splits =
        List.fold_left
          (fun (domain, int_domain, eqs, ss) (a, _root, ex, orig) ->
            let ss =
              match orig with Th_util.CS (Th_bitv, n) -> Q.(ss * n) | _ -> ss
            in
            match a, orig with
            | L.Builtin (is_true, BVULE, [x; y]), _ ->
              ...
              let c =
                if is_true then Constraint.bvule x y else Constraint.bvugt x y
              in
      ...
      if Options.get_debug_bitv () && not (Compat.List.is_empty eqs)
      then (
        Printer.print_dbg ~module_name:"Bitv_rel" ~function_name:"assume"
  ...
  in
  let result =
    { Sig_rel.assume = List.rev_append bvconv_assume assume; remove = [] }
  in
...
let case_split env uf ~for_model =
  if (not for_model) && Stdlib.(env.size_splits >= Options.get_max_split ())
  then []
  else
    let bl_domain =
    ...
    let f_acc r acc =
      ...
      then acc
        ...
        | Some (nunk', xs) when nunk = nunk' -> Some (nunk', SX.add r xs)
    ...
    | r ->
      ...
      let r =
        ...
        try Uf.make uf (Expr.Set.choose es) with Not_found -> r
      in
      let w = bitwidth r in
      ...
      in
      (* Make sure to pick a value that is compatible with the interval domain.

         This should not happen if cross-domain propagation was run to
         completion, which it should do, but it is OK if there are corner cases
         where it didn't (we are just going to be slower), so we want to handle
         this case.
      ...
      if Options.get_debug_bitv ()
      then
        Printer.print_dbg ~module_name:"Bitv_rel" ~function_name:"case_split"
          "[BV-CS-1] Setting %a to %a" X.print lhs X.print value;
      [Uf.LX.mkv_eq lhs value, true, Th_util.CS (Th_util.Th_bitv, Q.of_int size)]
    | exception Not_found -> []
...
let add env uf r t =
  ...
  | Tint -> (
    ...
    | { f = Op BV2Nat; xs = [x]; _ } ->
      ...
      env, Uf.GlobalDomains.add (module BV2Nat) bvconv ds, eqs
    | _ -> env, ds, [])
  | Tbitv _ -> (
    match E.term_view t with
...
let optimizing_objective _env uf Objective.Function.{ e; is_max; _ } =
  ...
  then None
    ...
    | [{ bv = Cte n; sz }] ->
      ...
      then Printer.print_dbg "%a has the value %a@." E.print e Z.pp_print n;
    ...
    | _ ->
      ...
      let value_z =
        if is_max
        then finite_upper_bound ~size:sz @@ fst (Intervals.Int.upper_bound int)
        else finite_lower_bound @@ fst (Intervals.Int.lower_bound int)
      in
...
let assume_th_elt t th_elt _ =
  ...
  | Util.Bitv -> failwith "This Theory does not support theories extension"
  | _ -> t

module Test = struct
  let shared_msb sz inf sup = sz - Z.numbits (Z.logxor inf sup)
end
