...
module EM = Matching.Make (struct
  ...
  let are_equal env s t ~init_terms:_ = Uf.are_equal env s t ~added_terms:false
...
type t =
  { inequations : P.t Inequalities.t MPL.t;
    ...
    improved_x : SX.t;
    classes : SE.t list;
    (* State of the union-find represented by all its equivalence classes. This
       state is kept for debugging purposes only. It is updated after assuming
       literals of the theory and returned by queries in case of
       inconsistency. *)
    size_splits : Q.t;
    int_sim : Sim.Core.t;
    rat_sim : Sim.Core.t;
...
module Sim_Wrap = struct
  let check_unsat_result simplex env =
    ...
    | Sim.Core.Unsat ex ->
      ...
      if get_debug_fm ()
      then
        Printer.print_dbg ~module_name:"IntervalCalculus"
          ~function_name:"check_unsat_result" "simplex derived unsat: %a"
  ...
  let solve env i =
    Timers.with_timer Timers.M_Simplex Timers.F_solve @@ fun () -> solve env i
  ...
  let extract_bound i get_lb =
    ...
    try
      ...
      let bvalue = if large then Sim.Core.R2.of_r bnd else q bnd in
  ...
  let add_if_better p _old _new simplex =
    ...
    if same_bnds old_mn new_mn && same_bnds old_mx new_mx
    then simplex
    else
      ...
      let simplex, _changed =
        ...
        | [(c, x)] ->
          assert (Q.is_one c);
          Sim.(Assert.var simplex x ?min:new_mn ?max:new_mx)
        | _ ->
          let l = List.rev_map (fun (c, x) -> x, c) l in
          Sim.(
            Assert.poly simplex (Core.P.from_list l) (alien_of p) ?min:new_mn
              ?max:new_mx)
      in
  ...
  let case_split =
    let gen_cs x n s orig =
      ...
      then
        Printer.print_dbg ~module_name:"IntervalCalculus"
          ~function_name:"case_split" "[Sim_CS-%d] %a = %a of size %a" orig
          X.print x Q.print n Q.print s;
    ...
    let aux_1 uf x (info, _) acc =
      ...
      | Some s
        when (Sim.Core.equals_optimum info.Sim.Core.value info.Sim.Core.mini
    ...
    fun env uf ->
      ...
      let acc = Sim.Core.MX.fold (aux_1 uf) int_sim.Sim.Core.non_basic None in
      let acc = Sim.Core.MX.fold (aux_1 uf) int_sim.Sim.Core.basic acc in
      match acc with
      ...
      | None -> (
        ...
        in
        let acc = Sim.Core.MX.fold (aux_2 env uf) int_sim.Sim.Core.basic acc in
        match acc with Some (x, n, s) -> gen_cs x n.v s 2 | None -> [])
  ...
  let infer_best_bounds env p =
    let best_bnd lp ~upper sim i set_new_bound =
      ...
      | Sim.Core.Max (mx, _sol) ->
        ...
        set_new_bound max_v.explanation (Q.mult q max_v.bvalue.v) ~is_le i
    in
    if get_debug_fpa () >= 2
    then
      Printer.print_dbg ~module_name:"IntervalCalculus"
        ~function_name:"infer_best_bounds" "#infer bounds for %a" P.print p;
    ...
    then
      Printer.print_dbg ~module_name:"IntervalCalculus"
        ~function_name:"infer_best_bounds" "## inferred bounds for %a: %a"
        P.print p I.print i;
    i
end
...
module MP = struct
  ...
  let assert_normalized_poly p =
    assert (
      ...
      Printer.print_err ~error:(not b)
        "[IC.assert_normalized_poly] %a is not normalized" P.print p;
      b)
  ...
  let n_add p i old ({ polynomes; _ } as env) =
    ...
    if I.is_strict_smaller i old || not (MP0.mem p polynomes)
    then
      let ty = P.type_info p in
  ...
  (* shadow the functions find and add of MP with the ones below to force the
     use of n_find and n_add for normalized polys *)
  [@@@ocaml.warning "-32"]
...
module MX = struct
  ...
  let assert_is_alien x =
    assert (
      let b = is_alien x in
      Printer.print_err ~error:(not b) "[IC.assert_is_alien] %a is not an alien"
        X.print x;
      b)
  ...
  let n_add x ((i, _) as e) old ({ monomes; _ } as env) =
    (*NB: adding a new entry into the map is considered as an improvement*)
    assert_is_alien x;
    if I.is_strict_smaller i old || not (MX0.mem x monomes)
    then
      let ty = X.type_info x in
  ...
  (* shadow the functions find and add of MX with the ones below to force the
     use of n_find and n_add for true aliens *)
  [@@@ocaml.warning "-32"]
...
(* generic find for values that may be non-alien or non normalized polys *)
let generic_find ?(only_inf = false) xp env =
  let is_mon = is_alien xp in
  if get_debug_fm ()
  then
    Printer.print_dbg ~flushed:false ~module_name:"IntervalCalculus"
      ~function_name:"generic_find" "Searching %s for: %a ... "
      (if only_inf then "a lower bound" else "an interval")
      X.print xp;
  try
    if not is_mon then raise Not_found;
    let i, use = MX.n_find xp env.monomes in
    if get_debug_fm () then Printer.print_dbg ~header:false "found";
    i, use, is_mon
  with Not_found ->
    (* according to this implem, it means that we can find aliens in polys but
       not in monomes. FIX THIS => an interval of x in monomes and in polys may
       be differents !!! *)
    if get_debug_fm ()
    then Printer.print_dbg ~flushed:false ~header:false "not_found@ ";
    ...
    then
      Printer.print_dbg ~flushed:false ~header:false
        "normalized into %a + %a * ( %a )@ " Q.print c Q.print d P.print p;
    assert (Q.sign d <> 0 && Q.sign c0 = 0);
    let ty = P.type_info p0 in
    let ip = try MP.n_find p env.polynomes with Not_found -> I.undefined ty in
    (* In some cases when this function is called, the caller only care about
       ...
       interval is not consistent/reachable/satisfiable. This interacts badly
       with callers who only care about the lower bound, as these callers do not
       catch the inconsistent exception, and this can lead to the solver
       returning Unsat/Valid even when it is incorrect to do so. (see issues
       #460 #340, and PRs #342 and #465). *)
    let ip =
      if only_inf
      then if Q.sign d >= 0 then I.only_borne_inf ip else I.only_borne_sup ip
      else ip
    in
    if get_debug_fm ()
    then
    ...
    if get_debug_fm ()
    then
      Printer.print_dbg ~header:false "scaled %a into %a" I.print ip I.print
        new_ip;
    new_ip, SX.empty, is_mon

(* generic add for values that may be non-alien or non normalized polys *)
...
module Debug = struct
  ...
  let add p =
    if get_debug_fm ()
    then
      print_dbg ~module_name:"IntervalCalculus" ~function_name:"add"
        "@[<v 2>New polynome added: %a]" P.print p

  let assume ~query a expl =
    if get_debug_fm ()
    then
      print_dbg ~module_name:"IntervalCalculus" ~function_name:"assume"
        "@[<v 2>%s We assume: %a@,explanations: %a@]"
        (if query then "[query]" else "")
        LR.print (LR.make a) Explanation.print expl

  let print_use fmt use = SX.iter (fprintf fmt "%a, " X.print) use

  let env env =
    if get_debug_fm ()
    then begin
      print_dbg ~module_name:"IntervalCalculus" ~function_name:"env"
        "@ ------------ FM: inequations-------------------------@ ";
      MPL.iter
        (fun a { ple0 = p; is_le; _ } ->
          print_dbg ~flushed:false ~header:false "%a%s0  |  %a@ " P.print p
            (if is_le then "<=" else "<")
            E.print a)
        env.inequations;
      ...
      MX.iter
        (fun x (i, use) ->
          print_dbg ~flushed:false ~header:false "%a : %a |-use-> {%a}@ "
      ...
      MP.iter
        (fun p i ->
          print_dbg ~flushed:false ~header:false "%a : %a@ " P.print p I.print i)
        env.polynomes;
  ...
  let implied_equalities l =
    ...
    then
      ...
      let print fmt (ra, ex, _) =
        fprintf fmt "@,%a %a" pp_literal ra Explanation.print ex
      in
      print_dbg ~module_name:"IntervalCalculus"
        ~function_name:"implied_equalities" "@[<v 2> %d implied equalities %a@]"
        (List.length l) (pp_list_no_space print) l

  let case_split r1 r2 =
    if get_debug_fm ()
    then
      print_dbg ~module_name:"IntervalCalculus" ~function_name:"case-split"
  ...
  let inconsistent_interval expl =
    ...
    then
      print_dbg ~module_name:"IntervalCalculus"
        ~function_name:"inconsistent_interval" "interval inconsistent %a"
  ...
  let added_inequation kind ineq =
    ...
    then begin
      print_dbg ~module_name:"IntervalCalculus"
        ~function_name:"added_inequation"
        "I derived the (%s) inequality: %a %s 0@ " kind P.print ineq.ple0
        (if ineq.is_le then "<=" else "<");
      print_dbg ~flushed:false ~header:false "from the following combination:@ ";
      Util.MI.iter
        (fun _a (coef, ple0, is_le) ->
          print_dbg ~flushed:false ~header:false "\t%a * (%a %s 0) +" Q.print
            coef P.print ple0
            (if is_le then "<=" else "<"))
        ineq.dep;
  ...
  let tighten_interval_modulo_eq_begin p1 p2 =
    ...
    then
      print_dbg ~module_name:"IntervalCalculus"
        ...
        "tighten intervals modulo eq: %a = %a" P.print p1 P.print p2
  ...
  let tighten_interval_modulo_eq_middle p1 p2 i1 i2 j =
    ...
    then
      print_dbg ~module_name:"IntervalCalculus"
        ...
        "%a has interval %a@ %a has interval %a@ intersection is %a" P.print p1
        I.print i1 P.print p2 I.print i2 I.print j
  ...
  let tighten_interval_modulo_eq_end p1 p2 b1 b2 =
    ...
    then begin
      ...
      then
        print_dbg ~module_name:"IntervalCalculus"
          ~function_name:"tighten_interval_modulo_eq_end" "> no improvement"
    end
end
(*BISECT-IGNORE-END*)
...
let calc_pow a b ty uf =
  ...
  try
    ...
    | Some c_y ->
      let res =
        (* x ** 1 -> x *)
        if Q.equal c_y Q.one
        then ra (* x ** 0 -> 1 *)
        else if Q.equal c_y Q.zero
        then alien_of (P.create [] Q.one ty)
        else
...
let empty uf =
  ( { inequations = MPL.empty;
      ...
      size_splits = Q.one;
      new_uf = Uf.empty;
      rat_sim = Sim.Solve.solve (Sim.Core.empty ~is_int:false ~check_invs:false);
      int_sim = Sim.Solve.solve (Sim.Core.empty ~is_int:true ~check_invs:false);
      th_axioms = ME.empty;
      linear_dep = ME.empty;
      syntactic_matching = []
    },
...
(*let up_improved env p oldi newi = if I.is_strict_smaller newi oldi then { env
  with improved = SP.add p env.improved } else env*)

(** computes an interval for vars = x_1^n_1 * ..... * x_i^n_i (1) if some var is
    not in monomes, then return undefined (2) check that all vars are in monomes
    before doing interval ops **)
let mult_bornes_vars vars env ty =
  try
    let l =
      List.rev_map
        (fun (y, n) ->
          let i, _, _ = generic_find y env in
          i, n)
        vars
    in
    List.fold_left
      (fun ui (yi, n) -> I.mult ui (I.power n yi))
      (I.point Q.one ty Explanation.empty)
      l
  with Not_found -> I.undefined ty

(** computes the interval of a polynome from those of its monomes. The monomes
    are supposed to be already added in env **)
let intervals_from_monomes ?(monomes_inited = true) env p =
  let pl, v = P.to_list p in
  (* in the case of integer polynomes, ensure that we only round once at the
     end, and not at every add/scale operation on intervals. *)
  let rational_interval =
    List.fold_left
      ...
      (I.point v Ty.Treal Explanation.empty)
      pl
  in
  I.coerce (P.type_info p) rational_interval

(* because, it's not sufficient to look in the interval that corresponds to the
...
let rec init_monomes_of_poly are_eq env p use_p expl =
  List.fold_left
    (fun env (_, x) ->
      ...
      with Not_found -> update_monome are_eq expl use_p env x)
    env
    (fst (P.to_list p))
...
and update_monome are_eq expl use_x env x =
    ...
    | _ -> (
      ...
      | Some t, _ ->
        ...
        | { E.f = Sy.Op Sy.Div; xs = [a; b]; _ } ->
          ...
          let env, ia = init_alien are_eq expl pa npa use_x env in
          let ia = I.add_explanation ia ea in
          (* take repr into account*)
          let env, ib = init_alien are_eq expl pb npb use_x env in
          let ib = I.add_explanation ib eb in
          (* take repr into account*)
          let ia, ib =
            match cannot_be_equal_to_zero env pb ib with
            | Entailed { ex; _ } when Q.equal ca cb && P.compare pa' pb' = 0 ->
            ...
            | Entailed { ex; _ } -> begin
              ...
              | Th_util.Entailed { ex = ex_eq; _ } ->
                ...
                I.point Q.one ty expl, I.point Q.one ty expl
              | Unknown -> ia, ib
            end
            | Unknown -> ia, ib
          in
  ...
  let u, use_x' =
    try MX.n_find x env.monomes
    with Not_found -> I.undefined (X.type_info x), use_x
  in
...
let rec tighten_ac are_eq x env expl =
  let ty = X.type_info x in
  let u, _use_x =
    try MX.n_find x env.monomes with Not_found -> I.undefined ty, SX.empty
  in
  try
    match X.ac_extract x with
    | Some { h; l = [(x, n)]; _ }
      ...
      env
    | Some { h; l = [(x, n)]; _ }
      when Symbols.equal h (Symbols.Op Symbols.Mult) && n > 2 ->
      let env =
        if is_alien x
        then
          let u = I.coerce ty (I.root n (I.coerce Treal u)) in
    ...
    (fun env (a, x) ->
      ...
      try
        ...
        let new_ix =
          I.scale (Q.inv a)
            (I.add i
               (I.affine_scale ~coef:(Q.minus d)
                  ~const:(Q.mult (Q.minus d) c)
                  inp))
        in
...
let update_polynomes_intervals env =
  MP.fold
    (fun p ip env ->
      ...
      if I.is_strict_smaller i ip
      then update_monomes_from_poly p i (MP.n_add p i ip env)
      else env)
    env.polynomes env
...
let find_one_eq x u =
  ...
  | Some (v, ex) when X.type_info x != Ty.Tint || Q.is_int v ->
    let eq = LR.mkv_eq x (alien_of (P.create [] v (X.type_info x))) in
...
let find_eq eqs x u env =
  ...
  | Some eq1 -> begin
    match X.ac_extract x with
    | Some { h; l = [(y, _)]; _ } when Symbols.equal h (Symbols.Op Symbols.Mult)
      ->
      let neweqs =
        try
...
let ineq_status { ple0 = p; is_le; _ } =
  ...
  | None ->
    ...
    then
      ...
      let c =
        Q.sign v
        (* equiv. to compare v Q.zero *)
      in
      if c > 0 || (c >= 0 && not is_le)
      then Bottom
      else if c = 0 && is_le
      then Trivial_eq
      else Trivial_ineq v
...
(*let ineqs_from_dep dep borne_inf is_le = List.map (fun {poly_orig = p; coef =
  c} -> let (m,v,ty) = P.mult_const minusone p in (* quelle valeur pour le
...
let fm_equalities eqs { dep; expl = ex; _ } =
  Util.MI.fold
    (fun _ (_, p, _) eqs -> (mk_equality p, None, ex, Th_util.Other) :: eqs)
    dep eqs
...
let update_intervals are_eq env eqs expl (a, x, v) is_le =
  ...
  let u =
    if Q.sign a > 0
    then I.new_borne_sup expl b ~is_le uints
    else I.new_borne_inf expl b ~is_le uints
  in
...
let update_ple0 are_eq env p0 is_le expl =
  ...
  then env
    ...
    let p, change =
      if Q.sign a < 0 then P.mult_const Q.m_one p0, true else p0, false
    in
    let p, c, _ = P.normal_form p in
    let c = Q.minus c in
    let u =
      if change
      then I.new_borne_inf expl c ~is_le (I.undefined ty)
      else I.new_borne_sup expl c ~is_le (I.undefined ty)
    in
    let u, pu =
      try
    ...
    in
    let env =
      if I.is_strict_smaller u pu
      then update_monomes_from_poly p u (MP.n_add p u pu env)
      else env
    in
    match P.to_list p0 with
    | [(a, x)], v -> fst (update_intervals are_eq env [] expl (a, x, v) is_le)
    | _ -> env
...
let register_relationship c x pi expl (x_rels, p_rels) =
  let x_rels =
    ...
    assert (s <> 0);
    let low, up =
      try MX0.find x x_rels with Not_found -> MP0.empty, MP0.empty
    in
    let v =
      if s < 0
      then MP0.add pi a low, up (* low_bnd(pi) / (-c) is a low_bnd of x *)
      else low, MP0.add pi a up (* low_bnd(pi) / (-c) is an up_bnd of x *)
    in
    MX0.add x v x_rels
  in
  let p_rels =
    ...
    assert (s <> 0);
    let low, up =
      try MP0.find p0 p_rels with Not_found -> MX0.empty, MX0.empty
    in
    let w =
      if s < 0
      then
  ...
  List.fold_left
    (fun ((env, eqs, rels) as acc) ineq ->
      ...
      | Monome (a, x, v) ->
        ...
        let env, eqs =
          update_intervals are_eq env eqs expl (a, x, v) ineq.is_le
        in
        env, eqs, rels
      | Other -> (
        match x_opt with
...
let split_problem current_age env ineqs aliens =
  let l, all_lvs =
    List.fold_left
      (fun (acc, all_lvs) ({ ple0 = p; _ } as ineq) ->
        ...
        | Bottom -> raise (Ex.Inconsistent (ineq.expl, env.classes))
        | _ ->
          let lvs =
      ...
      (fun x l ->
        ...
        | e :: lx ->
          let elx =
            List.fold_left
              (fun (l, lvs) (l', lvs') ->
                List.rev_append l l', SX.union lvs lvs')
              e lx
          in
          elx :: l_nx)
      all_lvs l
  ...
  let ll =
    List.filter
      (fun (ineqs, _) ->
        List.exists (fun ineq -> Z.equal current_age ineq.age) ineqs)
      ll
  in
...
let is_normalized_poly uf p =
  ...
  then true
    ...
    Printer.print_err "%a <= 0 NOT normalized@,It is equal to %a" X.print p
      X.print rp;
    false
  end
...
let better_bound_from_intervals env ({ ple0; is_le; dep; _ } as v) =
  ...
  then
    ...
    | true, false ->
      (* better bound, Large ineq becomes Strict *)
      { v with
        ple0 = new_p;
        expl;
        is_le = false;
...
let update_linear_dep env rclass_of ineqs =
  ...
  in
  let linear_dep =
    SE.fold (fun t linear_dep -> ME.add t terms linear_dep) terms env.linear_dep
  in
  { env with linear_dep }
...
let refine_x_bounds ix env rels is_low =
  MP.fold
    (fun p (m_cx, ineq_ex) ix ->
      try
        (* recall (construction of x_rels): -> is_low : low_bnd(pi) / (-c) is a
           low_bnd of x -> not is_low : low_bnd(pi) / (-c) is an up_bnd of x *)
...
let monomes_relational_deductions env x_rels =
  MX.fold
    (fun x (low, up) env ->
      let ix0, use_x =
        try MX.n_find x env.monomes with Not_found -> assert false
      in
      let ix = refine_x_bounds ix0 env low true in
      let ix = refine_x_bounds ix env up false in
      if I.is_strict_smaller ix ix0 then MX.n_add x (ix, use_x) ix0 env else env)
    x_rels env
...
let refine_p_bounds ip _p env rels is_low =
  MX.fold
    (fun x (cx, mc0, md0, ineq_ex) ip ->
      try
        (* recall (construction of p_rels): -> is_low : low_bnd(c*x) / (-d0) +
           (-c0) is a low_bnd of p0 -> not is_low : low_bnd(c*x) / (-d0) + (-c0)
           is an up_bnd of p0 where p = (p0 + c0) * d0 and c*x + p <= 0 *)
        assert (is_low == (Q.sign md0 > 0));
        let ix, _ =
          try MX.n_find x env.monomes with Not_found -> raise Exit
        in
        let bx, ex_b, is_le =
          (if Q.sign cx > 0 then I.borne_inf else I.borne_sup) ix
        in
...
let polynomes_relational_deductions env p_rels =
  MP.fold
    (fun p0 (low, up) env ->
      ...
      if not (MP.mem p0 env.polynomes || MX.mem xp env.monomes)
      then env
      else
        ...
        if I.is_strict_smaller ip ip0
        then
          if is_mon
          then MX.n_add xp (ip, use) ip0 env
...
let fm (module Oracle : S with type p = P.t) uf are_eq rclass_of env eqs =
  if get_debug_fm ()
  then
    Printer.print_dbg ~module_name:"IntervalCalculus" ~function_name:"fm"
      "in fm/fm-simplex";
...
let add_disequality are_eq env eqs p expl =
  ...
  | [], v ->
    if Q.sign v = 0 then raise (Ex.Inconsistent (expl, env.classes));
    env, eqs
  | [(a, x)], v ->
  ...
  | _ ->
    ...
    let i2 = try MP.n_find p env.polynomes with Not_found -> I.undefined ty in
    let i = I.exclude ~ex:expl (Q.minus c) i2 in
    let env =
      if I.is_strict_smaller i i2
      then update_monomes_from_poly p i (MP.n_add p i i2 env)
      else env
    in
    env, eqs
...
let add_equality are_eq env eqs p expl =
  ...
  | _ ->
    ...
    in
    let env = { env with known_eqs = SX.add (alien_of p) env.known_eqs } in
    env, eqs

let normal_form a =
  match a with
  | L.Builtin (false, L.LE, [r1; r2]) when X.type_info r1 == Ty.Tint ->
    let pred_r1 = P.sub (poly_of r1) (P.create [] Q.one Ty.Tint) in
    LR.mkv_builtin true L.LE [r2; alien_of pred_r1]
  | L.Builtin (true, L.LT, [r1; r2]) when X.type_info r1 == Ty.Tint ->
    let pred_r2 = P.sub (poly_of r2) (P.create [] Q.one Ty.Tint) in
    LR.mkv_builtin true L.LE [r1; alien_of pred_r2]
  | L.Builtin (false, L.LE, [r1; r2]) -> LR.mkv_builtin true L.LT [r2; r1]
  | L.Builtin (false, L.LT, [r1; r2]) -> LR.mkv_builtin true L.LE [r2; r1]
  | _ -> a
...
let tighten_eq_bounds env r1 r2 p1 p2 origin_eq expl =
  if P.is_const p1 != None || P.is_const p2 != None
  then env
  else
    ...
    | Th_util.Subst | Th_util.Other ->
      ...
      let env = if impr_i1 then generic_add r1 j us1 is_mon_1 env else env in
...
let assume ~query env uf la =
      ...
      (fun ((env, eqs, new_ineqs, rm) as acc) (a, root, expl, orig) ->
        ...
        try
          ...
          | L.Builtin (_, ((L.LE | L.LT) as n), [r1; r2]) ->
            ...
            let ineq = Oracle.create_ineq p1 p2 (n == L.LE) root expl in
            begin match ineq_status ineq with
            ...
            | Monome _ | Other ->
              let env =
                init_monomes_of_poly are_eq env ineq.ple0 SX.empty
                  Explanation.empty
              in
              let env = update_ple0 are_eq env ineq.ple0 (n == L.LE) expl in
  ...
  try
    ...
    if !nb_num = 0 || query
    then env, { Sig_rel.assume = []; remove = to_remove }
    else
      (* we only call fm when new ineqs are assumed *)
      let env, eqs =
        if new_ineqs && not (Options.get_no_fm ())
        then fm (module Oracle) uf are_eq rclass_of env eqs
        else env, eqs
      in
      let env = Sim_Wrap.solve env 1 in
...
let query env uf a_ex =
  try
    ...
    Th_util.Unknown
  with Ex.Inconsistent (ex, classes) -> Entailed { ex; classes }
  ...
  let o =
    MP.fold
      (fun p i o ->
        ...
        | Some s when Q.compare s Q.one > 0 -> begin
          match o with
...
let check_size for_model env res =
  ...
  then res
    ...
    | [(_, _, Th_util.CS (Th_util.Th_arith, s))] ->
      if
        Numbers.Q.compare (Q.mult s env.size_splits) (get_max_split ()) <= 0
        || Numbers.Q.sign (get_max_split ()) < 0
      then res
      else []
    | _ -> assert false
...
let default_case_split env uf ~for_model =
  ...
  | [] -> begin
    ...
    match check_size for_model env cs1, check_size for_model env cs2 with
    | [], cs | cs, [] -> cs
...
let add =
  let are_eq t1 t2 =
    if E.equal t1 t2
    then Th_util.Entailed { ex = Explanation.empty; classes = [] }
    else Unknown
  in
  fun env new_uf r t ->
    Debug.env env;
    try
      ...
      then
        let env =
          init_monomes_of_poly are_eq env p SX.empty Explanation.empty
        in
        let delayed, eqs = Rel_utils.Delayed.add env.delayed env.new_uf r t in
...
(* let extract_improved env = SP.fold (fun p acc -> MP.add p (MP.find p
   env.polynomes) acc) env.improved MP.empty *)

let new_terms _ = SE.empty

let case_split_union_of_intervals =
  let aux acc uf i z =
    if Uf.is_normalized uf z
    then
      ignore
      @@ I.fold
           (fun prev { lb = _; ub } ->
  ...
  fun env uf ->
    ...
    with Exit -> (
      ...
      | Some (r1, (pred, n)) ->
        ...
        let r2 = alien_of (P.create [] n ty) in
        [ ( LR.mkv_builtin true pred [r1; r2],
            true,
...
let int_constraints_from_map_intervals =
  let aux p xp i uf acc =
    if
      Uf.is_normalized uf xp && I.is_point i == None && P.type_info p == Ty.Tint
...
let model_from_simplex sim is_int env uf =
  ...
  | Sim.Core.Unsat ex ->
    (* when splitting on union of intervals, FM does not include related ineqs
       when crossing. So, we may miss some bounds/deductions, and FM-Simplex may
       fail to find a model *)
    raise (Ex.Inconsistent (Lazy.force ex, env.classes))
  | Sim.Core.Sat sol ->
    let { Sim.Core.main_vars; slake_vars; int_sol; epsilon = _ } =
      Lazy.force sol
    in
    let main_vars, _slake_vars =
      if int_sol || not is_int
    ...
    List.fold_left
      (fun acc (v, q) ->
        ...
        if SX.mem v env.known_eqs || not (Uf.is_normalized uf v)
        then
          ...
          then
            Printer.print_dbg ~module_name:"IntervalCalculus"
              ~function_name:"model_from_simplex" "[%s simplex] %a = %a"
...
let model_from_infinite_domains =
  let mk_cs acc (x, v, _ex) =
    (LR.view (LR.mk_eq x v), true, Th_util.CS (Th_util.Th_arith, Q.from_int 2))
    :: acc
  in
  fun env uf ->
    ...
    (* reuse existing rat_sim *)
    let int_sim =
      (* create a new int_sim with FM-Simplex encoding *)
...
(* Helper function used in [optimizing_objective] to pick a value for the
   polynomial [p] in its interval. We use this function to split the search
   space and bias it towards the optimum in the case the value produced by the
   optimization procedure doesn't satisfy some constraints that involve strict
   inequalities or the problem is unbounded. *)
let middle_value env ~is_max ty p bound =
  let interval =
    match MP0.find_opt p env.polynomes, bound with
    | Some i, Some bound -> begin
      try
        if is_max
        then I.new_borne_sup Ex.empty bound ~is_le:false i
        else I.new_borne_inf Ex.empty bound ~is_le:false i
...
let optimizing_objective_for_ty ~is_max ty env uf e =
  (* soundness: if there are expressions to optmize, this should be done without
     waiting for ~for_model flag to be true *)
  let repr, _ = Uf.find uf e in
  let r1 = Uf.make uf e in
  (* instead of repr, which may be a constant *)
  let p = poly_of repr in
  match P.is_const p with
  | Some optim ->
    if Options.get_debug_optimize ()
    then Printer.print_dbg "%a has the value %a@." E.print e Q.print optim;
  ...
  | None -> begin
    ...
    | Sim.Core.Unknown ->
      ...
      assert false
    | Sim.Core.Sat _ ->
      (* This answer is only returned by OcplibSimplex if we only want to check
         the satisfability of the linear program. *)
      assert false
    | Sim.Core.Unsat _ ->
    ...
    | Sim.Core.Unbounded _ ->
      let value =
        if is_max then Objective.Value.Pinfinity else Objective.Value.Minfinity
      in
      (* As the problem is unbounded, we need to produce a value for the
         ...
         constraint to let the case split mechanism pick a "large" (or "small")
         value in the interval (otherwise, later objectives are computed while
         assuming that the middle value is forced, which is incorrect). *)
      let case_split =
        ( LR.mkv_builtin false LT [r1; middle_value env ~is_max ty p None],
          true,
          Th_util.CS (Th_util.Th_arith, Q.one) )
      in
      Th_util.{ value; case_split }
    | Sim.Core.Max ((lazy Sim.Core.{ max_v; is_le }), _sol) ->
      ...
      then begin
        ...
        then
          Printer.print_dbg "%a has a %s: %a@." Expr.print e
            (if is_max then "maximum" else "minimum")
            Q.print optim
        else
          Printer.print_dbg "%a is a %s bound of %a" Q.print optim
            (if is_max then "upper" else "lower")
            Expr.print e
      end;
      let r2 = alien_of (P.create [] optim ty) in
      Debug.case_split r1 r2;
      ...
      let value =
        if is_le
        then Objective.Value.Value t2
        else begin
          if is_max
          then Objective.Value.Limit (Below, t2)
      ...
      let r2 =
        ...
        then r2
          ...
          (* As some bounds are strict, the value [r2] does not satisfy the
             constraints of the problem. In this case, we pick a value in the
             domain of the polynomial [p]. But we propagate the new value
             [value] for this objective. Indeed, we want to be able to print it
             while using the statement [get-objective]. *)
          let q =
...
let best_interval_of optimized env p =
  ...
  | None -> (
    let i =
      try
        let i, _, _ = generic_find (alien_of p) env in
        i
      with Not_found -> I.undefined (P.type_info p)
    in
    if SP.mem p !optimized
    then env, i
    else
      try
        ...
        if not (I.is_strict_smaller k i)
        then env, i
        else
          let env = MP.n_add p k i env in
          Sim_Wrap.solve env 1, k
      with I.NotConsistent expl ->
        if true (*get_debug_fpa() >= 2*)
        then begin[@ocaml.ppwarning
                    "TODO: find an example triggering this case!"]
          Printer.print_wrn "TODO: should check that this is correct !!!";
...
let extend_with_domain_substitution =
      ...
      (fun v_hs (lv, uv, ty) sbt ->
        ...
        then sbt
          ...
          let lb_val =
            ...
            | Some (q1, _), Some (q2, _) ->
              Printer.print_err
                "[Error] %a <= %a <= %a@,Which value should we choose?" Q.print
                q1 Var.print lb_var Q.print q2;
  ...
  fun (sbt, sbty) idoms ->
    ...
    with Exit ->
      if get_debug_fpa () >= 2
      then Printer.print_dbg "extend_with_domain_substitution failed!";
      None
      ...
      List.fold_left
        (fun (idoms, maps_to, env, uf) s ->
          ...
          | E.NotTheoryConst t ->
            ...
            let uf, _ = Uf.add uf tt in
            if
              X.leaves (fst (Uf.find uf tt)) == []
              || X.leaves (fst (X.make tt)) == []
            then raise (Sem_match_fails env);
          ...
          | E.LinearDependency (x, y) ->
            ...
            if not (terms_linear_dep env [x; y])
            then raise (Sem_match_fails env);
            let uf, _ = Uf.add uf x in
            let uf, _ = Uf.add uf y in
            idoms, maps_to, env, uf)
        (Var.Map.empty, [], env, uf)
        tr.E.semantic
    in
...
let semantic_matching lem_name tr sbt env uf optimized =
  ...
  | env, None -> env, None
  | env, Some (idom, mapsTo) -> begin
    match extend_with_domain_substitution sbt idom with
    | None -> env, None
...
let record_this_instance f accepted lorig =
  ...
  then
    ...
    | E.Unit _ | E.Clause _ | E.Literal _ | E.Skolem _ | E.Let _ | E.Iff _
    | E.Xor _ ->
      assert false
...
let profile_produced_terms menv lorig nf s trs =
  ...
  then
    let st0 =
      List.fold_left
        (fun st t -> E.sub_terms st (E.apply_subst s t))
        SE.empty trs
    in
    let name, loc, _ =
      match E.form_view lorig with
...
let new_facts_for_axiom ~do_syntactic_matching menv uf selector optimized substs
    accu =
  List.fold_left
    (fun acc
         ( { trigger_formula = f;
      ...
      List.fold_left
        (fun (env, acc)
          ...
          (* Here, we'll try to extends subst 's' to conver variables appearing
             in semantic triggers *)
          ...
          then
            Printer.print_dbg ~flushed:false ~module_name:"IntervalCalculus"
              ...
              "try to extend synt sbt %a of ax %a@ " (Var.Map.pp E.print) sbs
              E.print orig;
          if tr.E.semantic == [] && not do_syntactic_matching
          then
            (* pure syntactic insts already generated *)
            env, acc
          else if not (terms_linear_dep env torig)
          then (
            if get_debug_fpa () >= 2
            then Printer.print_dbg ~header:false "semantic matching failed(1)";
            env, acc)
          else
            match semantic_matching lem_name tr s env uf optimized with
            | env, None ->
              if get_debug_fpa () >= 2
              then Printer.print_dbg ~header:false "semantic matching failed(2)";
              env, acc
            | env, Some sbs ->
              if get_debug_fpa () >= 2
              then
                Printer.print_dbg ~header:false
                  "semantic matching succeeded:@ %a" (Var.Map.pp E.print)
                  (fst sbs);
              ...
              if accepted
              then begin
                let hyp = List.map (fun f -> E.apply_subst sbs f) tr.E.hyp in
                let p =
                  { E.ff = nf;
                    ...
                    lem = Some lorig;
                    from_terms = torig;
                    (* does'nt work if a 'gf' with theory_elim = true was
                       already assumed in the SAT !!! *)
                    theory_elim = false
                  }
                in
      ...
      (fun f (_th_ax, dep) accu ->
        ...
        then begin
          ...
          Printer.print_dbg ~module_name:"IntervalCalculus"
            ...
            "syntactic matching of Ax %s: got %d substs" (E.name_of_lemma f)
            !cpt
        end;
        res :: accu)
      env.th_axioms []
...
let instantiate ~do_syntactic_matching match_terms env uf selector =
  ...
  let env, insts =
    List.fold_left
      (fun accu substs ->
        new_facts_for_axiom ~do_syntactic_matching menv uf selector optimized
          substs accu)
  ...
  then
    Printer.print_dbg ~module_name:"IntervalCalculus"
      ~function_name:"instanciate" "IC.instantiate: %d insts generated"
      (List.length insts);
  env, insts
...
let assume_th_elt t th_elt dep =
  ...
  | Util.NIA | Util.NRA | Util.FPA | Util.RIA ->
    ...
    then
      Printer.print_dbg ~module_name:"IntervalCalculus"
        ~function_name:"assume_th_elt" "[Theory %s][%s] %a" th_name kd_str
        E.print ax_form;
