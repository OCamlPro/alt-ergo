        ...
        begin
          ...
          | E.Term { E.f = (Sy.Op Sy.Div); xs = [a; b]; _ } ->
            let ra, ea =
              ...
              else fst (X.make ~with_facts:false a), Explanation.empty (*otherwise, we loop*)
            in
            let rb, eb =
              let (rb, _) as e = Uf.find env.new_uf b in
              if List.filter (X.equal x) (X.leaves rb) == [] then e
              else fst (X.make ~with_facts:false b), Explanation.empty (*otherwise, we loop*)
            in
    ...
    List.fold_left
      (fun acc (v, q) ->
         ...
         if SX.mem v env.known_eqs || not (Uf.is_normalized uf v) then
           ...
           let r, _ = X.make ~with_facts:false t in
    ...
    let sbs =
      List.fold_left
        (fun ((sbt, sty) as sbs) (x, tx) ->
           ...
           let mk, _ = X.make ~with_facts:false t in
