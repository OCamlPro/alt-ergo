...
type 'a abstract =
  ...
  (* [Select { d_name; d_ty; d_arg }] represents the destructor [d_name] of the
     ADT [d_ty] on the ADT value [d_arg]. *)
  | Alien of 'a
...
let constr_of_destr ty dest =
  if Options.get_debug_adt ()
  then
    Printer.print_dbg ~module_name:"Adt" ~function_name:"constr_of_destr"
      "ty = %a" Ty.print ty;
  match ty with
  | Ty.Tadt (s, params) -> begin
...
module Shostak (X : ALIEN) = struct
  ...
  let is_mine_symb sy =
    ...
    | Sy.Op (Sy.Destruct _) ->
      (* A destructor is partially interpreted by the ADT theory. If we assume
         the tester of the constructor associated with this destructor, we
         propagate the non-guarded version of the destructor. See the
  ...
  let print ppf = function
    ...
    | Constr { c_name; c_args; _ } ->
      if Compat.List.is_empty c_args
      then Fmt.pf ppf "%a" DE.Term.Const.print c_name
      else
        Fmt.pf ppf "(%a %a)" DE.Term.Const.print c_name
          Fmt.(box @@ list ~sep:sp @@ pair nop X.print)
          c_args
    | Select d ->
  ...
  let is_mine u =
    ...
    | Select { d_arg; d_name; _ } -> (
      ...
      | Constr c -> begin
        try
          snd
          @@ List.find (fun (lbl, _) -> DE.Term.Const.equal d_name lbl) c.c_args
        with Not_found ->
  ...
  let equal s1 s2 =
    ...
    | Select d1, Select d2 ->
      ...
      && Ty.equal d1.d_ty d2.d_ty && X.equal d1.d_arg d2.d_arg
    | _ -> false
  ...
  let make t =
    ...
    | Sy.Op (Sy.Constr hs), Ty.Tadt (name, params) ->
      ...
      in
      let ctx =
        (* If [t] is a record constructor term of the form { x1 = t1; ...; xn =
           t2 } we generate the equation t.x1 = t1, ..., t.xn = tn and store
           them in the context returned by `X.make`. *)
        match cases with
        | [{ destrs; _ }] ->
          List.fold_left2
            (fun ctx x (d, d_ty) ->
    ...
    (* No risk ! if equal sel (embed sel_x) then X.term_embed t, ctx else sel_x,
       ctx (* canonization OK *) *)
    | _ -> assert false
  ...
  let hash x =
    ...
    | Constr { c_name; c_ty; c_args } ->
      List.fold_left
        ...
        (DE.Term.Const.hash c_name + (7 * Ty.hash c_ty))
        c_args
  ...
  let leaves r =
    match r with
    | Alien r | Select { d_arg = r; _ } -> X.leaves r
    | Constr { c_args; _ } ->
      SX.elements
      @@ List.fold_left
           (fun sx (_, r) ->
  ...
  let is_constant r =
    match r with
    | Alien r | Select { d_arg = r; _ } -> X.is_constant r
    | Constr { c_args; _ } ->
  ...
  (* not sure *)
  (* not (get_disable_adts ()) && match sb with | Sy.Op (Sy.Constr _) -> true |
     Sy.Op Sy.Destruct (_, guarded) -> not guarded | _ -> false *)
  ...
  let abstract_selectors p acc =
    ...
    (* assert false should probably reconstruct a new 'p' using args *)
    | ((Select ({ d_arg; _ } as s))
      ...
      *)
      ...
      if not (X.equal s_arg d_arg) [@ocaml.ppwarning "TODO: abstract Selectors"]
      then assert false;
      ...
      | Select ({ d_name; d_arg; _ } as s) ->
        ...
        let cons = E.mk_term (Sy.constr constr) xs (X.type_info d_arg) in
        if Options.get_debug_adt ()
        then
          Printer.print_dbg ~flushed:false ~module_name:"Adt"
            ~function_name:"abstract_selectors" "abstr with equality %a == %a@ "
        ...
        if Options.get_debug_adt ()
        then
          Printer.print_dbg ~header:false "%a becomes %a" X.print x X.print xx;
        xx, acc
      | _ -> x, acc
      end
  ...
  let solve r1 r2 pb =
    if Options.get_debug_adt ()
    then
      Printer.print_dbg ~module_name:"Adt" ~function_name:"solve"
    ...
    | Constr c1, Constr c2 -> (
      if not (DE.Term.Const.equal c1.c_name c2.c_name)
  ...
  let assign_value _ _ _ =
    (* Model generation is performed by the case-split mechanism in
       [Adt_rel]. *)
    None
  ...
  let to_model_term r =
    match embed r with
    | Constr { c_name; c_ty; c_args } ->
      let args = My_list.try_map (fun (_, arg) -> X.to_model_term arg) c_args in
      Option.bind args @@ fun args -> Some (E.mk_constr c_name args c_ty)
    | Select _ -> None
