...
module type S = sig
  ...
  val assume :
    ?ordered:bool ->
    (L.t * Th_util.lit_origin * Explanation.t * int * int) list ->
    t ->
    t * Expr.Set.t * int
...
module Main_Default : S = struct
  ...
  type choice = X.r Xliteral.view * Th_util.lit_origin * choice_sign * Ex.t
  (** the choice, the size, choice_sign, the explication set, the explication
      for this choice. *)
  ...
  let pp_choice ppf (sem_lit, lit_orig, _, ex) =
    ...
    | CS (k, _) ->
      Fmt.pf ppf "%a cs: %a (because %a)" Th_util.pp_theory k LR.print sem_lit
        Ex.print ex
    | NCS (k, _) ->
      Fmt.pf ppf "%a ncs: %a (because %a)" Th_util.pp_theory k LR.print sem_lit
        Ex.print ex
    | _ -> assert false
  ...
  module Debug = struct
    ...
    let subterms_of_assumed l =
      List.fold_left
        (List.fold_left (fun st (a, _, _) ->
             match L.view a with
             | LTerm a -> Expr.Set.union st (E.sub_terms SE.empty a)
             | LSem _ -> st))
        SE.empty l
      ...
      Hstring.Map.iter
        (fun hs (xs, ty, is_ac) ->
          print_dbg ~flushed:false ~header:false "logic %s%s : %a%a@ "
            ...
            (Hstring.view hs) print_arrow_type xs Ty.print ty)
        logics;
      print_dbg ~header:false "@]"
    ...
    (* unused -- let split_size sz = if get_debug_split () then print_dbg ">size
       case-split: %s" (Numbers.Q.to_string sz) *)
  ...
  let add_explanations_to_split (c, is_cs, size) =
    ...
    let ex_c_exp = if is_cs then Ex.add_fresh exp Ex.empty else Ex.empty in
  ...
  let look_for_sat ~for_model env sem_facts new_choices =
    let rec generate_choices env sem_facts acc_choices =
      let new_splits, base_env = CC_X.case_split env.gamma_finite ~for_model in
      let env = { env with gamma_finite = base_env } in
      match new_splits with
      | [] ->
        (* We cannot make any progress as theories don't produce new case-splits
           and we don't find any inconsistencies. We may obtain a complete model
           of our problem. *)
        { env with choices = List.rev acc_choices }, sem_facts
      | _ ->
        let new_choices = List.map add_explanations_to_split new_splits in
        propagate_choices env sem_facts acc_choices new_choices
    (* Propagates the choice made by case-splitting to the environment
       [gamma_finite] of the CC(X) algorithm. If there is no more choices to
       propagate, we call the [generate_choices] to either generate more choices
       or leave the function.

       @raise [Inconsistent] if we detect an inconsistent with the choice on the
       top of [new_choices] but this choice doesn't participate to the
       inconsistency. *)
      ...
      | ((c, lit_orig, CPos exp, ex_c_exp) as a) :: new_choices -> (
        ...
        with Ex.Inconsistent (dep, classes) -> (
          (* As we generate fresh explanation for each choice in
             [add_explanations_to_split], we know that the inconsistency
             involves the current choice if and only if the explanation [dep]
             contains the explanation exp. *)
          ...
          | Some dep ->
            ...
            let lit_orig =
              ...
              | _ ->
                (* Unreacheable as the exception [Inconsistent] is only raised
                   by [propagate_choices] on facts of origin [Th_util.CS]. *)
                assert false
            in
            Debug.split_backtrack neg_c dep;
            if Options.get_bottom_classes ()
            then
              Printer.print_dbg "bottom (case-split):%a"
  ...
  (* 1. Remove case-split decisions contain fresh variables (introduced by
     ...
     (because of backtrack) 2. Remove implications that are due to decisions
     removed in 1. *)
    ...
    List.filter
      (fun (_, _, _, ex) ->
        try
          ...
          true
        with Exit ->
          (* ignore implicated related to ignored decisions *)
          false)
      candidates_to_keep
  ...
  let reset_case_split_env t =
    { t with
      cs_pending_facts = [];
      (* be sure it's always correct when this function is called *)
      gamma_finite = t.gamma;
      (* we'll take gamma directly *)
      choices =
        []
        (* we'll not be able to attempt to replay choices. We're not in
           try-it *)
    }

  (* This function attempts to produce a first-order model by case-splitting. To
     do so, the function tries two successive strategies: - First, we replay in
     [gamma_finite] all the new facts learnt by the SAT solver since the last
     case-splitting phase in the environment. - If the first strategy fails, we
     synchronize [gamma] and [gamma_finite] and we try to propagate a subset of
     the choices. *)
    ...
    let r =
      try
        ...
        then
          ...
          try
            (* We attempt to replay all the facts learnt by the SAT solver since
               the last call to [do_case_split]. *)
          ...
          with Ex.Inconsistent _ ->
            ...
            (* We replay the conflict in look_for_sat, so we can safely ignore
               the explanation which is not useful. *)
            ...
            let t = reset_case_split_env t in
            look_for_sat ~for_model { t with choices = [] } [] filt_choices
        end
      with Ex.Inconsistent (dep, classes) ->
  ...
  let extract_from_semvalues acc l =
    List.fold_left
      (fun acc r ->
        match X.term_extract r with Some t, _ -> SE.add t acc | _ -> acc)
      acc l
  ...
  let extract_terms_from_choices acc choices =
    List.fold_left
      (fun acc (a, _, _, _) -> extract_terms_from_xliteral acc a)
      acc choices

  let extract_terms_from_assumed =
    List.fold_left (fun acc ((a : _ Sig_rel.literal), _, _) ->
        match a with
        | LTerm r -> begin
          match E.lit_view r with
          | E.Eq (t1, t2) -> SE.add t1 (SE.add t2 acc)
          | E.Eql l | E.Distinct l | E.Builtin (_, _, l) ->
            List.fold_right SE.add l acc
          | E.Pred (t1, _) -> SE.add t1 acc
        end
        | _ -> acc)
  ...
  let do_case_split_aux t ~for_model =
    ...
    let facts = CC_X.empty_facts () in
    List.iter
      (List.iter (fun (a, o, ex, _dlvl, _plvl) ->
           let a =
             match L.view a with
             | LTerm _ as a -> a
  ...
  let optimize_obj ~for_model add_objective obj t =
    let opt_split = Option.get (CC_X.optimizing_objective t.gamma obj) in
    match opt_split.value with
    | Unknown ->
      (* Cannot happen as [Rel.optimizing_objective] never returns an unknown
         value. *)
      assert false
    | (Pinfinity | Minfinity | Limit _) when not for_model ->
      (* We stop optimizing the objective function [obj] in this case, but we
         continue to produce a model if the flag [for_model] is up. *)
      ()
    | Pinfinity | Minfinity | Limit _ | Value _ ->
      let lview, is_cs, _ = opt_split.case_split in
      assert is_cs;
      Logs.debug ~src:Options.Sources.optimize (fun k ->
          k "Objective for %a is %a [split: %a]" Objective.Function.pp obj
            Objective.Value.pp opt_split.value Shostak.L.print
            (Shostak.L.make lview));
      add_objective obj opt_split.value
        Shostak.(Literal.make @@ LSem (L.make lview))

  let sat_splits t =
    if Options.get_enable_sat_cs ()
    then
  ...
  let do_case_split ?acts t origin =
    if Options.get_case_split_policy () == origin
    then
      match acts with
  ...
  let assume ordered in_facts t =
    ...
    let assumed, assumed_set, cpt, gamma =
      List.fold_left
        (fun ((assumed, assumed_set, cpt, gamma) as accu) (a, o, ex, dlvl, plvl)
           ->
          match L.view a with
          ...
          | aview ->
            let gamma, aview =
              ...
              | LSem a ->
                (* When assuming a semantic literal (typically a case split), we
                   may end up in cases where they contain terms that have not
                   been added.

                   One reason this can happen is when (the negation of) semantic
                   literals are pushed onto the trail at lower levels than they
                   were initially created (notably with with minimal
                   backjumps). *)
                let aview = LR.view a in
                let trms = extract_terms_from_xliteral SE.empty aview in
                let gamma =
                  SE.fold
                    (fun t gamma -> fst @@ CC_X.add_term gamma facts t Ex.empty)
                    trms gamma
                in
                gamma, LSem aview
            in
            ...
            let assumed_set =
              ...
              | _ -> assumed_set
            in
            (a, dlvl, plvl) :: assumed, assumed_set, cpt + 1, gamma)
        ([], t.assumed_set, 0, t.gamma)
        in_facts
    in
    if assumed == []
    then t, E.Set.empty, 0
    else
      let t =
        { t with
          assumed_set;
      ...
      let t = { t with gamma; terms = Expr.Set.union t.terms new_terms } in
      t, new_terms, cpt

  let get_debug_theories_instances th_instances ilvl dlvl =
    let module MF = Expr.Map in
    Printer.print_dbg ~flushed:false ~module_name:"Theory"
    ...
    List.iter
      (fun (f, m, inst) ->
        Printer.print_dbg ~flushed:false ~header:false "%3d  -->  %a@ " m
          Expr.print f;
        if true
        then begin
          MF.iter
            (fun f hyps ->
              Printer.print_dbg ~flushed:false ~header:false "@[<v 2>[inst]@ ";
              List.iter
                (fun h ->
  ...
  let theories_instances ~do_syntactic_matching t_match t selector dlvl ilvl =
    ...
    then get_debug_theories_instances instances dlvl ilvl;
  ...
  let query =
    ...
    in
    fun a t ->
      if Options.get_no_tcp ()
      then Th_util.Unknown
      else begin
        ...
        try
          ...
          | E.Distinct _ | E.Eql _ ->
            (* we only assume toplevel distinct with more that one arg. not
               interesting to do a query in this case ?? or query ? *)
            Th_util.Unknown
          | E.Pred (t1, b) ->
            let t = add_and_process_conseqs a t in
            if b
        ...
        with Ex.Inconsistent (ex, classes) -> Th_util.Entailed { ex; classes }
      end
  ...
  let query a t =
    Timers.with_timer Timers.M_CC Timers.F_query @@ fun () -> query a t
  ...
  let compute_concrete_model ~acts t =
    ...
    | Some obj -> optimize_obj ~for_model:true add_objective obj t
    | None -> ()
  ...
  let extract_concrete_model ~declared_ids env =
    ...
    in
    ( lazy
        (CC_X.extract_concrete_model ~prop_model:assumed_set ~defaults
           gamma_finite),
      objectives )
  ...
  let add_objective env fn value =
    (* We have to add the term [fn] and its subterms as the MaxSMT syntax allows
       to optimize expressions that are not part of the current context. *)
  ...
  let reinit_cpt () = Debug.reinit_cpt ()
end

module Main_Empty : S = struct
  type t = { assumed_set : E.Set.t }
