...
(** The Dolmen state option manager. Each module defined below is linked to an
    option that can be set, fetched et reset independently from the Options
    module, which is used as a static reference. *)
...
(** Options are divided in two categories: 1. options that can be updated
    anytime; 2. options that can only be updated during start mode. *)
...
(** Option for setting the max number of steps. Interfaces with the toplevel
    Steps module. The [set] function may raise Invalid_arg from the
    Steps.set_steps_bound call if the new option value is lower than the current
    number of steps. *)
