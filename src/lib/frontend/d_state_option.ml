...
let create_opt (type t) ?(on_update = fun _ _ -> Fun.id) (key : string)
  ...
  (module struct
    ...
    let set opt st = st |> on_update key opt |> State.set key opt

    let unsafe_get st = State.get key st

    let clear st = State.update_opt key (fun _ -> None) st

    let get st = try unsafe_get st with State.Key_not_found _ -> get ()
  end)
...
(* Similar to `create_opt`, except we fail if we set the option while we are not
   in start mode. *)
let create_opt_only_start_mode (type t) ?(on_update = fun _ _ -> Fun.id)
    (key : string) (get : unit -> t) : (module S with type t = t) =
...
module StrictMode = (val create_opt "strict_mode" O.get_strict_mode)
...
let get_sat_solver ?(sat = O.get_sat_solver ()) ?(no_th = O.get_no_theory ()) ()
    =
  let module SatCont = (val Sat_solver.get sat : Sat_solver_sig.SatContainer) in
  let module TH = (val Sat_solver.get_theory ~no_th) in
  (module SatCont.Make (TH) : Sat_solver_sig.S)

module SatSolverModule =
  (val create_opt_only_start_mode "sat_solver_module" (fun _ ->
           get_sat_solver ()))

let msatsolver =
  let on_update _ sat st = SatSolverModule.set (get_sat_solver ~sat ()) st in
...
let options_requiring_initialization = [(module SatSolverModule : S)]

let init st =
  List.fold_left
    (fun st (module S : S) -> S.set (S.get (S.clear st)) st)
    st options_requiring_initialization
