...
module Pp_smtlib_term = struct
  ...
  let to_string_type t = asprintf "%a" Ty.pp_smtlib t
  ...
  let rec print fmt t =
    ...
    | Sy.Lit lit, xs -> begin
      ...
      | Sy.L_neg_eq, [a; b] ->
        if Options.get_output_smtlib ()
        then fprintf fmt "(not (= %a %a))" print a print b
      ...
      | Sy.L_built Sy.LE, [a; b] ->
        if Options.get_output_smtlib ()
        then fprintf fmt "(<= %a %a)" print a print b
        else fprintf fmt "(%a <= %a)" print a print b
      ...
      | Sy.L_built Sy.BVULE, [a; b] ->
        if Options.get_output_smtlib ()
        then fprintf fmt "(bvule %a %a)" print a print b
        else fprintf fmt "(%a <= %a)" print a print b
      | Sy.L_neg_built Sy.BVULE, [a; b] ->
        if Options.get_output_smtlib ()
        then fprintf fmt "(bvugt %a %a)" print a print b
        else fprintf fmt "(%a > %a)" print a print b
      | Sy.L_built (Sy.IsConstr hs), [e] ->
        if Options.get_output_smtlib ()
        then fprintf fmt "((_ is %a) %a)" DE.Term.Const.print hs print e
        else fprintf fmt "(%a ? %a)" print e DE.Term.Const.print hs
      | Sy.L_neg_built (Sy.IsConstr hs), [e] ->
        if Options.get_output_smtlib ()
        then fprintf fmt "(not ((_ is %a) %a))" DE.Term.Const.print hs print e
        else fprintf fmt "not (%a ? %a)" print e DE.Term.Const.print hs
    ...
    | Sy.Op Sy.Get, [e1; e2] ->
      if Options.get_output_smtlib ()
      then fprintf fmt "(select %a %a)" print e1 print e2
      else fprintf fmt "%a[%a]" print e1 print e2
    | Sy.Op Sy.Set, [e1; e2; e3] ->
      if Options.get_output_smtlib ()
      then fprintf fmt "(store %a %a %a)" print e1 print e2 print e3
      else fprintf fmt "%a[%a<-%a]" print e1 print e2 print e3
    | Sy.Op Sy.Concat, [e1; e2] -> fprintf fmt "%a@@%a" print e1 print e2
    | Sy.Op (Sy.Extract (i, j)), [e] -> fprintf fmt "%a^{%d,%d}" print e i j
    ...
    | Sy.Op _, [e1; e2] ->
      if Options.get_output_smtlib ()
      then fprintf fmt "(%a %a %a)" Sy.print f print e1 print e2
      else fprintf fmt "(%a %a %a)" print e1 Sy.print f print e2
    ...
    | Sy.Name { hs = n; _ }, l -> begin
      let constraint_name =
        try
          let constraint_name, _, _ = MS.find (Hstring.view n) !constraints in
          constraint_name
        with _ ->
          let constraint_name = "c_" ^ Hstring.view n in
          constraints
            := MS.add (Hstring.view n)
                 ( constraint_name,
    ...
    | _, _ ->
      if Options.get_output_smtlib ()
      then fprintf fmt "(%a %a)" Sy.print f print_list xs
      else fprintf fmt "%a(%a)" Sy.print f print_list xs
...
module Why3CounterExample = struct
  let pp_term fmt t =
    ...
    then Pp_smtlib_term.print fmt t
    else E.print fmt t

  let output_constraints fmt prop_model =
    let assertions =
      Expr.Set.fold
        (fun e acc -> Format.dprintf "%t(assert %a)@ " acc pp_term e)
        prop_model (Format.dprintf "")
    in
    Format.fprintf fmt "@ ; constraints@ ";
    MS.iter
      (fun _ (name, ty, args_ty) ->
        match args_ty with
        | [] -> Format.fprintf fmt "(declare-const %s %s)@ " name ty
        | l ->
          Format.fprintf fmt "(declare-fun %s (%s) %s)@ " name
            (String.concat " " l) ty)
      !constraints;
...
let pp ppf { model; propositional; _ } =
  if Options.get_model_type_constraints ()
  then begin
