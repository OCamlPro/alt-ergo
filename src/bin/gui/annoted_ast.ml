...
type atyped_decl =
  ...
  | AGoal of Loc.t * Ty.goal_sort * string * aform annoted
...
and print_tt_desc fmt = function
  ...
  | TTprefix (s, t) ->
    fprintf fmt "%a %a" Sy.print_clean s print_tterm t
  | TTlet (binders , t2) ->
...
and print_term_binders fmt l =
  ...
  | (sy, t) :: l ->
    ...
    List.iter (fun (sy, t) ->
        fprintf fmt ",\n%a = %a" Sy.print_clean sy print_term t) l
...
let rec make_dep_atyped_decl dep d =
  ...
  | ARewriting (_, _, arwtl) ->
    List.fold_left
      (fun dep r ->
         let vars = List.map
             (fun (s,_) -> (Sy.to_string_clean s)) r.c.rwt_vars in
...
and add_at_desc_at errors indent (buffer:sbuffer) tags iter at =
  ...
  | ATapp (s, atl)  ->
    append_buf buffer ~iter ~tags
      (sprintf "%s(" (Sy.to_string_clean s));
...
let rec isin_aterm sl { at_desc; _ } =
  ...
  | ATconst _ -> false
  | ATvar sy ->
    List.mem (Sy.to_string_clean sy) sl
  | ATapp (sy, atl) ->
...
and findtags_aaterm sl aat acc =
  ...
  | ATapp (sy, atl) ->
    if List.mem (Sy.to_string_clean sy) sl || isin_aterm_list sl atl
