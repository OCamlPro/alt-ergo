...
(** type that contains a list of the axiom used in instances. axiom name, start
    pos, end pos, number of time its used in insstances, Used if its usefull to
    solve the goal (from unsat core), Unused otherwise Unknown if the unsat-core
    option is not setted *)

(** Type used to return the status of solving This can be usefull to match
    status instead of analysing results and errors fields *)
type status =
  | Unsat of int
  | Inconsistent of int
  | Sat of int
...
(** Record type that contains all results that can be returned by the Alt-Ergo's
    worker. *)
...
(** Take an optional file name, an optional worker identifier (integer) and the
    file content as a string and convert it to a json file into Js string *)

val file_from_json :
  Js_of_ocaml.Js.js_string Js_of_ocaml.Js.t ->
  string option * int option * string list
(** Take a Js string corresponding to a Json file and decoding in into an
    optional file name, an optional worker identifier and the file content *)

(** {3 Options functions} *)

val init_options : unit -> options
(** Return a record containing None for all options in the option type Since the
    function set_options in options_interface set only options with value (Some
    v), this function is use to create a record with all field to None. *)
...
(** Get a JS string corresponding of a Json and decoding it into a record of the
    options type. If some field are not included in the Json, the value None is
    set for this fields *)
