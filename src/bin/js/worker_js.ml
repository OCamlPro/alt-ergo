...
let check_buffer_content (buf, output) =
  ...
  if String.equal buf_cont ""
  then None
  else
    let buf = String.split_on_char '\n' buf_cont in
    Some buf

let check_context_content c = match c with [] -> None | _ -> Some c

let create_buffer () =
  let buf = Buffer.create 10 in
  let output = Format.formatter_of_buffer buf |> Options.Output.of_formatter in
  buf, output

let main worker_id filename filecontent =
  try
    (* Create buffer for each formatter The content of this buffers are then
       retrieved and send as results *)
    ...
    (* Status updated regarding if AE succed or failed (error or steplimit
       reached) *)
    ...
    let selector_inst orig =
      ...
      begin try incr (snd (Hashtbl.find tbl id))
    ...
    let compute_statistics () =
      ...
      Hashtbl.fold
        ...
        tbl []
    in
    Solving_loop.process_source ~selector_inst ~print_status
      (`Raw (filename, filecontent));
  ...
  | Errors.Error e ->
    ...
    { res with
      ...
      Worker_interface.diagnostic = Some [Format.asprintf "%a" Errors.report e]
    }
  | Solving_loop.Exit_with_code code ->
    let res = Worker_interface.init_results () in
    let msg = Fmt.str "exit code %d" code in
    { res with Worker_interface.worker_id; Worker_interface.status = Error msg }
  | exn ->
    let res = Worker_interface.init_results () in
    let msg = Fmt.str "Unknown error: %s" (Printexc.to_string exn) in
    { res with Worker_interface.worker_id; Worker_interface.status = Error msg }

(** Worker initialisation Run Alt-ergo with the input file (string) and the
    corresponding set of options Return a couple of list for status (one per
    goal) and errors *)
let () =
