...
let format_encoding =
  union
    [ case (Tag 1) ~title:"Native" (constant "Native")
        (function Native -> Some () | _ -> None)
        (fun () -> Native);
      case (Tag 2) ~title:"Smtlib2" (constant "Smtlib2")
        (function Smtlib2 -> Some () | _ -> None)
        (fun () -> Smtlib2);
      case (Tag 3) ~title:"Why3" (constant "Why3")
        (function Why3 -> Some () | _ -> None)
        (fun () -> Why3);
      case (Tag 4) ~title:"Unknown"
...
let sat_solver_encoding =
  union
    [ case (Tag 1) ~title:"Tableaux" (constant "Tableaux")
        (function Tableaux -> Some () | _ -> None)
        (fun () -> Tableaux);
      case (Tag 2) ~title:"CDCL" (constant "CDCL")
        (function CDCL -> Some () | _ -> None)
        (fun () -> CDCL);
      case (Tag 3) ~title:"CDCL_Tableaux" (constant "CDCL_Tableaux")
...
let instantiation_heuristic_encoding =
  union
    [ case (Tag 1) ~title:"INormal" (constant "INormal")
        (function INormal -> Some () | _ -> None)
        (fun () -> INormal);
      case (Tag 2) ~title:"IAuto" (constant "IAuto")
        (function IAuto -> Some () | _ -> None)
        (fun () -> IAuto);
      case (Tag 3) ~title:"IGreedy" (constant "IGreedy")
...
let opt_dbg1_encoding =
  conv
    (fun dbg1 -> dbg1)
    (fun dbg1 -> dbg1)
    (obj10 (opt "debug" bool) (opt "debug_ac" bool) (opt "debug_adt" bool)
       (opt "debug_arith" bool) (opt "debug_arrays" bool)
       (opt "debug_bitv" bool) (opt "debug_cc" bool) (opt "debug_combine" bool)
...
let opt_dbg2_encoding =
  conv
    (fun dbg2 -> dbg2)
    (fun dbg2 -> dbg2)
    (obj9 (opt "debug_fm" bool) (opt "debug_fpa" int31) (opt "debug_gc" bool)
       (opt "debug_interpretation" bool)
       (opt "debug_ite" bool)
       (opt "debug_matching" int31)
       (opt "debug_sat" bool) (opt "debug_split" bool)
...
let opt_dbg3_encoding =
  conv
    (fun dbg3 -> dbg3)
    (fun dbg3 -> dbg3)
    (obj4 (opt "debug_types" bool) (opt "debug_uf" bool)
...
let opt1_encoding =
  conv
    ...
    (obj8 (opt "rule" int31)
       ...
       (opt "max_split" int31) (opt "replay" bool)
...
let opt2_encoding =
  conv
    ...
    (obj6
       ...
       (opt "type_only" bool) (opt "type_smt2" bool))

let opt3_encoding =
  conv
    (fun opt3 -> opt3)
    (fun opt3 -> opt3)
    (obj7 (opt "disable_weaks" bool)
...
let opt4_encoding =
  conv
    ...
    (obj9 (opt "verbose" bool)
       ...
       (opt "nb_triggers" int31) (opt "no_ematching" bool)
...
let opt5_encoding =
  conv
    ...
    (obj10 (opt "arith_matchin" bool)
       ...
       (opt "no_backward" bool) (opt "no_decisions" bool))
...
let opt6_encoding =
  conv
    ...
    (obj9
       ...
       (opt "disable_ites" bool) (opt "inline_lets" bool) (opt "rewriting" bool)
       (opt "term_like_pp" bool) (opt "disable_adts" bool) (opt "no_ac" bool))
...
let opt7_encoding =
  conv
    ...
    (obj8
       ...
       (opt "no_fm" bool) (opt "no_nla" bool) (opt "no_tcp" bool)
       (opt "no_theory" bool) (opt "restricted" bool) (opt "tighten_vars" bool)
       (opt "timers" bool))
                ...
                (merge_objs opt3_encoding
                   (merge_objs opt4_encoding
                      (merge_objs opt5_encoding
                         (merge_objs opt6_encoding opt7_encoding))))))))
...
let options_to_json opt =
  ...
  in
  let dbg_opt3 =
    opt.debug_types, opt.debug_uf, opt.debug_unsat_core, opt.debug_use
  in
  let all_opt1 =
    ( opt.rule,
  ...
  let all_opt7 =
    ( opt.no_contracongru,
      ...
      opt.timers )
  in
  let json_all_options =
    Json.construct options_encoding
      ( dbg_opt1,
        ( dbg_opt2,
          ( dbg_opt3,
            ( all_opt1,
...
let options_from_json options =
  ...
  | Ok opts ->
    ...
    let ( debug_fm,
      ...
      dbg_opt2
    in
    let debug_types, debug_uf, debug_unsat_core, debug_use = dbg_opt3 in
    let ( rule,
          case_split_policy,
          enable_adts_cs,
          max_split,
...
let used_axiom_encoding =
  union
    [ case (Tag 1) ~title:"Used" (constant "Used")
        (function Used -> Some () | _ -> None)
        (fun () -> Used);
      case (Tag 2) ~title:"Unused" (constant "Unused")
        (function Unused -> Some () | _ -> None)
        (fun () -> Unused);
      case (Tag 3) ~title:"Unknown" (constant "Unknown")
        (function Unknown -> Some () | _ -> None)
        (fun () -> Unknown) ]

type statistics = (string * int * int * int * used_axiom) list
...
let file_encoding =
  conv
    (fun f -> f)
    (fun f -> f)
    (obj3 (opt "filename" string) (opt "worker_id" int31)
