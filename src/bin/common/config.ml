...
let rec find_map f = function
  | [] -> None
  | x :: xs -> ( match f x with Some _ as y -> y | None -> find_map f xs)
...
let load_plugin plugin =
  try AltErgoSites.Plugins.Plugins.load plugin
  with e ->
    AltErgoLib.Errors.run_error
      (Dynlink_error
         (Format.asprintf
            "@[<v>Loading the plugin %S failed!@,>> Failure message: %s" plugin
