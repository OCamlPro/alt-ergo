...
(* When adding another parser, remember to change this list too as it is used in
   the documentation *)
let formats_list =
  [ "native", Native;
    "altergo", Native;
...
let format_parser s =
  try Ok (My_list.assoc String.equal s formats_list)
  with Not_found ->
    Error
      (`Msg (Format.sprintf "The format %s is not accepted as input/output" s))
...
let model_type_parser = function
  ...
  | s ->
    Error
      (`Msg
         (Format.sprintf
            "The model kind %s is invalid. Only \"value\" and \"constraints\" \
             are allowed"
            s))

let model_type_printer fmt format =
  Format.fprintf fmt "%s"
    (match format with Value -> "value" | Constraints -> "constaints")
...
module Debug = struct
  ...
  let mk ~verbosity flags =
    ...
    |> List.iter (function
      ...
      | Constr ->
        ...
        set_level S.constr
      | Explanations -> Options.set_debug_explanations true
      | Fm ->
  ...
  let light_flag_term, medium_flag_term, full_flag_term =
    let enum_conv = List.map (fun v -> show v, v) all |> Arg.enum |> Arg.list in
    let light_doc =
      Format.sprintf
    ...
    let docv = "DEBUG" in
    ( Arg.(
        value & opt_all enum_conv [] & info ["d"; "debug"] ~docv ~doc:light_doc),
      Arg.(
        value & opt_all enum_conv []
        & info ["dd"; "ddebug"] ~docv ~doc:medium_doc),
      Arg.(
        value & opt_all enum_conv []
        & info ["ddd"; "dddebug"] ~docv ~doc:full_doc) )
...
let mk_case_split_opt case_split_policy enable_adts_cs max_split () =
  let res =
    ...
    | _ ->
      Error
        ("Bad value '" ^ case_split_policy ^ "' for option --case-split-policy")
  in
...
let mk_execution_opt input_format parse_only () preludes no_locs_in_answers
    colors_in_output no_headers_in_output no_formatting_in_output
    no_forced_flush_in_output pretty_output type_only type_smt2 =
  ...
  let output_with_forced_flush =
    (not no_forced_flush_in_output) && not pretty_output
  in
...
let mk_internal_opt disable_weaks enable_assertions warning_as_error
    continue_on_error gc_policy =
  let gc_policy =
...
let mk_limit_opt age_bound fm_cross_limit timelimit_interpretation steps_bound
  ...
  let set_limit t d =
    ...
    | Some t ->
      if Sys.win32
      then (
        Printer.print_wrn "timelimit not supported on Win32 (ignored)";
        d)
      else t
    | None -> d
  in
  if steps_bound < -1
  then `Error (false, "--steps-bound argument should be positive")
  else
...
let mk_output_opt produce_models objectives_in_interpretation unsat_core
  ...
  let output_format =
    match output_format with None -> Native | Some fmt -> fmt
  in
  let model_type = match model_type with None -> Value | Some v -> v in
...
let mk_profiling_opt cumulative_time_profiling profiling profiling_plugin
    get_verbose =
  let profiling, profiling_period =
    match profiling with Some f -> true, f | None -> false, 0.
  in
...
let mk_quantifiers_opt instantiation_heuristic instantiate_after_backjump
    max_multi_triggers_size nb_triggers no_ematching no_user_triggers
...
let mk_sat_opt get_bottom_classes disable_flat_formulas_simplification
    enable_restarts no_arith_matching no_backjumping no_backward no_decisions
    no_decisions_on no_minimal_bj no_sat_learning =
  let arith_matching = not no_arith_matching in
  let mk_no_decisions_on ndo =
    List.fold_left
      (fun set s -> match s with "" -> set | s -> Util.SS.add s set)
      Util.SS.empty
      (Str.split (Str.regexp ",") ndo)
  in
...
let halt_opt version_info where =
  ...
  let handle_version_info vi =
    ...
    then
      Printer.print_std
        "@[<v 0>Version          = %s@,Release commit   = %s@]@."
        Version._version Version._release_commit
  in
  try
    match where with
    | Some w ->
  ...
  with
  | Failure f -> `Error (false, f) (* TODO(Steven): dead code? *)
  | Error (b, m) -> `Error (b, m)
...
let mk_opts file () () debug_flags ddebug_flags dddebug_flags backtrace rule ()
  ...
  Rule.mk rule;
  let path = match file with Some f -> `File f | None -> `Stdin in
  if halt_opt
  then `Ok None
  (* If save_used_context was invoked as an option it should automatically set
     unsat_core to true *)
  else begin
...
let parse_case_split_opt =
    ...
    let doc =
      Format.sprintf
        "Case-split policy. Set the case-split policy to use. Possible values \
         are %s."
        (Arg.doc_alts
           ["after-theory-assume"; "before-matching"; "after-matching"])
    in
    let docv = "PLCY" in
    Arg.(
      value
      & opt string "after-theory-assume"
      & info ["case-split-policy"] ~docv ~docs ~doc)
  in
  let enable_adts_cs =
    let doc = "Enable case-split for Algebraic Datatypes theory." in
    Arg.(value & flag & info ["enable-adts-cs"] ~docs ~doc)
  in
  let enable_sat_cs =
  ...
  let max_split =
    ...
    let doc = Format.sprintf "Maximum size of case-split." in
    let docv = "VAL" in
    Arg.(value & opt string dv & info ["max-split"] ~docv ~docs ~doc)
  in
  Term.(
    ret
      (const mk_case_split_opt $ case_split_policy $ enable_adts_cs $ max_split
     $ enable_sat_cs))
...
let parse_context_opt =
  ...
  let replay =
    ...
    Arg.(value & flag & info ["replay"] ~docs ~doc)
  in
  let replay_all_used_context =
    let doc =
      "Replay with all axioms and predicates saved in $(i,.used) files of the \
       current directory."
    in
    Arg.(value & flag & info ["replay-all-used-context"] ~docs ~doc)
  in
  let replay_used_context =
    let doc = "Replay with axioms and predicates saved in $(i,.used) file." in
    Arg.(value & flag & info ["r"; "replay-used-context"] ~doc)
  in
  let save_used_context =
    let doc =
      "Save used axioms and predicates in a $(i,.used) file. This option \
       implies $(b,--unsat-core)."
    in
    Arg.(value & flag & info ["s"; "save-used-context"] ~doc)
  in
  Term.(
    ret
...
let parse_execution_opt =
  ...
  let input_format =
    let doc =
      Format.sprintf
        "Set the default input format to $(docv) and must be %s. Useful when \
         the extension does not allow to automatically select a parser (eg. JS \
         mode, GUI mode, ...)."
        (Arg.doc_alts (fst @@ List.split formats_list))
    in
    let docv = "FMT" in
  ...
  let parse_only =
    ...
    Arg.(value & flag & info ["parse-only"] ~docs ~doc)
  in
  let parsers =
    let load_parser verbose path =
      try
        ...
        Ok ()
      with Errors.Error e -> Error (Format.asprintf "%a" Errors.report e)
    in
    let load_parsers verbose paths =
      List.fold_left
        (fun res path -> Result.bind res (fun () -> load_parser verbose path))
        (Ok ()) paths
    in
    let doc = "Register a new parser for Alt-Ergo." in
    let arg =
      Arg.(value & opt_all string [] & info ["add-parser"] ~docs ~doc)
    in
  ...
  let preludes =
    let parse_prelude p =
      ...
      then
        if Sys.is_directory p then Fmt.error "'%s' is a directory" p else Ok p
      else
        match Config.lookup_prelude p with
        | Some p' -> Ok p'
        | None -> Error (Fmt.str "cannot load prelude '%s': no such file" p)
    in
    let prelude = Arg.(conv' (parse_prelude, conv_printer string)) in
    let doc =
      "Add a file that will be loaded as a prelude. The command is cumulative, \
       and the order of successive preludes is preserved."
    in
    Arg.(
      value & opt_all prelude (get_preludes ()) & info ["prelude"] ~docs ~doc)
  in
  let no_locs_in_answers =
    let doc =
      "Do not show the locations of goals when printing solver's answers."
    in
    Arg.(value & flag & info ["no-locs-in-answers"] ~docs ~doc)
  in
  let colors_in_output =
    let doc = "Print output with colors." in
    Arg.(value & flag & info ["colors-in-output"] ~docs ~doc)
  in
  let no_headers_in_output =
    let doc = "Print output without headers." in
    Arg.(value & flag & info ["no-headers-in-output"] ~docs ~doc)
  in
  let no_formatting_in_output =
    let doc = "Don not use formatting rule in output." in
    Arg.(value & flag & info ["no-formatting-in-output"] ~docs ~doc)
  in
  let no_forced_flush_in_output =
    let doc = "Print output without forced flush at the end of each print." in
    Arg.(value & flag & info ["no-forced-flush-in-output"] ~docs ~doc)
  in
  let pretty_output =
    let doc = "Print output with formatting rules, headers and colors" in
    Arg.(value & flag & info ["p"; "pretty-output"] ~doc)
  in
  let type_only =
  ...
  let type_smt2 =
    ...
    Arg.(value & flag & info ["type-smt2"] ~docs ~doc)
  in
  Term.(
    ret
      (const mk_execution_opt $ input_format $ parse_only $ parsers $ preludes
...
let parse_halt_opt =
  ...
  let version_info =
    let doc =
      "Print some info about this version (version, date released, date \
       commited) ."
    in
    Arg.(value & flag & info ["version-info"] ~docs ~doc)
  in
  let where =
    let doc =
      Format.sprintf
        "Print the directory of $(docv). Possible arguments are %s."
        (Arg.doc_alts ["plugins"; "preludes"])
    in
    let docv = "DIR" in
    Arg.(value & opt (some string) None & info ["where"] ~docv ~docs ~doc)
  in
  Term.(ret (const halt_opt $ version_info $ where))

let parse_internal_opt =
  let docs = s_internal in
  let disable_weaks =
    let doc =
      "Prevent the GC from collecting hashconsed data structrures that are not \
       reachable (useful for more determinism)."
    in
    Arg.(value & flag & info ["disable-weaks"] ~docs ~doc)
  in
  let enable_assertions =
    let doc = "Enable verification of some heavy invariants." in
    Arg.(value & flag & info ["enable-assertions"] ~docs ~doc)
  in
  let warning_as_error =
    let doc = "Enable warning as error" in
    Arg.(value & flag & info ["warning-as-error"] ~docs ~doc)
  in
  let continue_on_error =
    let doc = "Sets Alt-ergo's behavior to continue on errors" in
    Arg.(value & flag & info ["continue-on-error"] ~docs ~doc)
  in
  let gc_policy =
    let doc =
      "Set the gc policy allocation. 0 = next-fit policy, 1 = first-fit \
       policy, 2 = best-fit policy. See the Gc module of the OCaml \
       distribution for more informations."
    in
    let docv = "PLCY" in
    Arg.(value & opt int 0 & info ["gc-policy"] ~docv ~docs ~doc)
  in
  Term.(
    ret
...
let parse_limit_opt =
  ...
  let age_bound =
    ...
    let docv = "AGE" in
    Arg.(
      value & opt int (get_age_bound ()) & info ["age-bound"] ~docv ~docs ~doc)
  in
  let fm_cross_limit =
    ...
    let doc =
      Format.sprintf
        "Skip Fourier-Motzkin variables elimination steps that may produce a \
         number of inequalities that is greater than the given limit. However, \
         unit eliminations are always done."
    in
    let docv = "VAL" in
    Arg.(value & opt string dv & info ["fm-cross-limit"] ~docv ~docs ~doc)
  in
  let timelimit_interpretation =
    let doc =
      "Set the time limit to $(docv) seconds for model generation (not \
  ...
  let timelimit =
    ...
    in
    let docv = "VAL" in
    Arg.(value & opt (some float) None & info ["t"; "timelimit"] ~docv ~doc)
  in
  let timelimit_per_goal =
    let doc =
      "Set the timelimit given by the $(i,--timelimit) option to apply for \
       each goal, in case of multiple goals per file. In this case, time spent \
       in preprocessing is separated from resolution time. Not relevant for \
       GUI-mode."
    in
    Arg.(value & flag & info ["timelimit-per-goal"] ~docs ~doc)
  in
  Term.(
    ret
...
let parse_output_opt =
  ...
  let interpretation, dump_models, dump_models_on, frontend =
    let interpretation =
      let doc =
        Fmt.str
          "This option allowed choosing when the model generation occurs. It \
           has been removed in Alt-Ergo 2.7.0. If model generation is enabled, \
           it is performed as the final step, which was the default in \
           previous versions."
      in
      let docv = "VAL" in
      let deprecated = "this option is deprecated and is ignored." in
      Arg.(
        value & opt string "dummy"
    ...
    let dump_models_on =
      ...
      Arg.(
        value
        & opt chan (Output.create_channel "stderr")
    ...
    in
    ( Term.(
        const mk_produce_models $ interpretation $ produce_models $ dump_models),
      dump_models,
      dump_models_on,
      frontend )
  in
  ...
  let sat_solver =
    let sat_solver : _ Arg.conv =
      let parse = function
        | "CDCL" | "satML" -> Ok Util.CDCL
        | "CDCL-Tableaux" | "satML-Tableaux" | "CDCL-tableaux"
        | "satML-tableaux" ->
    ...
    let doc =
      Format.sprintf
        "Choose the SAT solver to use. Default value is %s (i.e. %ssolver). \
         Possible options are %s."
        default sum_up
        (Arg.doc_alts
           ["CDCL"; "satML"; "CDCL-Tableaux"; "satML-Tableaux"; "Tableaux-CDCL"])
    in
    let docv = "SAT" in
    Arg.(
      value
      & opt sat_solver Util.CDCL_Tableaux
  ...
  let cdcl_tableaux_inst =
    let no_tableaux_cdcl_in_instantiation =
      ...
      Arg.(
        value & flag
        & info ["no-tableaux-cdcl-in-instantiation"] ~docs:s_sat ~doc)
    in
  ...
  let cdcl_tableaux_th =
    let no_tableaux_cdcl_in_theories =
      let doc =
        "When satML is used, this disables the use of a tableaux-like method \
         for theories with the CDCL solver."
      in
      Arg.(
        value & flag & info ["no-tableaux-cdcl-in-theories"] ~docs:s_sat ~doc)
    in
    Term.(const not $ no_tableaux_cdcl_in_theories)
  in
  let strict_mode =
    let strict_mode_arg =
      let doc = "Enable strict mode (compliance with SMT-LIB standard)" in
  ...
  let objectives_in_interpretation =
    let doc =
      " inline pretty-printing of optimized expressions in the model instead \
       of a dedicated section '(objectives ...)'. Be aware that a part of the \
       model may be shrunk or not accurate if some expressions to optimize are \
       unbounded."
    in
    Arg.(
      value & flag
      & info
          [ "objectives-in-interpretation";
            "objectives-in-model";
            "obj-in-interpretation";
            "obj-in-model" ]
          ~doc)
  in
  let unsat_core =
  ...
  let output_format =
    let doc =
      Format.sprintf
        "Control the output format of the solver, $(docv) must be %s.The \
         alt-ergo native format outputs Valid/Invalid/I don't know.The smtlib2 \
         format outputs unsat/sat/unknown.If left unspecified, Alt-Ergo will \
         use its heuristics to determine the adequate output format according \
         to the input format.It must be noticed that not specifying an output \
  ...
  in
  let set_dump_models = Term.(const set_dump_models $ dump_models) in
  let set_dump_models_on =
    Term.(const Output.set_dump_models $ dump_models_on)
  in
  let set_frontend = Term.(const ignore $ frontend) in
  Term.(
    ret
      (const mk_output_opt $ interpretation $ objectives_in_interpretation
     $ unsat_core $ output_format $ model_type $ set_dump_models
     $ set_dump_models_on $ set_sat_options $ set_frontend))

let parse_profiling_opt =
  let docs = s_profiling in
  let cumulative_time_profiling =
    let doc = "Record the time spent in called functions in callers" in
    Arg.(value & flag & info ["cumulative-time-profiling"] ~docs ~doc)
  in
  let profiling =
    let doc =
      "Activate the profiling module with the given frequency. Use Ctrl-C to \
       switch between different views and \"Ctrl + AltGr + \\\\ \" to exit."
    in
    let docv = "DELAY" in
    Arg.(value & opt (some float) None & info ["profiling"] ~docv ~docs ~doc)
  in
  let profiling_plugin =
    ...
    Arg.(
      value
      & opt string (get_profiling_plugin ())
      & info ["profiling-plugin"] ~docv ~docs ~doc)
  in
  Term.(
    ret
...
let parse_quantifiers_opt =
  ...
  let instantiation_heuristic =
    let doc =
      Format.sprintf
        "Change the instantiation heuristic. $(docv) must be %s. By default %s \
         is used for both sat solvers. %s does one less phase of \
         instantiation. %s use all available ground terms in instantiation."
        (Arg.doc_alts ["normal"; "auto"; "greedy"])
        (Arg.doc_quote "auto") (Arg.doc_quote "normal") (Arg.doc_quote "greedy")
    in
    let docv = "VAL" in
    Arg.(
      value
      & opt instantiation_heuristic_conv IAuto
      & info ["instantiation-heuristic"] ~docv ~docs ~doc)
  in
  let instantiate_after_backjump =
    let doc =
      "Make a (normal) instantiation round after every backjump/backtrack."
    in
    Arg.(value & flag & info ["inst-after-bj"] ~docs ~doc)
  in
  let max_multi_triggers_size =
    let doc =
      "Maximum size of multi-triggers, i.e. the maximum number of independent \
       patterns in a multi-trigger."
    in
    let docv = "VAL" in
    Arg.(
      value
      & opt int (get_max_multi_triggers_size ())
      & info ["max-multi-triggers-size"] ~docv ~docs ~doc)
  in
  let nb_triggers =
    let doc =
    ...
    Arg.(
      value
      & opt int (get_nb_triggers ())
      & info ["nb-triggers"] ~docv ~docs ~doc)
  in
  let no_ematching =
    let doc = "Disable matching modulo ground equalities." in
    Arg.(value & flag & info ["no-ematching"] ~docs ~doc)
  in
  let no_user_triggers =
    let doc = "Ignore user triggers, except for triggers of theories' axioms" in
    Arg.(value & flag & info ["no-user-triggers"] ~docs ~doc)
  in
  let normalize_instances =
    let doc =
      "Normalize generated substitutions by matching w.r.t. the state of the \
       theory. This means that only terms that are greater (w.r.t. depth) than \
       the initial terms of the problem are normalized."
    in
    Arg.(value & flag & info ["normalize-instances"] ~docs ~doc)
  in
  let triggers_var =
    let doc = "Allows variables as triggers." in
    Arg.(value & flag & info ["triggers-var"] ~docs ~doc)
  in
  Term.(
    ret
...
let parse_sat_opt =
  ...
  let get_bottom_classes =
    ...
    Arg.(value & flag & info ["bottom-classes"] ~docs ~doc)
  in
  let disable_flat_formulas_simplification =
    let doc = "Disable facts simplifications in satML's flat formulas." in
    Arg.(
      value & flag & info ["disable-flat-formulas-simplification"] ~docs ~doc)
  in
  let enable_restarts =
    let doc =
      "For satML: enable restarts or not. Default behavior is 'false'."
    in
    Arg.(value & flag & info ["enable-restarts"] ~docs ~doc)
  in
  let no_arith_matching =
    let doc = "Disable (the weak form of) matching modulo linear arithmetic." in
    Arg.(value & flag & info ["no-arith-matching"] ~docs ~doc)
  in
  let no_backjumping =
    let doc = "Disable backjumping mechanism in the functional SAT solver." in
    Arg.(value & flag & info ["no-backjumping"] ~docs ~doc)
  in
  let no_backward =
    let doc =
      "Disable backward reasoning step (starting from the goal) done in the \
       default SAT solver before deciding."
    in
    Arg.(value & flag & info ["no-backward"] ~docs ~doc)
  in
  let no_decisions =
    let doc = "Disable decisions at the SAT level." in
    Arg.(value & flag & info ["no-decisions"] ~docs ~doc)
  in
  let no_decisions_on =
    let doc =
      "Disable decisions at the SAT level for the instances generated from the \
       given axioms. Arguments should be separated with a comma."
    in
    let docv = "[INST1; INST2; ...]" in
    Arg.(value & opt string "" & info ["no-decisions-on"] ~docv ~docs ~doc)
  in
  let no_minimal_bj =
    let doc = "Disable minimal backjumping in satML CDCL solver." in
    Arg.(value & flag & info ["no-minimal-bj"] ~docs ~doc)
  in
  let no_sat_learning =
    let doc =
      "Disable learning/caching of unit facts in the Default SAT. These facts \
       are used to improve bcp."
    in
    Arg.(value & flag & info ["no-sat-learning"] ~docs ~doc)
  in
  Term.(
    const mk_sat_opt $ get_bottom_classes $ disable_flat_formulas_simplification
    $ enable_restarts $ no_arith_matching $ no_backjumping $ no_backward
    $ no_decisions $ no_decisions_on $ no_minimal_bj $ no_sat_learning)

let parse_term_opt =
  let docs = s_term in
  let disable_ites =
    let doc = "Disable handling of ite(s) on terms in the backend." in
    Arg.(value & flag & info ["disable-ites"] ~docs ~doc)
  in
  let inline_lets =
    let doc =
      "Enable substitution of variables bounds by Let. The default behavior is \
       to only substitute variables that are bound to a constant, or that \
       appear at most once."
    in
    Arg.(value & flag & info ["inline-lets"] ~docs ~doc)
  in
  let rewriting =
    let doc = "Use rewriting instead of axiomatic approach." in
    Arg.(value & flag & info ["rwt"; "rewriting"] ~docs ~doc)
  in
  let no_term_like_pp =
    let doc = "Do not output semantic values as terms." in
    Arg.(value & flag & info ["no-term-like-pp"] ~docs ~doc)
  in
  Term.(
    ret
      (const mk_term_opt $ disable_ites $ inline_lets $ rewriting
     $ no_term_like_pp))
...
let parse_theory_opt =
  ...
  let inequalities_plugin =
    let load_inequalities_plugin debug path =
      ...
      | "" ->
        if debug
        then
          Printer.print_dbg "Using the 'FM module' for arithmetic inequalities";
        Ok ()
      | path -> (
        try
          ...
          if debug
          then
            Printer.print_dbg "Using the 'inequalities' reasoner (FM module) %S"
              path;
          Ok ()
        with Errors.Error e -> Error (Format.asprintf "%a" Errors.report e))
    in
    let arg =
      ...
      in
      Arg.(value & opt string "" & info ["inequalities-plugin"] ~docs ~doc)
    in
    let term =
  ...
  let no_contracongru =
    ...
    Arg.(value & flag & info ["no-contracongru"] ~docs ~doc)
  in
  let no_fm =
    let doc = "Disable Fourier-Motzkin algorithm." in
    Arg.(value & flag & info ["no-fm"] ~docs ~doc)
  in
  let no_nla =
    let doc =
      "Disable non-linear arithmetic reasoning (i.e. non-linear multplication, \
       division and modulo on integers and rationals). Non-linear \
       multiplication remains AC."
    in
    Arg.(value & flag & info ["no-nla"] ~docs ~doc)
  in
  let no_tcp =
    let doc =
      "Deactivate Boolean Constant Propagation (BCP) modulo theories."
    in
    Arg.(value & flag & info ["no-tcp"] ~docs ~doc)
  in
  let no_theory =
    let doc = "Completely deactivate theory reasoning." in
    Arg.(value & flag & info ["no-theory"] ~docs ~doc)
  in
  let restricted =
    let doc =
      "Restrict set of decision procedures (equality, arithmetic and AC)."
    in
    Arg.(value & flag & info ["restricted"] ~docs ~doc)
  in
  let tighten_vars =
    let doc = "Compute the best bounds for arithmetic variables." in
    Arg.(value & flag & info ["tighten-vars"] ~docs ~doc)
  in
  let theories =
    ...
    let enable_theories =
      let doc =
        Format.asprintf
          "Enable builtin theory, multiple comma-separated values\n\
    ...
    and disable_theories =
      let doc =
        Format.asprintf
          "Disable builtin theory, multiple comma-separated\n\
    ...
    in
    Term.(
      cli_parse_result (const preludes $ enable_theories $ disable_theories))
  in
  Term.(
    ret
      (const mk_theory_opt $ inequalities_plugin $ no_contracongru $ no_fm
     $ no_nla $ no_tcp $ no_theory $ restricted $ tighten_vars $ theories))
...
let parse_fmt_opt =
  ...
  let regular_output =
    ...
    in
    Arg.(value & opt string "stdout" & info ["regular-output"] ~docs ~doc ~docv)
  in
  let diagnostic_output =
    ...
    in
    Arg.(
      value & opt string "stderr" & info ["diagnostic-output"] ~docs ~doc ~docv)
  in
...
let main =
  let file =
    let doc =
      "Source file. Must be suffixed by $(i,.ae), ($(i,.mlw) and $(i,.why) are \
       deprecated), $(i,.smt2) or $(i,.psmt2)."
    in
  ...
  (* Specify the order in which the sections should appear Default behaviour
     gives an unpleasant result with non standard sections appearing before
     standard ones *)
  let man =
    [ `S Manpage.s_options;
      `S s_execution;
      ...
      `S s_debug;
      `P
        "These options are used to output debug info for the concerned part of \
         the solver.They are $(b,not) used to check internal consistency.";
      `S Manpage.s_bugs;
      `P "You can open an issue on: https://github.com/OCamlPro/alt-ergo/issues";
  ...
  let term =
    Term.(
      ret
       ...
       $ Debug.light_flag_term $ Debug.medium_flag_term $ Debug.full_flag_term
       $ Debug.backtrace_flag $ Rule.flag_term $ parse_execution_opt
       $ parse_halt_opt $ parse_internal_opt $ parse_limit_opt
       $ parse_profiling_opt $ parse_quantifiers_opt $ parse_sat_opt
       $ parse_term_opt $ parse_output_opt $ parse_theory_opt $ parse_fmt_opt))
  in
  let info = Cmd.info "alt-ergo" ~version:Version._version ~doc ~exits ~man in
  Cmd.v info term
