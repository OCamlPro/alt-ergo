...
let parse_quantifiers_opt =
  ...
  let instantiation_heuristic =
    let doc = Format.sprintf
        "Change the set of term used for instantiation. \
         $(docv) must be %s. By default %s is used for the cdcl sat solver \
         and %s is used for the functionnal one. \
    ...
    Arg.(value & opt instantiation_heuristic_conv INormal &
         info ["instantiation-heuristic"] ~docv ~doc) in
