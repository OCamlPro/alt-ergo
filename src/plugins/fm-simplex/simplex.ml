...
type t1 =
  { mutable a : (int * Q.t) array;
    mutable c : Q.t * Q.t
  }

type t2 =
  { mutable a2 : Q.t array;
    mutable c2 : Q.t * Q.t
  }
...
module Simplex (C : Coef_Type) = struct
  ...
  module C2 = struct
    ...
    let compare (c1, k1) (c2, k2) =
      let r = C.compare c1 c2 in
      if r <> 0 then r else C.compare k1 k2
  ...
  module D = struct
    ...
    let matrix_stats matrix co =
      if !dsimplex
      then begin
        print_dbg ~flushed:false "taille: %d x %d@ " (Array.length co.a2)
          (List.length matrix);
        let z = ref 0 in
        let nz = ref 0 in
        List.iter
    ...
    let given_problem co eqs s_neq nb_vars =
      ...
      then begin
        print_dbg ~flushed:false
          "%s@ I am given a problem of size %d:@ @[<v 2> max: %a;@ " sep nb_vars
          poly0 co;
        List.iter
          (fun (_, (pp, pn, ctt)) ->
            print_dbg ~flushed:false ~header:false "(%a)  + (%a) + %s =  0;@ "
    ...
    let auxiliary_problem sbt s_neq co h_i_s =
      ...
      then begin
        ...
        List.iter
          (fun ((s, i), p) ->
            print_dbg ~flushed:false ~header:false "(L%d,%d) |-> %a@ " s i poly
              p)
          sbt;
        let (s, i), pneq = s_neq in
        print_dbg ~header:false "@]s_neq:@ (L%d,%d) |-> %a@ cost:@ %a@ %s" s i
          poly pneq poly co sep
      end
    ...
    let compacted_problem basic non_basic matrix co =
      ...
      then begin
        ...
        H.iter
          (fun i s -> print_dbg ~flushed:false ~header:false "L%i |-> %d@ " s i)
          non_basic;
        ...
        List.iter
          (fun (i, p) ->
            print_dbg ~flushed:false ~header:false "%d |-> %a@ " i (ppoly sp) p)
          matrix;
    ...
    let psystem fmt (ctx, co, distr) =
      ...
      List.iter (fun (i, p) -> fprintf fmt "%d    = %a" i (ppoly sp) p) ctx;
      fprintf fmt "cost = %a" (ppoly sp) co

    let report_unsat ctx co distr =
      if !dsimplex
      then
        print_dbg "%s@ pb aux's result:(E_unsat)@ %a@ %s" sep psystem
          (ctx, co, distr) sep

    let report_max ctx co distr =
      if !dsimplex
      then
        print_dbg "%s@ pb aux's result:(E_max)@ %a@ %s" sep psystem
          (ctx, co, distr) sep

    let given_problem2 ctx co distr =
      if !dsimplex
      then
        print_dbg "%s@ [solve] given pb:@ %a@ %s" sep psystem (ctx, co, distr)
          sep

    let in_simplex ctx co distr =
      if !dsimplex
      then
        print_dbg "%s@ [simplex] I start with:@ %a@ %s" sep psystem
          (ctx, co, distr) sep
    ...
    let retrieved_cost co =
      if !dsimplex then print_dbg "retrieved_const: %a" (ppoly 4) co
    ...
    let psbt fmt sbt =
      ...
      fprintf fmt "%d |->  %a / (old %d)" sbt.lhs (ppoly sp) sbt.rhs sbt.old_lhs

    let choosed_var ch_vr =
      if !dsimplex then print_dbg "choosed var's index: %d" ch_vr

    let choosed_eq ch_eq =
      if !dsimplex then print_dbg "choosed eq:     %a" pline ch_eq

    let pivot_result sbt =
      if !dsimplex then print_dbg "pivot's result: %a" psbt sbt

    let change_pivot ch_vr old_vr =
      if !dsimplex then print_dbg "ch_vr: %d et old_vr: %d" ch_vr old_vr

    let init_simplex_pb ctx co distr ghost line =
      if !dsimplex
      then
        print_dbg
          "%s@ init_simplex: pb_aux@ %a@ choosed var's index: %d@ choosed \
           eq:     %a@ %s"
          sep psystem (ctx, co, distr) ghost pline line sep
  end
  ...
  module Normalizer = struct
    ...
    let subst_in_p ({ a; c } as pp) ((s, lhs), { a = rhs_a; c = rhs_c }) =
      let _, v = a.(lhs) in
    ...
    let normalize_sbt sbt zsbt =
      Vec.iter (fun elt -> Vec.iter (z_subst_in_p (snd elt)) zsbt) sbt;
    ...
    let solve_zero_list zsbt zsbt_inv l =
      if !dsimplex then Printer.print_dbg "[eq_solve] 0 = 0 (modulo Li >= 0)";
      List.iter
        (fun (s, _coef) ->
          if not zsbt_inv.(s)
          then begin
    ...
    let substs_from_equalities eqs h_i_s len =
      if !dsimplex then Printer.print_dbg ~flushed:false "subst from eqs:@ ";
      Vec.clear sbt;
      Vec.clear zsbt;
      ...
      let eqs =
        List.fold_left
          (fun acc (_, ((lp, ln, ctt) as lp_ln)) ->
            ...
            if !dsimplex
            then
              Printer.print_dbg ~flushed:false ~header:false "  >> poly %a@ "
                D.poly p;
            match lp, ln with
            | [], [] -> assert false
            | _ :: _, [] when sg = 0 ->
              solve_zero_list zsbt zsbt_inv lp;
              acc
            | [], _ :: _ when sg = 0 ->
              solve_zero_list zsbt zsbt_inv ln;
              acc
          ...
          try
            ...
            | Some n ->
              ...
              then (
                ...
                then
                  Printer.print_dbg ~flushed:false ~header:false
                    "new pivot (L%d,%d) |-> %a@ " s pivot D.poly p;
                Vec.push sbt ln)
            | _ ->
              let (((s, pivot), p) as ln) = pivot_in_p len p in
              if !dsimplex
    ...
    (* let mm = !mm in let mmT = List.fast_sort (fun {a=a1} {a=a2} -> try for i
       = 0 to Array.length a1 -1 do let _, c1 = a1.(i) in let _, c2 = a2.(i) in
       if C.is_zero c1 then raise (Out 1); if C.is_zero c2 then raise (Out
       (-1)); let c = C.compare c1 c2 in if c <> 0 then raise (Out c) done; 0
       with Out c -> c ) mm in fprintf fmt "MM@."; List.iter (fun p -> fprintf
       fmt "%a@." D.poly01 p; )mm; fprintf fmt "MMT@."; List.iter (fun p ->
       fprintf fmt "%a@." D.poly01 p; )mmT; *)
    ...
    let make_problem co eqs s_neq len =
      ...
      let print fmt i = fprintf fmt "L%i -> 0 ;@ " i in
      if !dsimplex
      then
        Printer.print_dbg ~flushed:false "@[<v 2>ZERO substs:@ %a"
    ...
    let compact_poly_2 { a; c } base new_len h_zsbt =
      let non_basic = H.create 101 in
      let old_i = ref 0 in
      let f3 i =
        while H.mem base !old_i || H.mem h_zsbt !old_i do
          incr old_i
        done;
        let s, c = a.(!old_i) in
        H.add non_basic i s;
    ...
    let compact_poly { a; c } base new_len h_zsbt =
      let old_i = ref 0 in
      let f4 _ =
        while H.mem base !old_i || H.mem h_zsbt !old_i do
          incr old_i
    ...
    (* XXX : Faire des simplifications pour eliminer les lignes triviales de la
       matrice de la forme s_i |-> ctt ??? XXX : Faire des simplifications pour
       eliminer les colonnes nulles ??? *)
    let compact_problem co matrix len new_len zsbt =
      ...
      let matrix =
        List.fold_left
          (fun acc ((_, p) as line) ->
            if !dsimplex
            then Printer.print_dbg "compact_problem: LINE %a" D.pline line;
            if array_is_null p.a2
            then
      ...
      let distr =
        Array.init len (fun i ->
            ...
            with Not_found -> (
              ...
              with Not_found ->
                if !dsimplex
                then Printer.print_dbg "Colonne vide ! donc supprimee";
                -20000))
      in
      co, matrix, distr
    ...
    let norm_main co eqs s_neq nb_vars =
      ...
      if !dsimplex
      then Printer.print_dbg "new_len = %d (excluant les pivots)" new_len;
  ...
  module Core_Simplex = struct
      ...
      | line :: ctx ->
        List.fold_left
          (fun ((_, p') as line') ((_, p) as line) ->
            if C2.compare p.c2 p'.c2 < 0 then line else line')
          line ctx

    let subst ({ a2; c2 } as pp) lhs { a2 = rhs_a2; c2 = rhs_c2 } =
      let v = a2.(lhs) in
      if not (C.is_zero v)
      then begin
        a2.(lhs) <- C.zero;
    ...
    let subst_line { old_lhs; lhs; rhs } (i, p) =
      if i = old_lhs
      then begin
        p.a2 <- rhs.a2;
        p.c2 <- rhs.c2
    ...
    let change_pivot ch_vr (old_vr, { a2 = old_a; c2 }) distr _order =
      ...
      { old_lhs = old_vr;
        lhs = ch_vr;
        rhs = mult_const old_a c2 (C.div C.m_one v)
      }
    ...
    let loops distr order co_cst =
      if C2.compare co_cst !last_cost = 0
      then begin
        incr cpt;
    ...
    let rec simplex ctx co distr order =
      (* let ppoly fmt {a2=a2; c2=c2} = Array.iter (fun c -> fprintf fmt "%s "
         (C.to_string c)) a2; fprintf fmt " %s@." (C2.to_string c2) in let
         psystem fmt (ctx, co, distr) = fprintf fmt "@.tbl: "; Array.iteri (fun
         i s -> fprintf fmt "%d -> %s | " i s) distr; fprintf fmt "@.@.";
         List.iter (fun (i,p) -> fprintf fmt "%d = %a" i ppoly p) ctx; fprintf
         fmt "cost = %a@." ppoly co in fprintf fmt
         "@.####################################################@."; fprintf fmt
         "%a" psystem (ctx, co, distr); fprintf fmt
         "@.####################################################@."; *)
      if !main_simplex && loops distr order co.c2 then raise (E_max (ctx, co));
      incr nbloops;
      (* fprintf fmt "nb_loops = %d @." !nbloops; fprintf fmt "ici: main = %b :
         %s @." !main_simplex (C2.to_string co.c2); (*D.matrix_stats ctx
         co;*) *)
      if
        !main_simplex
        && C.compare (fst co.c2) C.zero >= 0
        && C.compare (snd co.c2) C.zero >= 0
      then raise (E_max (ctx, co));
    ...
    let delete_ghost ghost ghost_p ctx distr order =
      ...
      try
        for i = 0 to !len - 1 do
          if C.compare ghost_p.a2.(i) C.zero <> 0
          then (
            ch_vr := i;
            raise Exit)
        done;
    ...
    let init_simplex ctx ((_, p) as line) distr order =
      ...
      subst_ctx ctx sbt;
      subst co sbt.lhs sbt.rhs;
      try simplex ctx co distr order with
    ...
    let solve co ctx distr order =
      ...
      try
        ...
        if C2.compare p.c2 C2.zero >= 0
        then simplex ctx co distr order
        else begin
          co_opt := Some co;
          main_simplex := false;
    ...
    let infos_of distr q { c2; _ } ctx =
      let acc0 =
        List.fold_left
          (fun acc (i, p) -> if C2.is_zero p.c2 then acc else (i, p.c2) :: acc)
          [] ctx
      in
      let acc = ref [] in
      let inf i s =
        if s > 0
        then try acc := (s, List.assoc i acc0) :: !acc with Not_found -> ()
      in
      Array.iteri inf distr;
      { vof = c2; vals = !acc; ctx; distr; order = q }
    ...
    let result_extraction distr q = function
      | I_max (ctx_ex, co_ex) ->
        ...
        if !dsimplex
        then Printer.print_dbg ">result size %d" (List.length res.vals);
        Max res
  ...
  let main co eqs s_neq nb_vars =
    ...
    (*incr cpt;
      fprintf fmt "%d@." !cpt; *)
    let res =
      D.given_problem co eqs s_neq nb_vars;
      try
        (*fprintf fmt "avant norm@."; fprintf fmt " nb_eqs = %d@." (List.length
          eqs); fprintf fmt " nb_vars = %d@." nb_vars;*)
        let co, matrix, distr = Normalizer.norm_main co eqs s_neq nb_vars in
        (*fprintf fmt "apres norm@."; fprintf fmt " nb_sbts = %d@." (List.length
          matrix); fprintf fmt " nb_vars = %d@." (Array.length distr);*)
  ...
  let subst_spec ({ a2; c2 } as pp) v { a2 = rhs_a2; c2 = rhs_c2 } =
    if not (C.is_zero v)
    then begin
      let f7a j w =
        let rhs_j = rhs_a2.(j) in
  ...
  let partial_restart res (max_ctt : (int * Q.t) list) =
    ...
    if !dsimplex
    then
      Printer.print_dbg ~flushed:false "new: %a"
        (Printer.pp_list_no_space print)
        max_ctt;
    ...
    | Unsat rr | Unbound rr | Max rr -> (
      ...
      | [] -> assert false
      | (_, a) :: _ ->
        if !dsimplex then Printer.print_dbg ~header:false "tbl: @ ";
        if !dsimplex
        then
          Array.iteri
            (fun i s ->
              Printer.print_dbg ~flushed:false ~header:false "%d -> L%d | @ " i
                s)
            rr.distr;
        let len = Array.length a.a2 in
        let cost = { a2 = Array.make len Q.zero; c2 = Q.zero, Q.zero } in
        Array.iteri
          (fun i ld ->
            if !dsimplex
            then
              Printer.print_dbg ~flushed:false ~header:false
                "> AVANT: cost: %a@ traitement de l'index %d@ "
                (D.ppoly (D.max_poly cost))
                cost i;
            begin try
              ...
              try cost.a2.(i) <- Q.add cost.a2.(i) q
        ...
        if !dsimplex
        then
          Printer.print_dbg ~header:false "> RES cost: %a"
            (D.ppoly (D.max_poly cost))
            cost;
        (* XXX *)
        ...
        (* XXX *)
        let res = Core_Simplex.solve cost rr.ctx rr.distr (rr.order, []) in
        let res = Core_Simplex.result_extraction rr.distr rr.order res in
        (*XXX*Timer.Simplex_main.stop();*)
        res)

  (* let main co eqs s_neq nb_vars = let res = main co eqs s_neq nb_vars in let
     res' = partial_restart res co in (match res, res' with | Eq_unsat, Eq_unsat
     -> () | Unsat rr, Unsat r -> (*let r1, i1 = rr.vof in let r2, i2 = r.vof in
     assert (Q.equal r1 r2 && Q.equal i1 i2) *) ()

     | Unbound rr,Unbound r -> let r1, i1 = rr.vof in let r2, i2 = r.vof in
     assert (Q.equal r1 r2 && Q.equal i1 i2)

     | Max rr, Max r -> let r1, i1 = rr.vof in let r2, i2 = r.vof in assert
     (Q.equal r1 r2 && Q.equal i1 i2)
