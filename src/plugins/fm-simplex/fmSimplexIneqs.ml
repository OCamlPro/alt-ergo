...
module Container : Inequalities.Container_SIG = struct
  module Make (P : Polynome.T with type r = Shostak.Combine.r) :
    ...
    module SCache = Simplex_cache.MAKE (struct
      type t = X.r
    ...
    let print_answer (vof, vals) =
      let print fmt (l, v) = fprintf fmt "L(%d) -> %a@," l print_couple v in
      Printer.print_dbg ~header:false
        "vof = %a@ @[<v 2>assignement returned by the Simplex@ %a" print_couple
        vof
    ...
    let print_parsed_answer answer =
      ...
      then
        match answer with
        | Unsat { vof; vals; _ } ->
          Printer.print_dbg "I read: the simplex problem is not feasible (<=)";
          print_answer (vof, vals)
        | Eq_unsat ->
          Printer.print_dbg "I read: the simplex problem is not feasible (=)"
        | Unbound { vof; vals; _ } ->
          Printer.print_dbg "I read: the simplex problem is not bounded";
          print_answer (vof, vals)
        | Max { vof; vals; _ } ->
          Printer.print_dbg "I read: the simplex problem has a solution";
          print_answer (vof, vals)
    ...
    let polynomials_bounding_pb sum ctt lambdas =
      let vars_elim_eqs =
        MX.fold (fun x (l1, l2) acc -> (x, (l1, l2, Q.zero)) :: acc) sum []
      in
    ...
    let explain vals constrs =
      List.fold_left
        (fun expl (ld, (re, eps)) ->
          if Q.compare re Q.zero = 0 && Q.compare eps Q.zero = 0
          then expl (* XXX eps ? re ? *)
          else
    ...
    let tighten_polynomials add_ineqs are_eq acc sum ctt lambdas nb_constrs
        constrs =
      if get_debug_fm ()
      then Printer.print_dbg ~flushed:false "tighten_polynomials@ ";
      ...
      let sim_res =
        ...
        | None ->
          if !dsimplex
          then
            Printer.print_dbg ~flushed:false ~header:false "Simplex poly in@ ";
          incr cpt;
          if !dsimplex
          then
            Printer.print_dbg ~flushed:false ~header:false "new simplex %d@ "
              !cpt;
          let res = Simplex_Q.main max_ctt equas s_neq nb_constrs in
          if !dsimplex then Printer.print_dbg ~header:false "Simplex poly out";
          SCache.register r_max_ctt r_equas r_s_neq !cpt res;
          res
        | Some (n, res, ctt') ->
          if SCache.MI.compare Q.compare r_max_ctt ctt' = 0
          then begin
            if !dsimplex then Printer.print_dbg "reuse RESULTS of simplex %d" n;
            res
          end
          else begin
            if !dsimplex then Printer.print_dbg "reuse  simplex %d" n;
    ...
    let tighten_monomial add_ineqs are_eq acc x sum_x is_pos sum ctt lambdas
        nb_constrs constrs =
      if get_debug_fm ()
      then
        Printer.print_dbg ~flushed:false "tighten_monomial %s%a "
          (if is_pos then "+" else "-")
          X.print x;
      let max_ctt, equas, s_neq =
        monomial_bounding_pb sum ctt lambdas x sum_x is_pos
      in
      let r_max_ctt, r_equas, r_s_neq = SCache.make_repr max_ctt equas s_neq in
      let sim_res =
        ...
        | None ->
          ...
          let res = Simplex_Q.main max_ctt equas s_neq nb_constrs in
          if !dsimplex
          then Printer.print_dbg ~header:false "Simplex monomes out";
        ...
        | Some (n, res, ctt') ->
          ...
          then begin
            if !dsimplex
            then Printer.print_dbg ~header:false "reuse RESULTS of simplex %d" n;
            res
          end
          else begin
            if !dsimplex
            then Printer.print_dbg ~header:false "reuse  simplex %d" n;
            let res = Simplex_Q.partial_restart res max_ctt in
            res
          end
      in
    ...
    let tighten_monomials add_ineqs are_eq acc sum ctt lds nb_ctrs ctrs =
      MX.fold
        (fun x sum_x acc ->
          ...
          let acc =
            tighten_monomial add_ineqs are_eq acc x sum_x true sum ctt lds
              nb_ctrs ctrs
          in
          let acc =
            tighten_monomial add_ineqs are_eq acc x sum_x false sum ctt lds
              nb_ctrs ctrs
          in
          acc)
        sum acc
    ...
    let fm_simplex add_ineqs are_eq acc constrs nb_constrs =
      let print fmt (id, { ple0; _ }) =
        fprintf fmt "%d) %a <= 0" id P.print ple0
      in
      if get_debug_fm ()
      then
        Printer.print_dbg "begin fm-simplex: nb_constrs = %d@ %a" nb_constrs
          (Printer.pp_list_no_space print)
          constrs;
      ...
      let acc =
        ...
        then acc
          ...
          let acc =
            tighten_polynomials add_ineqs are_eq acc sum ctt lambdas nb_constrs
              constrs
          in
          if Options.get_tighten_vars ()
          then
            tighten_monomials add_ineqs are_eq acc sum ctt lambdas nb_constrs
              constrs
          else acc
      in
      if get_debug_fm () then Printer.print_dbg "end fm-simplex";
      acc
    ...
    let check_is_rat mp =
      ...
      begin try
        MINEQS.iter
          (fun p _ ->
            is_rat := is_rat_poly p;
            raise Exit)
          mp
      with Exit -> ()
      end;
      let is_rat = !is_rat in
      assert (MINEQS.fold (fun p _ z -> z && is_rat == is_rat_poly p) mp true);
      is_rat

    let fmSimplex add_ineqs are_eq acc mp =
      if check_is_rat mp
      then fourierMotzkin add_ineqs are_eq acc mp
      else
...
let () =
  Inequalities.set_current (module Container : Inequalities.Container_SIG)
