...
(* This plugin allows to solve polynomial goals (conjunctions of
   polynomial inequalities) using a Sum of Squares (SOS) encoding into
   a SemiDefinite Programming (SDP) optimization problem.

   Details can be found in the paper: A Non-linear Arithmetic
   Procedure for Control-Command Software Verification, Pierre Roux,
   Mohamed Iguernlala, Sylvain Conchon, TACAS 2018

   In practice, the code in this file mostly translates Alt-Ergo
   polynomials to the input format of the OSDP library, calls the
   library and raises an Intervals.NotConsistent exception, with
   explanations (set of inconsistent polynomial constraints), when
   OSDP succeeds to prove inconsistency. *)
open AltErgoLib

module PolyIneqs : PolynomialInequalities.S = struct
  ...
  let p_to_osdp p =
    ...
    let rec trans_monom t env next = match Expr.term_view t with
      | Expr.Term { Expr.f = Symbols.Op Symbols.Mult ; xs = l ; _ } ->
        List.fold_left
          (combine_opt_env SosP.mult trans_monom)
          (Some (SosP.one, env, next)) l
      | _ ->
        let v, env, next =
          try Expr.Map.find t env, env, next
          with Not_found -> next, Expr.Map.add t next env, next + 1 in
      ...
      List.fold_left
        (fun opt (c, m) ->
           combine_opt_env
             ...
             trans_monom opt m)
        (Some (SosP.const (Q.to_zarith pc), Expr.Map.empty, 0)) pm in
    match opt with
    | None -> None
  ...
  let partition polys =
    let uf =
      List.fold_left
        (fun uf (_, env) ->
           match Expr.Map.bindings env with
           | [] -> uf
           | (t, _) :: env ->
    ...
    List.fold_left
      (fun m (pi, env) ->
         try
           let t, _ = Expr.Map.choose env in
           let r, _ = Uf.find uf t in
  ...
  let merge polys =
    ...
    let merge_env env env' =
      let next, trans =
        let sz e = 1 + Expr.Map.fold (fun _ -> max) e (-1) in
        sz env, Array.make (sz env') 0 in
      let env'', _ =
        Expr.Map.fold
          (fun v i (env, next) ->
             let j, env, next =
               try Expr.Map.find v env, env, next
               with Not_found -> next, Expr.Map.add v next env, next + 1 in
             trans.(i) <- j;
             env, next)
          env' (env, next) in
      env'', trans in
            ...
            List.fold_left
              (fun (sum, sigmas) (p, d) ->
                 let s =
                   ...
                   Sos.of_list l in
                 (* Format.printf *)
                 (*   "p = %a ~~> s = %a@." Sos.Poly.pp p Sos.pp s; *)
                 Sos.(sum - s * !!p), s :: sigmas)
              (Sos.(!!Poly.zero), []) (List.combine pge0 degs) in
          (* Format.printf "sum: %a@." Sos.pp sum; *)
          (* Format.printf "degrees: @[%a@]@." (Osdp.Utils.pp_list ~sep:",@ "
               (fun fmt p -> Format.fprrintf \
  ...
  let test_polynomes polys =
    ...
    if Options.get_debug_sdp () then
      Printer.print_dbg "[sdp] %d partition(s)@." (List.length polys);
    polys
    |> List.iter
      (fun polys ->
         ...
         if List.exists (fun ((p, _), _) -> SosP.degree p > 1) polys then
           ...
           let res =
             try
               ...
               if Options.get_debug_sdp () then
                 Printer.print_dbg "[sdp] reuse result in cache@.";
               tmp
             with Not_found ->
               if Options.get_debug_sdp () then begin
                 Printer.print_dbg "[sdp] new problem@.";
                 Printer.print_dbg
                   "[sdp] @[<v>%a@]@."
                   (Osdp.Utils.pp_list
                      ~sep: ",@ "
                      (fun fmt (p, _) ->
                         Format.fprintf fmt "%a >= 0" SosP.pp p))
                   pge0;
               end;
           ...
           | Some expls ->
             if Options.get_debug_sdp () then
               Printer.print_dbg "[sdp] Inconsistent@.";
             raise (I.NotConsistent expls)
      )
end

let () =
  PolynomialInequalities.set_test_polynomes PolyIneqs.test_polynomes
